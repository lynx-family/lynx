...
TEST(TransformOperationsTest, MatchingPrefixLength) {
  ...
  TransformOperations no_common_prefix;
  no_common_prefix.AppendSkew(10.0f, 20.0f);
  no_common_prefix.AppendScale(4.0f, 4.0f);
  EXPECT_EQ(left.MatchingPrefixLength(no_common_prefix), 0u);
}
...
TEST(TransformOperationsTest, EmptyListMatchesIdentityOperations) {
  ...
  TransformOperations skewed;
  skewed.AppendSkew(0.0f, 0.0f);
  EXPECT_EQ(skewed.MatchingPrefixLength(empty), skewed.size());

  TransformOperations rotated_y;
  rotated_y.AppendRotate(TransformOperation::kRotateY, 0.0f);
  EXPECT_EQ(rotated_y.MatchingPrefixLength(empty), rotated_y.size());

  TransformOperations rotated_z;
  rotated_z.AppendRotate(TransformOperation::kRotateZ, 0.0f);
  EXPECT_EQ(rotated_z.MatchingPrefixLength(empty), rotated_z.size());

  TransformOperations multiple;
  multiple.AppendTranslate(kZero, kZero, kZero);
  multiple.AppendScale(1.0f, 1.0f);
  multiple.AppendSkew(0.0f, 0.0f);
  multiple.AppendRotate(TransformOperation::kRotateZ, 0.0f);
  EXPECT_EQ(multiple.MatchingPrefixLength(empty), multiple.size());
}
...
TEST(TransformOperationsTest, MatrixFallbackBlendsMismatchedLists) {
  TransformOperations from;
  from.AppendScale(1.0f, 1.0f);
  from.AppendTranslate({10.0f, LengthUnit::kNumber},
                       {20.0f, LengthUnit::kNumber}, kZero);

  TransformOperations to;
  to.AppendTranslate({110.0f, LengthUnit::kNumber},
                     {70.0f, LengthUnit::kNumber}, kZero);
  to.AppendScale(1.0f, 1.0f);

  TransformOperations result;
  ASSERT_TRUE(
      to.TryBlend(from, 0.25f, kReferenceWidth, kReferenceHeight, &result));
  ASSERT_EQ(result.size(), 1u);
  EXPECT_EQ(result.GetOperations()[0].type, TransformOperation::kMatrix3d);

  const Matrix44 matrix =
      result.ApplyRemaining(0, kReferenceWidth, kReferenceHeight);
  EXPECT_NEAR(matrix.rc(0, 0), 1.0f, 0.0001f);
  EXPECT_NEAR(matrix.rc(1, 1), 1.0f, 0.0001f);
  EXPECT_NEAR(matrix.rc(0, 3), 35.0f, 0.0001f);
  EXPECT_NEAR(matrix.rc(1, 3), 32.5f, 0.0001f);
}
