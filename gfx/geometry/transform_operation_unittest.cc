...
TEST(TransformOperationTest, BlendsPercentageZAsRawStackingValue) {
  TransformOperation from =
      MakeTranslate({}, {}, {100.0f, LengthUnit::kPercent});
  TransformOperation to = MakeTranslate({}, {}, {50.0f, LengthUnit::kPercent});

  TransformOperation result;
  ASSERT_TRUE(TransformOperation::BlendTransformOperations(
      &from, &to, 0.5f, 0.0f, 0.0f, &result));
  ExpectTranslate(result, {}, {}, {75.0f, LengthUnit::kNumber});
  EXPECT_FLOAT_EQ(result.GetMatrix(0.0f, 0.0f).rc(2, 3), 75.0f);
}
