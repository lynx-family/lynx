...
TEST(IOSAnimationCapabilitiesTest, ReusesImmutableCapabilityStorage) {
  const auto& first = GetIOSAnimationBackendCapabilities();
  const auto& second = GetIOSAnimationBackendCapabilities();
  EXPECT_EQ(&first, &second);
  EXPECT_EQ(first.properties.data(), second.properties.data());
}
...
TEST(IOSAnimationCapabilitiesTest, DeclaresKeyframeAndTransitionProperties) {
  ...
  EXPECT_EQ(capabilities.event_capabilities.size(), 2u);
  for (const auto kind :
       {AnimationKind::kKeyframe, AnimationKind::kTransition}) {
    EXPECT_TRUE(capabilities.SupportsEvents(kind, kAnimationEventStart |
                                                      kAnimationEventEnd |
                                                      kAnimationEventCancel));
    EXPECT_FALSE(capabilities.SupportsEvents(kind, kAnimationEventIteration));
...
TEST(IOSAnimationCapabilitiesTest, DeclaresDetailedTransformFeatures) {
  ...
  for (const auto kind :
    ...
    EXPECT_EQ(transform->transform.translate_x_units, kAllTransformUnits);
    EXPECT_EQ(transform->transform.translate_y_units, kAllTransformUnits);
    EXPECT_EQ(transform->transform.translate_z_units, kTransformUnitNumber);
