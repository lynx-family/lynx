...
EVENTS = {
    "start": "kAnimationEventStart",
    "end": "kAnimationEventEnd",
    "cancel": "kAnimationEventCancel",
    "iteration": "kAnimationEventIteration",
}
...
TRANSFORM_OPERATION_FIELDS = {
    "translate": (("x", UNITS), ("y", UNITS), ("z", UNITS)),
...
def _transform_assignments(capability: Dict[str, object], index: int) -> List[str]:
    ...
    for operation_name, fields in TRANSFORM_OPERATION_FIELDS.items():
        ...
        for field_name, mapping in fields:
            ...
            target_field = {
                ("translate", "x"): "translate_x_units",
                ("translate", "y"): "translate_y_units",
                ("translate", "z"): "translate_z_units",
...
def generate_header(document: Dict[str, object]) -> str:
    unknown_root_fields = set(document) - {"backend", "capabilities", "events"}
    ...
    blocks: List[str] = []
    events = _require_mapping(document.get("events", {}), "events")
    for kind, names in events.items():
        if kind not in KINDS:
            raise ValueError(f"unsupported event animation kind: {kind}")
        if names == []:
            continue
        mapped_events = _mapped_values(names, EVENTS, f"events.{kind}")
        blocks.append(
            "  capabilities.event_capabilities.push_back(\n"
            f"      {{AnimationKind::{KINDS[kind]}, {_mask(mapped_events)}}});")
