...
#include "third_party/binding/gen_test/test_element.h"
...
Napi::Class* NapiTestElement::Class(Napi::Env env) {
  ...
  base::InlineVector<Wrapped::PropertyDescriptor, 1> props;
  ...
  // Cache the class
  clazz = new Napi::Class(Wrapped::DefineClass(env, "TestElement", props.size(), props.data<const napi_property_descriptor>()));
...
void NapiTestElement::Install(Napi::Env env, Object& target) {
  if (target.Has("TestElement").FromMaybe(false)) {
    return;
  }
