    ...
    class Autolink
      class << self
        ...
        def generate_registry(output_dir, extensions)
          ...
          File.write(File.join(output_dir, 'ExtensionRegistry.h'), header_source)
          File.write(File.join(output_dir, 'ExtensionRegistry.m'), implementation_source(components))
        ...
        def podspec_source
          <<~PODSPEC
            Pod::Spec.new do |s|
              ...
              s.author = 'Lynx'
              s.source = { :path => '.' }
              s.source_files = 'ExtensionRegistry.{h,m}'
              s.dependency 'Lynx'
              s.requires_arc = true
            end
          PODSPEC
