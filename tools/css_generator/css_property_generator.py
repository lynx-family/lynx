...
"""
  ...
  """
    ...
    for val in json_obj:
        ...
        css_str += template.substitute(
            name=utils.underline2hump(val["name"]),
            css_name=val["name"],
        ...
        if val["consumption_status"] != "skip":
            if val["consumption_status"] == "layout-only":
                layout_macro_str += layout_template.substitute(
                    name=utils.underline2hump(val["name"]),
        ...
        i = i + 1
        css_java_str += css_java_template.substitute(
            name=utils.underline2hump(val["name"]), id=val["id"] + 1
        )
        css_java_constant_str += css_java_constant_str_template.substitute(
            css_name=val["name"]
        )
  ...
  """
    return (
        define_template.substitute(css=css_str),
        css_java_str,
        css_java_constant_str,
        layout_macro_template.substitute(css=layout_macro_str),
    )
...
def genCSSProperty(json_obj):
    (
        css_properties,
        css_java_properties,
        css_java_constant_properties,
        layout_properties_macro,
    ) = gen(json_obj)
    utils.genPropID(utils.getCSSPropertyPath(), css_properties, layout_properties_macro)
    utils.genTest(utils.getCSSPropertyTestPath(), json_obj)
    # utils.fillTemplate(utils.getJavaCSSPropertyPath(), css_java_properties)

    utils.genJavaPropId(
        utils.getJavaCSSPropertyPath(),
        css_java_properties,
        css_java_constant_properties,
    )
