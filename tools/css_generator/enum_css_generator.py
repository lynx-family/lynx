...
"""
    ...
    utils.genCSSType(
        utils.getCSSStylePath() + "auto_gen_css_type.h", "".join(css_types_source)
    )
...
"""
    ...
    utils.genJavaStyleConstants(
        utils.getAutoGenJavaStyleConstantPath(), "".join(source)
    )
...
"""
    ...
    for css_json in css_object_jsons:
        ...
        source += enum_template.substitute(
            class_name=class_name, enum_values=enum_values_source
        )
...
def genEnumToStringHeader(handler_name, css_object_jsons):
    ...
    utils.genAutoGenCSSDecoderHeader(
        utils.getAutoGenCSSDebugHeaderPath(), "".join(source)
    )
...
${cases}  }
    ...
    #                     ''.join(source))
    utils.genAutoGenCSSDecoderSource(
        utils.getAutoGenCSSDebugSourcePath(), "".join(source)
    )
