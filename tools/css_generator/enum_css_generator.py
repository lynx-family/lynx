...
""")
    enum_values_template = string.Template(
        """  k${enum_value} = ${index},   // version:${version}
""")
    css_types_source = ''
    for css_json in css_object_jsons:
        ...
        css_types_source += enum_template.substitute(
            class_name=class_name, enum_values=enum_values_source)
    utils.genCSSType(utils.getCSSStylePath() + "auto_gen_css_type.h",
                     ''.join(css_types_source))
...
def genJavaEnum(handler_name, css_object_jsons):
    enum_values_java_template = string.Template(
        """  public static final int ${class_name}_${enum_value} = ${index};  // version:${version}
""")
    source = ''
    for css_json in css_object_jsons:
        class_name = utils.underline2hump(css_json['name'])
        consumption_status = css_json['consumption_status']
        if (consumption_status == 'layout-only'
                or handleSpecialID(class_name)):
            continue
    ...
    utils.genJavaStyleConstants(utils.getAutoGenJavaStyleConstantPath(),
                                ''.join(source))


def genStringToEnum(handler_name, css_object_jsons):
    source = ''
    func_statement_template = string.Template(
        """using starlight::${class_name}Type;
static bool To${class_name}Type(std::string_view str, int& result) {
  ...
  return true;
    ...
    if_source_template = string.Template("""if (str == "${enum_value}") {
        ...
        for i, val in enumerate(values):
            ...
            if i == 0:
                if_source = if_source_template.substitute(
                    ...
                    enum_value=enum_value,
                    enum_upper_value=enum_upper_value)
            else:
...
def genOCEnum(handler_name, css_object_jsons):
    enum_template = string.Template(
        """typedef NS_ENUM(NSUInteger, Lynx${class_name}Type) {
${enum_values}};
\n""")
    enum_values_template = string.Template(
        """  Lynx${class_name}${enum_value} = ${index},   // version:${version}
""")
    source = ''
    for css_json in css_object_jsons:
        enum_values_source = ""
        ...
        for i, val in enumerate(css_json['values']):
            ...
            enum_values_source += enum_values_template.substitute(
                ...
                index=i,
                version=val['version'])
        source += enum_template.substitute(class_name=class_name,
                                           enum_values=enum_values_source)
    # utils.fillTemplate(utils.getOCStyleConstantPath(), ''.join(source))
    utils.genOCLynxCSSType(utils.getAutoGenOCStyleConstantPath(),
                           ''.join(source))
...
def genEnumToStringHeader(handler_name, css_object_jsons):
    ...
    for css_json in css_object_jsons:
        class_name = utils.underline2hump(css_json['name'])
        func_define_template = string.Template(
            """  static std::string To${class_name}Type(lynx::starlight::${class_name}Type type);\n\n"""
        )
        source += func_define_template.substitute(class_name=class_name)
    # utils.fillTemplate(utils.getCSSDebugHeaderPath(), ''.join(source))
    utils.genAutoGenCSSDecoderHeader(utils.getAutoGenCSSDebugHeaderPath(),
                                     ''.join(source))


def genEnumToStringSource(handler_name, css_object_jsons):
    func_statement_template = string.Template(
        """std::string AutoGenCSSDecoder::To${class_name}Type(${class_name}Type type) {
  switch (type) {
${cases}  }
}\n\n""")
    cases_template = string.Template(
        """    case ${class_name}Type::k${enum_upper_value}:
      return "${enum_value}";\n""")
    source = ''
    for css_json in css_object_jsons:
        class_name = utils.underline2hump(css_json['name'])
...
#                     ''.join(source))
    utils.genAutoGenCSSDecoderSource(utils.getAutoGenCSSDebugSourcePath(),
                                     ''.join(source))
