...
def cmake_list_entry_context(cmake_min_version, project_name, cmake_impl_path):
  cmake_list_entry_content = '''# Automatically generated by %s
...
include(${CMAKE_CURRENT_SOURCE_DIR}/CMakeLists_impl/${FLAVOR_NAME}${BUILD_TYPE}${ANDROID_ABI}/%s)
''' % (os.path.basename(__file__), cmake_min_version, project_name, cmake_impl_path)

  return cmake_list_entry_content
...
def generate_cmake_list_entry_script(cmake_target):
  cmake_min_version = cmake_target.cmake_version
  project_name = cmake_target.output_name
  start_target_path = cmake_target.output_path
  cmake_impl_path = cmake_target.file_name

  cmake_list_entry_path = os.path.join(start_target_path, cmake_impl_path)
  if os.path.exists(cmake_list_entry_path):
    return 0
  entry_context = cmake_list_entry_context(cmake_min_version, project_name, cmake_impl_path)
  writer = Writer(cmake_list_entry_path)
  writer.out.write(entry_context)

  return 0
  
  
def write_project(project, target, is_generate_for_gradle):
  ...
  cmake_impl_relative_path = target.file_name
  if is_generate_for_gradle:
    cmake_impl_relative_path = os.path.join('CMakeLists_impl', cmake_secondary_dir_name, target.file_name)
  cmake_out_path = os.path.join(target_path, cmake_impl_relative_path)
  ...
  for sub in start_target.sub_cmake_target:
    sub_target = Target(sub, project)
    r |= write_project(project, sub_target, is_generate_for_gradle)
  return r

def gn_to_cmake(project_json_object, cmake_targets, is_generate_for_gradle, only_generate_entries):
  ...
  for target_name in cmake_targets:
    ...
    if is_generate_for_gradle:
      r |= generate_cmake_list_entry_script(cmake_target)
      if only_generate_entries:
        continue
    r |= write_project(project, cmake_target, is_generate_for_gradle)
  return r
...
def parse_args(arg_tuple, ret):
  key = arg_tuple[0]
  value = arg_tuple[1]
  arg_options = [
    '--cmake-target',
    '--gen-for-gradle',
    '--only-generate-entries'
  ]
  if key in arg_options:
    ret[key] = value
  else:
    arg_options_str = '`{}` '.format(*arg_options)
    print(f'Currently, only the parameters [{arg_options_str}] are supported for setting.')
...
def main():
  ...
  if len(sys.argv) > 1:
    ...
    args = {}
    if len(sys.argv) > 2:
      arg_lists = sys.argv[2].split(",")
      for arg in arg_lists:
        arg_tuple = arg.split("=")
        parse_args(arg_tuple, args)
    cmake_target = args.get("--cmake-target", "")
    is_generate_for_gradle = args.get("--gen-for-gradle", "false") == "true"
    only_generate_entries = args.get("--only-generate-entries", "false") == "true"
    cmake_targets = []
    if cmake_target != "":
    ...
    if len(cmake_targets) <= 0:
      print("You haven't defined the cmake_target in your GN project yet.")
      return -1
    print("cmake_targets: ", cmake_targets)
    
    return gn_to_cmake(project, cmake_targets, is_generate_for_gradle, only_generate_entries)
  else:
