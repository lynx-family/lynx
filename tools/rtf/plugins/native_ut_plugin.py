...
from core.utils.gn_args_helper import inject_flutter_cxx, inject_use_xcode
...
class NativeUTListener(Options.OptionsListener):
    ...
    def did_include(self, options, builders, targets, coverage):
        ...
        inject_use_xcode(builders, options.get_bool("enable_use_xcode", False))
...
class NativeUTPlugin(Plugin):
    ...
    def accept(self, args):
        ...
        for template_name in template_names:
            ...
            with open(template, "r") as template_file:
                ...
                enable_use_xcode = getattr(args, "enable_use_xcode", False)
                context.insert_variable(
                    "enable_flutter_cxx", "true" if enable_flutter_cxx else "false"
                )
                context.insert_variable(
                    "enable_use_xcode", "true" if enable_use_xcode else "false"
                )
                ...
                inject_use_xcode(
                    context.find_variable("builder") or {}, enable_use_xcode
                )
    ...
    def build_command_args(self, subparser):
        ...
        subparser_subparser.add_argument(
            "--enable-use-xcode",
            dest="enable_use_xcode",
            action="store_true",
            default=False,
            help="Enable use_xcode in GN args. Default: disabled (false)",
        )
