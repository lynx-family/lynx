...
// Creates an HTTP GET request for `url`. A null URL is treated as an empty
// string. The caller owns the returned object until it is either released with
// `lynx_http_request_release` or passed to `lynx_http_service_request`, which
// consumes it.
...
// its data are borrowed and remain valid only for the duration of the callback.
// The callback must not release the response or invoke its callback again.
// `user_data` is passed through from `lynx_http_response_create`.
typedef void (*lynx_http_response_callback_func)(lynx_http_response_t* response,
                                                 void* user_data);
// Creates an HTTP response object with a completion callback. The caller owns
// the returned object until it is either released with
// `lynx_http_response_release` or passed to `lynx_http_service_request`, which
// consumes it.
...
// Retrieves the status code of an HTTP response. Returns -1 if `response` is
// null.
LYNX_CAPI_EXPORT int lynx_http_response_get_status_code(lynx_http_response_t*);
// Retrieves the body of an HTTP response and returns its length. The returned
// data is owned by `response` and remains valid only while `response` is alive.
// Returns 0 and stores null in `data`, when provided, if `response` is null.
...
// `request` and `response` on every path, including when the service is null or
// has no bound handler. A bound handler takes ownership and must eventually
// release each object exactly once. Completion and release must be serialized
// by the handler. The response callback may run synchronously.
