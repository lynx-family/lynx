...
void LynxResourceLoaderEmbedder::LoadResource(
  ...
  lynx_resource_request_t* fetcher_request = lynx_resource_request_create(
      request.url, static_cast<lynx_resource_type_e>(request.type));
  lynx_resource_response_t* fetcher_response =
      lynx_resource_response_create(fml::MakeCopyable(
