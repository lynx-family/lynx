...
class LynxOverlayView(context: LynxContext, val proxy: LynxUIOverlay) : UIGroup<AndroidView>(context) {
    ...
    private var mContainerPopupTagConflictWarned = false
    ...
    override fun afterPropsUpdated(props: StylesDiffMap?) {
        ...
        applyOverlayScope()
        warnIgnoredContainerPopupTag()
        reconcilePresentation()
    }

    private fun warnIgnoredContainerPopupTag() {
        val hasConflict =
            mAppliedOverlayScope == OverlayScope.FRAGMENT &&
                !mDialog.containerPopupTag.isNullOrEmpty()
        if (hasConflict && !mContainerPopupTagConflictWarned) {
            LLog.w(
                TAG,
                "android-container-popup-tag is ignored when android-overlay-scope is fragment"
            )
        }
        mContainerPopupTagConflictWarned = hasConflict
    }
    ...
    private fun applyOverlayScope() {
        ...
        if (mAppliedOverlayScope == mOverlayScope) {
            if (mOverlayScope == OverlayScope.FRAGMENT) {
    ...
    private fun unregisterPresentationObservers() {
        ...
        observer?.let {
            ...
            mScrollChangedListener?.let { listener ->
                it.removeOnScrollChangedListener(listener)
            }
            mDrawListener?.let { listener ->
                it.removeOnDrawListener(listener)
            }
        }
        mObserver = null
    ...
    private fun activatePresentationResources() {
        LynxOverlayManager.setGlobalIdActive(mId, true)
    ...
    private fun deactivatePresentationResources() {
        mDialog.cancelPassThroughGesture()
        LynxOverlayManager.setGlobalIdActive(mId, false)
