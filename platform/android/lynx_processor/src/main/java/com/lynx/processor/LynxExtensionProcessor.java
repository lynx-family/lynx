...
public class LynxExtensionProcessor extends AbstractProcessor {
  ...
  public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
    ...
    String providerPackageName = getProviderPackageName(roundEnv);
  ...
  private String getProviderPackageName(RoundEnvironment roundEnv) {
    Map<String, String> options = processingEnv.getOptions();
    String packageName = options.get(OPTION_EXTENSION_PACKAGE_NAME);
    if (packageName != null && packageName.trim().length() > 0) {
      return packageName.trim();
    }
    String behaviorPackageName = getBehaviorGeneratorPackageName(roundEnv);
    if (behaviorPackageName.length() > 0) {
      return behaviorPackageName;
    }
