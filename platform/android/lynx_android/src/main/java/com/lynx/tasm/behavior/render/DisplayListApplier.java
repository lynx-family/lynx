...
public class DisplayListApplier implements Drawable.Callback {
  ...
  private static final int MIN_ITEM_SIZE = 56;
  ...
  private void processContentOperations(Canvas canvas) {
    ...
    if (mItemsBuffer.capacity() < ITEM_SIZE_HEADER_BYTES) {
      return;
    }
    int itemSize = getIntAt(0);
    if (itemSize < MIN_ITEM_SIZE) {
      return;
    }
    int itemsCount = (mItemsBuffer.capacity() - ITEM_SIZE_HEADER_BYTES) / itemSize;

    int currentItemIndex = mItemIndex;

    while (currentItemIndex < itemsCount) {
      int itemByteOffset = ITEM_SIZE_HEADER_BYTES + currentItemIndex * itemSize;
      int op = getIntAt(itemByteOffset); // type field at offset 0

      switch (op) {
        case OP_BEGIN: {
          float x = mFragmentDepth == 0 ? .0f : getFloatAt(itemByteOffset + BEGIN_X_OFFSET);
          float y = mFragmentDepth == 0 ? .0f : getFloatAt(itemByteOffset + BEGIN_Y_OFFSET);
          canvas.save();
