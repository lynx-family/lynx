...
public class MeaningfulContentSnapshot {
  ...
  public void takeScreenshot(ILynxUIRenderer uiRenderer) {
    ...
    if (uiRenderer == null || !LynxEnv.inst().enableFspScreenshot()) {
      return;
    }
    ...
    LynxEventReporter.runOnReportThread(() -> {
      ...
      if (lynxViewBitmap != null) {
        String base64String = BitmapUtils.bitmapToBase64v2(lynxViewBitmap, 30);
        props.put("data", base64String);
      }
      UIThreadUtils.runOnUiThread(() -> {
        TraceEvent.instant(TraceEventDef.CATEGORY_LYNX, TraceEventDef.FSP_SNAPSHOT_BASE_64_BITMAP,
            traceCurrentTimestampUs, props);
      });
    });
  }
