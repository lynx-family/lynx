...
public class Arguments {
   ...
   */
  public static JavaOnlyArray makeNativeArray(List objects) {
    JavaOnlyArray nativeArray = new JavaOnlyArray();
    if (objects == null) {
      return nativeArray;
    }
    for (Object elem : objects) {
      ...
      } else if (elem instanceof JavaOnlyArray) {
        nativeArray.pushArray((JavaOnlyArray) elem);
      } else if (elem instanceof JavaOnlyMap) {
        nativeArray.pushMap((JavaOnlyMap) elem);
      } else {
   ...
   */
  public static <T> JavaOnlyArray makeNativeArray(final Object objects) {
    if (objects == null) {
  ...
  private static void addEntry(JavaOnlyMap nativeMap, String key, Object value) {
    ...
    } else if (value instanceof JavaOnlyArray) {
      nativeMap.putArray(key, (JavaOnlyArray) value);
    } else if (value instanceof JavaOnlyMap) {
      nativeMap.putMap(key, (JavaOnlyMap) value);
    } else {
   ...
   */
  public static JavaOnlyMap makeNativeMap(Map<String, Object> objects) {
    JavaOnlyMap nativeMap = new JavaOnlyMap();
    if (objects == null) {
      return nativeMap;
    }
  ...
  public static JavaOnlyArray fromJavaArgs(Object[] args) {
    JavaOnlyArray arguments = new JavaOnlyArray();
    for (int i = 0; i < args.length; i++) {
      ...
      } else if (argumentClass == JavaOnlyMap.class) {
        arguments.pushMap((JavaOnlyMap) argument);
      } else if (argumentClass == JavaOnlyArray.class) {
        arguments.pushArray((JavaOnlyArray) argument);
      } else {
   ...
   *     {@code Bundle},
   *              {@code List}, {@code Number}, {@code Boolean}, and all array types supported in
   * {@link #fromArray(Object)}.
   * @return the converted {@link WritableArray}
