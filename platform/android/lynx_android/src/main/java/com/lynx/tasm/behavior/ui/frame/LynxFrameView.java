...
public final class LynxFrameView extends UIBodyView {
  ...
  void setInitData(long dataPtr) {
    ...
    mInitData = updateTemplateData(mInitData, dataPtr);
  }
  ...
  void setGlobalProps(long dataPtr) {
    ...
    mGlobalProps = updateTemplateData(mGlobalProps, dataPtr);
  }

  private static TemplateData updateTemplateData(TemplateData current, long dataPtr) {
    if (current != null) {
      if (current.getNativePtr() == dataPtr) {
        return current;
      }
      current.recycle();
    }
