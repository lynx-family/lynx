...
public class UIListContainer extends UISimpleView<ListContainerView>
  ...
  private JavaOnlyArray mStickyStartIndexes = new JavaOnlyArray();
  private JavaOnlyArray mStickyEndIndexes = new JavaOnlyArray();
  private UIComponent mPrevStickyStartItem = null;
  private UIComponent mPrevStickyEndItem = null;
  ...
  public void insertChild(LynxBaseUI child, int index) {
    ...
    if (mEnableListSticky && !mUpdateStickyForDiff) {
      ...
      updateStickyInfoForInsertedChild(
          child, mStickyTopItems, mStickyStartIndexes, indexFromItemKey);
      updateStickyInfoForInsertedChild(
          child, mStickyBottomItems, mStickyEndIndexes, indexFromItemKey);
    }
  }
  ...
  public void setDiffInfo(ReadableMap map) {
    if (map instanceof JavaOnlyMap) {
      ...
      if (tempStickyTopIndexes instanceof JavaOnlyArray) {
        mStickyStartIndexes = ((JavaOnlyArray) tempStickyTopIndexes);
      }
      ...
      if (tempStickyBottomIndexes instanceof JavaOnlyArray) {
        mStickyEndIndexes = ((JavaOnlyArray) tempStickyBottomIndexes);
      }
  ...
  public void updateStickyStarts() {
    ...
    ListIterator<Object> listIterator =
        mStickyStartIndexes.listIterator(mStickyStartIndexes.size());
    ...
    if (stickyStartItem != null) {
      if (mPrevStickyStartItem != stickyStartItem) {
        LynxDetailEvent event = null;
        if (mIsVertical) {
          event = new LynxDetailEvent(getSign(), "stickytop");
          event.addDetail("top", stickyStartItem.getItemKey());
        } else {
          event = new LynxDetailEvent(getSign(), "stickyleft");
          event.addDetail("left", stickyStartItem.getItemKey());
        }
        mContext.getEventEmitter().sendCustomEvent(event);
        mPrevStickyStartItem = stickyStartItem;
      }
  ...
  public void updateStickyEnds() {
    ...
    for (Object EndIndex : mStickyEndIndexes) {
    ...
    if (stickyEndItem != null) {
      if (mPrevStickyEndItem != stickyEndItem) {
        LynxDetailEvent event = null;
        if (mIsVertical) {
          event = new LynxDetailEvent(getSign(), "stickybottom");
          event.addDetail("bottom", stickyEndItem.getItemKey());
        } else {
          event = new LynxDetailEvent(getSign(), "stickyright");
          event.addDetail("right", stickyEndItem.getItemKey());
        }

        mContext.getEventEmitter().sendCustomEvent(event);
        mPrevStickyEndItem = stickyEndItem;
      }
  ...
  public void onPropsUpdated() {
    ...
    if (mEnableListSticky && mUpdateStickyForDiff) {
      ...
      generateStickyItemKeySet(mStickyTopItemKeySet, mStickyStartIndexes, mStickyTopItemMap);
      generateStickyItemKeySet(mStickyBottomItemKeySet, mStickyEndIndexes, mStickyBottomItemMap);
    }
  }
