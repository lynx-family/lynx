...
class LynxExtensionScanner {
    static List<LynxExtensionInfo> scan(File startDir) {
        Set<File> nodeModulesDirs = findNodeModulesDirs(startDir)
        List<LynxExtensionInfo> result = []
        nodeModulesDirs.each { File nodeModules ->
            findManifestFiles(nodeModules).each { File manifest ->
                LynxExtensionInfo info = parseManifest(manifest)
                if (info != null) {
                    result << info
                }
            }
    ...
    private static void addManifest(List<File> manifests, File packageDir) {
        ...
        File manifest = new File(packageDir, 'lynx.ext.json')
        if (manifest.isFile()) {
            manifests << manifest
        }
    }

    private static LynxExtensionInfo parseManifest(File manifest) {
        Object json
        try {
        ...
        new LynxExtensionInfo(npmName, packageDir, manifest, packageName.trim(), sourceDir,
