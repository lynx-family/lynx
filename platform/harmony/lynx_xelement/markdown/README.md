...
Lynx XElement Markdown provides `<x-markdown>` on Harmony by registering a
native C++ XElement. Registration happens from the `liblynx.so` native XElement
...
`@lynx/serval_markdown`. Harmony arm64 and x86_64 builds register
`<x-markdown>` inside `liblynx.so` and link to the package-provided
