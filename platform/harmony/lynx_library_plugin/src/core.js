...
function generateRegistryHar(outputDir, libraries, lynxDependency) {
  ...
  const dependencies = {};
  if (sortedLibraries.some(hasHarmonyProvider)) {
    dependencies['@lynx/lynx'] = lynxDependency;
  }
...
function generateRegistrySource(libraries) {
  ...
  const imports = [];
  const entries = [];
  const addonInitializers = [];
  let providerIndex = 0;
  let addonIndex = 0;

  for (const library of sortedLibraries) {
    const providerExportName =
      library.providerExportName === undefined
        ? 'LynxLibraryProviderImpl'
        : library.providerExportName;
    if (providerExportName !== null) {
      const providerAlias = `Provider${providerIndex}`;
      providerIndex += 1;
      imports.push(
        `import { ${providerExportName} as ${providerAlias} } from '${library.ohPackageName}';`
      );
      entries.push(
        `  { packageName: ${JSON.stringify(library.npmPackageName)}, provider: new ${providerAlias}() }`
      );
    }
    for (const addon of library.nodeApiAddons ?? []) {
      const initializerAlias = `InitializeNodeApiAddon${addonIndex}`;
      addonIndex += 1;
      imports.push(
        `import { ${addon.initializerExportName} as ${initializerAlias} } from '${library.ohPackageName}';`
      );
      addonInitializers.push(
        generateNodeApiAddonInitializer(
          library,
          addon,
          initializerAlias
        )
...
`
  ...
  const providerSetup =
    entries.length > 0
      ...
      : '';
  return `// Generated by @lynx-js/lynx-library-plugin. Do not edit.
${providerDeclarations}\
${imports.length > 0 ? `${imports.join('\n')}\n` : ''}
export function setupGlobal(): void {
${addonInitializers.join('\n')}
${providerSetup}\
}
`;
}

function generateNodeApiAddonInitializer(library, addon, initializerAlias) {
  if (addon.required) {
    return `  ${initializerAlias}();`;
  }
  return `  try {
    ${initializerAlias}();
  } catch (error) {
    console.warn(${JSON.stringify(
      `Failed to initialize optional Lynx Node-API addon ${addon.name} from ${library.npmPackageName}: `
    )} + String(error));
  }`;
}
...
function hasHarmonyProvider(library) {
  return library.providerExportName !== null;
}
...
function readHarmonyLibrary(packageRoot, harmony, readJson5File) {
  ...
  const manifestPath = path.join(packageRoot, 'lynx.lib.json');
  const providerExportName = readProviderExportName(harmony, manifestPath);
  const nodeApiAddons = readNodeApiAddons(harmony, manifestPath);
...
function readProviderExportName(harmony, manifestPath) {
  if (!Object.prototype.hasOwnProperty.call(harmony, 'providerExportName')) {
    return 'LynxLibraryProviderImpl';
  }
  if (harmony.providerExportName === null) {
    return null;
  }
  return requireArkTsIdentifier(
    harmony.providerExportName,
    `${manifestPath}: platforms.harmony.providerExportName`
  );
}

function readNodeApiAddons(harmony, manifestPath) {
  const addons = harmony.nodeApiAddons;
  if (addons === undefined) {
    return [];
  }
  if (!Array.isArray(addons)) {
    throw new Error(
      `${manifestPath}: platforms.harmony.nodeApiAddons must be an array`
    );
  }
  return addons.map((addon, index) => {
    const field = `${manifestPath}: platforms.harmony.nodeApiAddons[${index}]`;
    if (!isObject(addon)) {
      throw new Error(`${field} must be an object`);
    }
    const name = requireAddonName(addon.name, `${field}.name`);
    const libraryName =
      addon.libraryName === undefined
        ? name
        : requireAddonName(addon.libraryName, `${field}.libraryName`);
    const initializerExportName = requireArkTsIdentifier(
      addon.initializerExportName,
      `${field}.initializerExportName`
    );
...
function generateHapStartup(modulePath, sourceRoot, existingStartup) {
  ...
  fs.writeFileSync(path.join(etsDir, '.gitignore'), '*\n');
  fs.mkdirSync(profileDir, { recursive: true });

  fs.writeFileSync(
    path.join(etsDir, `${STARTUP_TASK_NAME}.ets`),
    `// Generated by @lynx-js/lynx-library-plugin. Do not edit.
...
function requireArkTsIdentifier(value, label) {
  const identifier = requireNonEmptyString(value, label).trim();
  if (!/^[A-Za-z_$][A-Za-z0-9_$]*$/.test(identifier)) {
    throw new Error(`${label} must be a valid ArkTS identifier`);
  }
  return identifier;
}

function requireAddonName(value, label) {
  const name = requireNonEmptyString(value, label).trim();
  if (
    name.length > 128 ||
    name.includes('..') ||
    ...
    throw new Error(
      `${label} must match [A-Za-z0-9_.-] without '..' and be at most 128 characters`
    );
  }
  return name;
}
