...
void KeyframeAnimator::Run(const std::string& name) {
  ...
  ui_->SendAnimationEvent("animationstart", name);
}
...
static void CalStartEnd(float time, AnimationProperty type,
                        KeyframeParsedData* keyframeParsedData) {
  if (0 == time) {
    keyframeParsedData->has_start_keyframe_.insert(type);
  }
  if (1 == time) {
    keyframeParsedData->has_end_keyframe_.insert(type);
  }
}
