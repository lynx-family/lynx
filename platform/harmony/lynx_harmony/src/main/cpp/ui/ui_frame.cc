...
void UIFrame::SetFrameAppBundle(
    const std::shared_ptr<tasm::LynxTemplateBundle>& bundle) {
  if (!bundle) {
    return;
  }
...
void UIFrame::HandleIntrinsicSizeChanged(float width, float height) {
  ...
  if (std::abs(intrinsic_width_ - width) < 0.5f &&
      std::abs(intrinsic_height_ - height) < 0.5f) {
    return;
  }
  ...
  if (context_) {
    context_->FindShadowNodeAndRunTask(
        Sign(), [width, height](ShadowNode* shadow_node) {
          if (shadow_node && std::string(shadow_node->Tag()) == "frame") {
...
napi_value UIFrame::CreateNativeFrameValue() {
  ...
  if (napi_create_object(env_, &native_frame) != napi_ok) {
    return nullptr;
  }
...
void UIFrame::TryLoadBundle() {
  ...
  if (napi_create_string_utf8(env_, src_.c_str(), src_.length(), &src) !=
      napi_ok) {
    return;
  }
  napi_value bundle =
...
bool UIFrame::CallHostMethod(const char* method, size_t argc, napi_value* argv,
  ...
  napi_value func;
  if (napi_get_named_property(env_, host, method, &func) != napi_ok) {
    return false;
  }
  napi_valuetype type;
  if (napi_typeof(env_, func, &type) != napi_ok || type != napi_function) {
    return false;
  }
  napi_value local_result;
  return napi_call_function(env_, host, func, argc, argv,
                            result ? result : &local_result) == napi_ok;
}
