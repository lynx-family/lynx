...
static constexpr const int32_t kFrameNumberDone = -1;
...
void ImageDrawable::Render(OH_Drawing_Canvas* canvas) {
  ...
  if (!pixel_maps_->isAnimImage()) {
    ...
    auto frame_number_to_draw =
        GetFrameNumberToRender(animation_time_ms, last_frame_anim_time_ms_);

    if (frame_number_to_draw == kFrameNumberDone) {
      frame_number_to_draw = pixel_maps_->FrameCount() - 1;
      DispatchAnimationStop();
      is_running_ = false;
    } else if (frame_number_to_draw == 0) {
      if (last_drawn_frame_number_ != kFrameNumberDone &&
          last_drawn_frame_number_ != 0 &&
          actual_render_time_start_ms >= expected_render_time_ms_) {
        DispatchAnimationRepeat();
      }
    }
    const auto draw_bitmap =
        pixel_maps_->FramePixelMap(frame_number_to_draw)->DrawBitmap();
    DrawPixelMap(canvas, draw_bitmap);
    last_drawn_frame_number_ = frame_number_to_draw;
    ...
    if (is_running_) {
      ...
      if (target_render_time_for_next_frame_ms != 0) {
        scheduled_render_time_for_next_frame_ms =
            target_render_time_for_next_frame_ms + frame_scheduling_delay_ms_;
...
void ImageDrawable::UpdateLoopCount(int loop_count) {
...
void ImageDrawable::StartAnimation() {
  ...
  last_drawn_frame_number_ = kFrameNumberDone;
  if (is_paused_) {
    ...
    last_drawn_frame_number_ = paused_last_drawn_frame_number_;
    is_paused_ = false;
  }
  InvalidateSelf();
...
void ImageDrawable::PauseAnimation() {
  ...
  paused_last_drawn_frame_number_ = last_drawn_frame_number_;
  is_paused_ = true;
  UnScheduleSelf();
}
...
int32_t ImageDrawable::GetFrameNumberToRender(uint64_t animation_time_ms,
                                              uint64_t last_frame_time_ms) {
  if (!IsInfiniteAnimation()) {
    const auto loop_count = animation_time_ms / GetLoopDuration();
    if (loop_count >= loop_count_) {
      return kFrameNumberDone;
    }
  }
  const auto time_in_current_loop_ms = animation_time_ms % GetLoopDuration();
  return GetFrameNumberWithinLoop(time_in_current_loop_ms);
}

int64_t ImageDrawable::GetLoopDuration() { return pixel_maps_->LoopDuration(); }
...
uint64_t ImageDrawable::GetTargetRenderTimeForNextFrameMS(
  ...
  auto loop_duration = GetLoopDuration();

  if (loop_duration == 0) {
    return 0;
  }
  if (!IsInfiniteAnimation()) {
    auto loop_count = animation_time_ms / GetLoopDuration();
    if (loop_count >= loop_count_) {
      return 0;
    }
  }
  auto time_passed_in_current_loop_ms = animation_time_ms & loop_duration;
  auto time_of_next_frame_in_loop_ms = 0;
  int frame_count = pixel_maps_->FrameCount();
  for (int i = 0; i < frame_count && time_of_next_frame_in_loop_ms <=
