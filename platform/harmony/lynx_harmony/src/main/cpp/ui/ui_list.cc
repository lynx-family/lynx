...
void UIList::UpdateStickyStartView(float scroll_offset_x,
  ...
  if (sticky_start_item) {
    if (prev_sticky_start_item_ != sticky_start_item) {
      auto param = lepus::Dictionary::Create();
      if (!is_horizontal_) {
        param->SetValue("top", sticky_start_item->item_key());
        CustomEvent event{Sign(), "stickytop", "detail", lepus_value(param)};
        context_->SendEvent(event);

      } else {
        param->SetValue("left", sticky_start_item->item_key());
        CustomEvent event{Sign(), "stickyleft", "detail", lepus_value(param)};
        context_->SendEvent(event);
      }

      prev_sticky_start_item_ = sticky_start_item;
    }
...
void UIList::UpdateStickyEndView(float scroll_offset_x, float scroll_offset_y) {
  ...
  if (sticky_end_item) {
    if (prev_sticky_end_item_ != sticky_end_item) {
      auto param = lepus::Dictionary::Create();
      if (!is_horizontal_) {
        param->SetValue("bottom", sticky_end_item->item_key());
        CustomEvent event{Sign(), "stickybottom", "detail", lepus_value(param)};
        context_->SendEvent(event);
      } else {
        param->SetValue("right", sticky_end_item->item_key());
        CustomEvent event{Sign(), "stickyright", "detail", lepus_value(param)};
        context_->SendEvent(event);
      }

      prev_sticky_end_item_ = sticky_end_item;
    }
