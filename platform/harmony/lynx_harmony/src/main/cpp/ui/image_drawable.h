...
class ImageDrawable {
 public:
  ...
  void UpdateLoopCount(int loop_count);
 ...
 private:
  ...
  //  bool is_change_loop_count_{false};
  ...
  uint64_t frame_scheduling_delay_ms_{8};
  uint64_t frame_scheduling_offset_ms_{0};

  int32_t paused_last_drawn_frame_number_{-1};
  int32_t last_drawn_frame_number_{-1};
  //  int32_t dropped_frames_{0};

  void ScheduleNextFrame(uint64_t target_animation_time_ms);

  int32_t GetFrameNumberToRender(uint64_t animation_time_ms,
