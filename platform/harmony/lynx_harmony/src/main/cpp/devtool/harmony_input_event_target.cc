...
constexpr int32_t kLegacyPointerId = 0;
...
HarmonyInputEventTarget::HarmonyInputEventTarget(
    ...
    : lynx_context_(lynx_context), state_(std::make_shared<State>()) {}

HarmonyInputEventTarget::~HarmonyInputEventTarget() { Shutdown(); }
...
input::PointerCapabilities HarmonyInputEventTarget::GetPointerCapabilities()
  ...
  capabilities.supports_touch =
      state_ && state_->alive.load(std::memory_order_acquire) && lynx_context &&
      lynx_context->HasWindowInfo() && IsAvailable();
...
bool HarmonyInputEventTarget::InjectPointerEvent(
  ...
  auto lynx_context = lynx_context_.lock();
  if (!state_ || !state_->alive.load(std::memory_order_acquire) ||
      !lynx_context || !lynx_context->GetUITaskRunner() ||
  ...
  lynx_context->RunOnUIThread([state = state_, lynx_context, event]() {
    if (!state || !state->alive.load(std::memory_order_acquire)) {
      return;
    }
    state->pending_injection_succeeded &=
        InjectPointerEventOnUIThread(lynx_context.get(), event, state.get());
  });
  return true;
}
...
void HarmonyInputEventTarget::WaitForInputProcessed(
  ...
  auto* caller_loop = fml::MessageLoop::IsInitializedForCurrentThread();
  if (!caller_loop) {
    callback(false);
    return;
  }
  auto caller_task_runner = caller_loop->GetTaskRunner();
  auto lynx_context = lynx_context_.lock();
  if (!lynx_context || !lynx_context->GetUITaskRunner()) {
    callback(false);
    return;
  }

  lynx_context->GetUITaskRunner()->PostTask(
      [state = state_, caller_task_runner,
       callback = std::move(callback)]() mutable {
        bool succeeded = false;
        if (state && state->alive.load(std::memory_order_acquire)) {
          succeeded = state->pending_injection_succeeded;
          state->pending_injection_succeeded = true;
        }
        fml::TaskRunner::RunNowOrPostTask(
            caller_task_runner, [callback = std::move(callback),
                                 succeeded]() mutable { callback(succeeded); });
      });
}

void HarmonyInputEventTarget::EmulateTouchFromMouseEvent(
  ...
  auto lynx_context = lynx_context_.lock();
  if (!state_ || !state_->alive.load(std::memory_order_acquire) ||
      !lynx_context || !lynx_context->GetUITaskRunner() || !IsAvailable()) {
    return;
  }
  lynx_context->RunOnUIThread([state = state_, lynx_context, event_type, x,
                               y]() {
    if (!state || !state->alive.load(std::memory_order_acquire)) {
      return;
    }
    EmulateTouchOnUIThread(lynx_context.get(), event_type, x, y, state.get());
  });
}

void HarmonyInputEventTarget::Shutdown() {
  auto state = state_;
  ...
  lynx_context->RunOnUIThread([state, lynx_context]() {
    CancelActiveTouch(lynx_context.get(), NowUs(), state.get());
  });
}

bool HarmonyInputEventTarget::IsAvailable() {
  return GetOhInputTouchEventApi() && GetWindowInjectTouchEventFunc();
}

bool HarmonyInputEventTarget::InjectPointerEventOnUIThread(
  ...
  const float x = pointer->x / scaled_density;
  const float y = pointer->y / scaled_density;
  ...
  switch (event.type) {
    case input::PointerEventType::kDown: {
      ...
      const bool injected =
          InjectTouch(context, TOUCH_ACTION_DOWN, kPlatformPrimaryPointerId, x,
                      y, event_time_us);
      if (injected) {
        state->touch_active = true;
        state->active_pointer_id = pointer->id;
        state->active_touch_x = x;
        state->active_touch_y = y;
      }
      return injected;
    }
    case input::PointerEventType::kMove:
      ...
      if (InjectTouch(context, TOUCH_ACTION_MOVE, kPlatformPrimaryPointerId, x,
                      y, event_time_us)) {
        state->active_touch_x = x;
        state->active_touch_y = y;
        return true;
    ...
    case input::PointerEventType::kUp:
      ...
      if (InjectTouch(context, TOUCH_ACTION_UP, kPlatformPrimaryPointerId, x, y,
                      event_time_us)) {
...
void HarmonyInputEventTarget::EmulateTouchOnUIThread(
  ...
  if (event_type == LynxEventSimulationProxy::kMousePressed) {
    if (state->touch_active) {
      CancelActiveTouch(context, event.timestamp_us, state);
    }
    event.type = input::PointerEventType::kDown;
    pointer.id = kLegacyPointerId;
    event.buttons = input::kPrimaryButton;
    event.click_count = 1;
  } else if (event_type == LynxEventSimulationProxy::kMouseMoved) {
    if (!state->touch_active) {
      return;
    }
    event.type = input::PointerEventType::kMove;
    pointer.id = state->active_pointer_id;
    event.buttons = input::kPrimaryButton;
  } else if (event_type == LynxEventSimulationProxy::kMouseReleased) {
    if (!state->touch_active) {
      return;
    }
    event.type = input::PointerEventType::kUp;
    pointer.id = state->active_pointer_id;
  } else {
    return;
  }

  pointer.x = static_cast<float>(x);
  pointer.y = static_cast<float>(y);
  event.changed_pointer_id = pointer.id;
  event.pointers.push_back(pointer);
  InjectPointerEventOnUIThread(context, event, state);
}

bool HarmonyInputEventTarget::InjectTouch(LynxContext* context, int32_t action,
...
bool HarmonyInputEventTarget::ToEventPoint(LynxContext* context, float x,
...
bool HarmonyInputEventTarget::CancelActiveTouch(LynxContext* context,
                                                int64_t event_time_us,
                                                State* state) {
...
void HarmonyInputEventTarget::ResetActiveTouch(State* state) {
  if (!state) {
    return;
  }
