...
napi_value LynxTemplateRenderer::NativeReset(napi_env env,
  ...
  if (status != napi_ok) {
    LOGE(
        "LynxTemplateRenderer NativeReset failed to get callback info, "
        "napi_status="
  ...
  if (argc < 17) {
    LOGE(
        "LynxTemplateRenderer NativeReset invalid args: require at least 17, "
        "got "
        << argc << ", napi_status=" << static_cast<int32_t>(status));
    return nullptr;
  }
  ...
  if (argc > 17) {
    ...
    if (status != napi_ok) {
      napi_throw_error(
          env, nullptr,
          "LynxTemplateRenderer NativeReset napi_typeof runtime wrapper "
          "failed");
      return nullptr;
    }
