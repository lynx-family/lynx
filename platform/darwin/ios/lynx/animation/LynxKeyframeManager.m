...
- (void)applyAnimationInfo:(LynxAnimationInfo*)info
            ...
            reuseKeyframes:(BOOL)reuseKeyframes
               animationID:(uint64_t)animationID
                generation:(uint32_t)generation
                    cancel:(BOOL)cancel
                    detach:(BOOL)detach
                  playback:(LynxPlatformAnimationPlayback*)playback {
  ...
  if (cancel || detach) {
    if (detach) {
      [animator detachPlatformAnimation];
    } else {
      [animator destroy];
    }
  ...
  animator.platformPlayback = playback;
...
- (BOOL)hasAnimationRunning {
  for (LynxKeyframeAnimator* animator in _animators.objectEnumerator) {
    if ([animator isRunning]) {
      return YES;
    }
  }
  for (LynxKeyframeAnimator* animator in _platformAnimators.objectEnumerator) {
    if ([animator isRunning]) {
      return YES;
    }
  }
