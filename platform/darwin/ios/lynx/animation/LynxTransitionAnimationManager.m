...
- (BOOL)preparePlatformTransition:(LynxAnimationInfo*)info
                      ...
                      animationID:(uint64_t)animationID
                       generation:(uint32_t)generation
                           cancel:(BOOL)cancel
              hadActiveTransition:(BOOL*)hadActiveTransition {
  if (_platformTransitionGenerations == nil) {
...
- (LynxAnimationDelegate*)createTransitionDelegate:(LynxAnimationInfo*)info {
  __weak LynxTransitionAnimationManager* weakSelf = self;
  NSNumber* propertyKey = @(info.prop);
  NSNumber* animationKey = _platformAnimationIDsByProperty[propertyKey];
  const uint32_t generation =
      animationKey != nil ? _platformTransitionGenerations[animationKey].unsignedIntValue : 0;
  return [LynxAnimationDelegate
      initWithDidStart:^(CAAnimation* animation) {
        __strong LynxTransitionAnimationManager* strongSelf = weakSelf;
        if (!strongSelf ||
            (animationKey != nil && ![strongSelf isCurrentPlatformTransition:animationKey
                                                                 propertyKey:propertyKey
                                                                  generation:generation])) {
          return;
        }
