...
- (void)testPlatformRendererDarwinKeepsCenterAnchorForUntransformedPage {
  ...
  begin.type = lynx::tasm::DisplayListOpType::kBegin;
  begin.payload.begin.id = 13;
  begin.payload.begin.type = static_cast<int32_t>(PlatformRendererType::kPage);
  begin.payload.begin.w = static_cast<float>(pageFrame.size.width);
  begin.payload.begin.h = static_cast<float>(pageFrame.size.height);
  list.AppendItem(begin);
  lynx::tasm::DisplayListItem end{};
  end.type = lynx::tasm::DisplayListOpType::kEnd;
  list.AppendItem(end);
...
- (void)testPlatformRendererDarwinPreservesPageOriginMovedBySetFrameAfterTransform {
  ...
  fullList.AppendItem(begin);
  fullList.AppendItem(end);
...
- (void)testPlatformRendererDarwinRestoresOverlayOriginForSubtreeOnlyTransformUpdate {
  ...
  begin.type = lynx::tasm::DisplayListOpType::kBegin;
  begin.payload.begin.id = 13;
  begin.payload.begin.type = static_cast<int32_t>(PlatformRendererType::kUnknown);
  initialList.AppendItem(begin);
  lynx::tasm::DisplayListItem end{};
  end.type = lynx::tasm::DisplayListOpType::kEnd;
  initialList.AppendItem(end);
