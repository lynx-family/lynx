...
- (void)drawBoxShadowWithShadowBox:(const RoundedRectangle &)shadowBox
                           clipBox:(const RoundedRectangle &)clipBox
                             color:(uint32_t)argb
                        blurRadius:(float)blurRadius
                          clipMode:(int32_t)clipMode;
  ...
  while (reader_.HasNext()) {
    ...
    switch (op) {
      ...
      case DisplayListOpType::kBoxShadow: {
        int32_t shadow_box_index = item.payload.box_shadow.shadow_box_index;
        int32_t clip_box_index = item.payload.box_shadow.clip_box_index;
        uint32_t color = item.payload.box_shadow.color;
        float blur_radius = item.payload.box_shadow.blur_radius;
        int32_t clip_mode = item.payload.box_shadow.clip_mode;

        if (shadow_box_index < 0 || static_cast<size_t>(shadow_box_index) >= box_array_.size() ||
            clip_box_index < 0 || static_cast<size_t>(clip_box_index) >= box_array_.size()) {
          break;
        }

        [self drawBoxShadowWithShadowBox:box_array_[shadow_box_index]
                                 clipBox:box_array_[clip_box_index]
                                   color:color
...
- (void)drawBoxShadowWithShadowBox:(const RoundedRectangle &)shadowBox
  ...
  shadowLayer.frame = CGRectMake(shadowBox.GetX() + left_offset_, shadowBox.GetY() + top_offset_,
                                 shadowBox.GetWidth(), shadowBox.GetHeight());
  shadowLayer.shadowColor = [self colorFromARGB:argb].CGColor;
  shadowLayer.shadowRadius = blurRadius;
  shadowLayer.shadowOpacity = 1.0f;
  if (clipMode == 1) {  // kInset
    shadowLayer.masksToBounds = YES;
  }
  [self insertLayer:shadowLayer forOp:DisplayListOpType::kBoxShadow];
}
