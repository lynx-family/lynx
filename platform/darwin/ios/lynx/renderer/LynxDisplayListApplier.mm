...
- (UIImage *)createBorderImageWithOutBox:(const RoundedRectangle &)outBox
  ...
  CGSize size = CGSizeMake(outBox.GetWidth(), outBox.GetHeight());

  // Calculate border insets: {top, left, bottom, right}
  UIEdgeInsets borderInsets = UIEdgeInsetsMake(
      innerBox.GetY() - outBox.GetY(),                                              // top
      innerBox.GetX() - outBox.GetX(),                                              // left
      outBox.GetY() + outBox.GetHeight() - innerBox.GetY() - innerBox.GetHeight(),  // bottom
      outBox.GetX() + outBox.GetWidth() - innerBox.GetX() - innerBox.GetWidth()     // right
  );
