...
- (void)assertExternalMaskReconcilesPartialPaintWithVisibleOverflow:(BOOL)visibleOverflow
                                                       cornerHeight:(CGFloat)cornerHeight {
  if (@available(iOS 11.0, *)) {
    for (NSNumber *clearNativeClipping in @[ @NO, @YES ]) {
      for (UIView *view in @[ [UIView new], [UIScrollView new] ]) {
        UIView *container = [UIView new];
        [container addSubview:view];
        LynxUIView *ui = [self roundedUIWithView:view];
        [LynxPropsProcessor updateProp:@[ @10, @0, @(cornerHeight), @0 ]
        ...
        [ui onNodeReadyForUIOwner];
        XCTAssertNil(ui.backgroundManager.backgroundLayer);
        XCTAssertTrue(CGColorEqualToColor(view.layer.backgroundColor, UIColor.redColor.CGColor));
        XCTAssertEqual(view.layer.borderWidth, visibleOverflow ? 0 : 1);
        XCTAssertEqual(view.layer.cornerRadius, cornerHeight == 0 ? 0 : 10);

        CAShapeLayer *mask = [CAShapeLayer layer];
        mask.frame = view.bounds;
        mask.path = [UIBezierPath bezierPathWithRect:CGRectInset(view.bounds, 8, 8)].CGPath;
        if (clearNativeClipping.boolValue) {
          // External owners may reset native clipping before installing their mask.
        ...
        XCTAssertEqual(view.layer.maskedCorners, 15u);
        XCTAssertEqual(view.layer.borderWidth, 0);
        XCTAssertTrue(view.layer.backgroundColor == nil ||
                      CGColorGetAlpha(view.layer.backgroundColor) == 0);
        XCTAssertNotNil(ui.backgroundManager.backgroundLayer);
        XCTAssertEqual(ui.backgroundManager.backgroundLayer.type, LynxBgTypeComplex);
        XCTAssertEqual(ui.backgroundManager.backgroundLayer.superlayer, container.layer);
        if (!visibleOverflow) {
          XCTAssertNotNil(ui.backgroundManager.borderLayer);
          XCTAssertNotEqual(ui.backgroundManager.borderLayer.type, LynxBgTypeSimple);
          XCTAssertEqual(ui.backgroundManager.borderLayer.superlayer, container.layer);
        }

        id manager = OCMPartialMock(ui.backgroundManager);
        OCMReject([manager applyEffect]);
        OCMReject([manager applyEffect:NO]);
        OCMReject([manager applyEffect:YES]);
...
- (void)testExternalMaskClearingBoundsClippingPreservesPartialPaint {
  [self assertExternalMaskReconcilesPartialPaintWithVisibleOverflow:NO cornerHeight:10];
}

- (void)testVisibleOverflowReconcilesPartialPaintForExternalMask {
  [self assertExternalMaskReconcilesPartialPaintWithVisibleOverflow:YES cornerHeight:10];
}

- (void)testZeroAxisCornersReconcilePaintForExternalMask {
  [self assertExternalMaskReconcilesPartialPaintWithVisibleOverflow:NO cornerHeight:0];
  [self assertExternalMaskReconcilesPartialPaintWithVisibleOverflow:YES cornerHeight:0];
}
