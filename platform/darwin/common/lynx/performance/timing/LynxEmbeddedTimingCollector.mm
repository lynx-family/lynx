...
- (void)emitSetupTimingIfReady {
  if (self.hasEmitSetupTiming || self.setupTimingTarget == nil) {
    return;
  }
  ...
  __weak id setupTimingTarget = self.setupTimingTarget;
  [LynxEventReporter runOnReportThread:^{
    id strongTarget = setupTimingTarget;
    if (strongTarget) {
      [strongTarget onTimingSetup:timingInfo];
    }
  }];
}
