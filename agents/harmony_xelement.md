...
Harmony `x-markdown` should follow the native XElement integration path:

- `x-markdown` is registered from the native XElement registry.
...
- This path does not build or load `liblynx_markdown.so`; logs should show
  `x-markdown: registered native XElement built into liblynx`.

Do not implement `x-markdown` as an ArkTS `UIBase` wrapper around
