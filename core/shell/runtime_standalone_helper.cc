...
InitRuntimeStandaloneResult InitRuntimeStandalone(
  ...
  auto js_bundle_proxy =
      std::make_unique<StandaloneBundleProxy>(lazy_bundle_loader);
  ...
  runtime_actor->ActAsync(
      [native_module_manager, preload_js_paths = std::move(preload_js_paths),
       ...
       weak_js_bundle_holder =
           std::weak_ptr<piper::JsBundleHolder>(js_bundle_holder)](
  ...
  return {runtime_actor,        performance_actor,  native_runtime_facade,
          white_board_delegate, lazy_bundle_loader, std::move(js_bundle_proxy),
          js_bundle_holder};
}
