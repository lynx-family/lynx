...
void LynxShell::RegisterNotificationCallbacks() {
  if (!memory_pressure_callback_) {
    memory_pressure_callback_ = std::make_unique<
        base::NotificationCallback>(base::NotificationCallback::CallbackList{
           ...
           if (std::strcmp(type, "bts") == 0) {
             this->runtime_actor_->Act([&](auto& rt) {
               ...
               if (bts_js_executor) {
                 ...
                 if (bts_rt) {
                   ...
                   if (runtime::RuntimeManager::IsSingleJSContext(group_id)) {
             ...
             this->engine_actor_->Act([&](auto& engine) {
               ...
               if (default_entry) {
                 ...
                 if (mts_vm) {
                   std::string identifier = "instance_" +
                                            std::to_string(this->instance_id_) +
                                            "(mts)";
                   intptr_t payload[2] = {
        ...
        {LYNX_ON_TRACE_BEGIN_NOTIFICATION,
         [this](const std::string& tag, [[maybe_unused]] intptr_t data) {
           ...
           this->engine_actor_->Act([&](auto& engine) {
             auto default_entry =
                 engine->GetTasm()->FindTemplateEntry(tasm::DEFAULT_ENTRY_NAME);
             if (default_entry) {
               auto mts_vm = default_entry->GetVm();
               if (mts_vm) {
                 TRACE_EVENT_INSTANT(
                     ...
                     [&](lynx::perfetto::EventContext ctx) {
                       ctx.event()->add_debug_annotations(
                           "instance_id", std::to_string(this->instance_id_));
                       ctx.event()->add_debug_annotations("url", this->url_);
                       ctx.event()->add_debug_annotations(
                           "desc",
               ...
               if (bts_rt) {
                 TRACE_EVENT_INSTANT(
                     ...
                     [&](lynx::perfetto::EventContext ctx) {
                       ...
                       ctx.event()->add_debug_annotations(
                           "desc",
                           bts_rt->getSharedVM()->GetDebugDescription());
                       ctx.event()->add_debug_annotations(
                           "ptr", std::to_string(reinterpret_cast<intptr_t>(
