...
int GetBTSConsoleLogLevel(const std::string& func_name) {
  if (func_name == ConsoleWarn) {
    return CONSOLE_LOG_WARNING;
  }
  if (func_name == ConsoleError || func_name == ConsoleAssert) {
    return CONSOLE_LOG_ERROR;
  }
  return CONSOLE_LOG_INFO;
}

void LogBTSConsoleEventFallback(const std::string& func_name,
                                const std::string& args) {
  std::string msg = "[bts-console][" + func_name + "] " + args;
  switch (GetBTSConsoleLogLevel(func_name)) {
...
void MTSRuntime::OnBTSConsoleEvent(const std::string& func_name,
  ...
  if (!delegate_->IsRuntimeEnabled()) {
    LogBTSConsoleEventFallback(func_name, args);
  }
