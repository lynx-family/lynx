...
void BTSRuntimeStandalone::EvaluateScript(
  ...
  if (!page_config) {
    LOGE("EvaluateScript with template bundle failed: page config is null.");
    return;
  }
  auto runtime_type = page_config->GetDSL();
  if (entry_file.empty() && runtime_type == tasm::PackageInstanceDSL::UNKOWN) {
    LOGE("EvaluateScript with template bundle failed: unknown dsl.");
    return;
  }
  uint64_t trace_flow_id = TRACE_FLOW_ID();
  TRACE_EVENT(LYNX_TRACE_CATEGORY_VITALS, EVALUATE_SCRIPT_STANDALONE,
