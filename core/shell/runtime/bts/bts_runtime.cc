...
void BTSRuntime::TransitionToFullRuntime() {
  ...
  // If another runtime with the same group_id has already loaded lynx_core.js
  // into the shared JS context, skip the re-evaluation and only update state.
  auto* wrapper =
      runtime::RuntimeManager::Instance()->GetContextWrapper(group_id_);
  if (wrapper && wrapper->isCoreJSLoaded()) {
    UpdateState(State::kJsCoreLoaded);
    return;
  }
