...
namespace lynx {
    namespace shell {
        void
        ListEngineProxyImpl::ScrollByListContainer(int32_t tag, float offset_x, float offset_y, float original_x,
                                                   float original_y) {
        ...
        void ListEngineProxyImpl::ScrollToPosition(int32_t tag, int index, float offset, int align,
                                                   bool smooth) {
            if (auto engine_actor = engine_actor_.lock()) {
                engine_actor->Act(
