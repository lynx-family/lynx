...
// A null lifecycle selects the optimized path. The LynxTemplateRender Java
// instance owns the native shell lifetime directly in this mode, so JNI calls
// intentionally skip the AtomicLifecycle synchronization.
bool TryLockLifecycle(AtomicLifecycle* lifecycle) {
  return lifecycle == nullptr || AtomicLifecycle::TryLock(lifecycle);
}

void FreeLifecycle(AtomicLifecycle* lifecycle) {
