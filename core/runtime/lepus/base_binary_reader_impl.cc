...
bool BaseBinaryReader::DecodeLepusNGContextBundleImpl(
    runtime::ContextBundle* bundle) {
...
#if !ENABLE_JUST_LEPUSNG
bool BaseBinaryReader::DecodeVMContextBundleImpl(
    runtime::ContextBundle* bundle) {
  auto vm_bundle = static_cast<VMContextBundle*>(bundle);
  auto parent = fml::Ref<Function>(nullptr);
  return DeserializeGlobal(vm_bundle->lepus_root_global()) &&
