...
TEST_F(LEPUSIRTestToplevelPasses, UpdateToplevelClosureVarBasic) {
  ...
  // 6. Verify child_lepus_func has the mapping
  EXPECT_EQ(child_lepus_func->GetClosureVarToplevelReg(0), physical_reg);
}
...
TEST_F(LEPUSIRTestToplevelPasses,
  ...
  EXPECT_EQ(child_lepus_func->GetClosureVarToplevelReg(0),
