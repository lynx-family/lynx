  ...
  for (auto relocation : relocations_) {
    ...
    switch (type) {
      case Relocation::Jmp: {
        ...
        if (LEPUS_UNLIKELY(
                !lynx::lepus::Instruction::IsJmpInstruction(*jmp_inst))) {
          throw ::lynx::lepus::CompileException(
      ...
      case Relocation::CmpJmp: {
        ...
        auto jmp_inst = GetInstruction(loc);
        if (LEPUS_UNLIKELY(
                !lynx::lepus::Instruction::IsCmpJmpInstruction(*jmp_inst))) {
          throw ::lynx::lepus::CompileException(
  ...
  if (is_consecutive_after_array) {
    ...
    for (auto i = 0; i < new_insts_size; i++) {
      ...
      if (op_count >= array_size) {
        a = b = c = 0;
        ADD_INSTRUCTION(inst, lynx::lepus::Instruction::ABCDCode(d, a, b, c));
        break;
      }
...
void InstructionSelectionPass::GenerateGetUpvalueInst(GetUpvalueInst* inst,
  ...
  auto toplevel_reg =
      current_func->GetLepusFunction()->GetClosureVarToplevelReg(index);
