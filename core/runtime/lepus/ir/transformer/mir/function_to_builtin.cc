  ...
  for (auto& bb : *func) {
    for (auto it = bb.InstBegin(); it != bb.InstEnd(); ++it) {
      ...
      if (auto* get_upvalue = llvh::dyn_cast<GetUpvalueInst>(callee)) {
        ...
        // in Function::upvalue_index_to_toplevel_reg_. This is the same mapping
        // used by instruction selection to emit GetToplevelClosureVar.
        auto current_lepus_func = get_upvalue->GetFunc()->GetLepusFunction();
        if (current_lepus_func == nullptr) {
          continue;
        }
        const long toplevel_reg =
            current_lepus_func->GetClosureVarToplevelReg(upvalue_index);
        if (toplevel_reg < 0) {
          continue;
        }
