...
namespace lepus {
namespace ir {
void UpdateToplevelClosureVar::ProcessChildFunction(
    fml::RefPtr<Function>& parent_func) {
  for (auto child : parent_func->GetChildFunction()) {
    for (auto i = 0; i < child->UpvaluesSize(); i++) {
      auto upvalue = child->GetUpvalue(i);

      auto new_reg = parent_func->GetClosureVarToplevelReg(upvalue->register_);
