...
#ifndef CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_ADT_STLEXTRAS_H_
#define CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_ADT_STLEXTRAS_H_
...
#include "core/runtime/lepus/ir/llvh/include/llvh/ADT/Optional.h"
...
auto reverse(
    ...
    -> decltype(make_range(llvh::make_reverse_iterator(std::end(C)),
                           llvh::make_reverse_iterator(std::begin(C)))) {
  return make_range(llvh::make_reverse_iterator(std::end(C)),
                    llvh::make_reverse_iterator(std::begin(C)));
}
...
class early_inc_iterator_impl
 ...
 protected:
#if LLVH_ENABLE_ABI_BREAKING_CHECKS
...
class zippy {
 ...
 private:
  ...
  template <size_t... Ns>
  iterator begin_impl(index_sequence<Ns...>) const {
    return iterator(std::begin(std::get<Ns>(ts))...);
  }
  template <size_t... Ns>
  iterator end_impl(index_sequence<Ns...>) const {
...
class concat_iterator
  ...
  void increment(index_sequence<Ns...>) {
    ...
    llvh_unreachable("Attempted to increment an end concat iterator!");
  }
  ...
  ValueT& get(index_sequence<Ns...>) const {
    ...
    llvh_unreachable("Attempted to get a pointer from an end concat iterator!");
  }
...
class concat_range {
 ...
 private:
  ...
  template <size_t... Ns>
  iterator begin_impl(index_sequence<Ns...>) {
    return iterator(std::get<Ns>(Ranges)...);
  }
  template <size_t... Ns>
  iterator end_impl(index_sequence<Ns...>) {
...
inline void sort(Container&& C) {
  llvh::sort(adl_begin(C), adl_end(C));
}
...
inline void sort(Container&& C, Compare Comp) {
  llvh::sort(adl_begin(C), adl_end(C), Comp);
}
...
#endif  // CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_ADT_STLEXTRAS_H_
