...
#ifndef CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_ADT_APINT_H_
#define CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_ADT_APINT_H_
...
#include "core/runtime/lepus/ir/llvh/include/llvh/Support/MathExtras.h"
...
class LLVM_NODISCARD APInt {
 ...
 public:
  ...
  bool isOneValue() const {
    if (isSingleWord()) return U.VAL == 1;
  ...
  bool isMaxSignedValue() const {
    if (isSingleWord()) return U.VAL == ((WordType(1) << (BitWidth - 1)) - 1);
  ...
  bool isMinSignedValue() const {
    if (isSingleWord()) return U.VAL == (WordType(1) << (BitWidth - 1));
  ...
  bool isPowerOf2() const {
    if (isSingleWord()) return isPowerOf2_64(U.VAL);
  ...
  bool isMask() const {
    if (isSingleWord()) return isMask_64(U.VAL);
    unsigned Ones = countTrailingOnesSlowCase();
  ...
  bool isShiftedMask() const {
    if (isSingleWord()) return isShiftedMask_64(U.VAL);
  ...
  static bool isSameValue(const APInt& I1, const APInt& I2) {
    if (I1.getBitWidth() == I2.getBitWidth()) return I1 == I2;
  ...
  bool operator!() const {
    if (isSingleWord()) return U.VAL == 0;
  ...
  bool operator==(const APInt& RHS) const {
    ...
    if (isSingleWord()) return U.VAL == RHS.U.VAL;
  ...
  bool intersects(const APInt& RHS) const {
    assert(BitWidth == RHS.BitWidth && "Bit widths must be the same");
    if (isSingleWord()) return (U.VAL & RHS.U.VAL) != 0;
  ...
  unsigned getMinSignedBits() const {
    if (isNegative()) return BitWidth - countLeadingOnes() + 1;
  ...
  int64_t getSExtValue() const {
    if (isSingleWord()) return SignExtend64(U.VAL, BitWidth);
  ...
  unsigned countLeadingZeros() const {
    if (isSingleWord()) {
      ...
      return llvh::countLeadingZeros(U.VAL) - unusedBits;
    }
  ...
  unsigned countLeadingOnes() const {
    if (isSingleWord())
      return llvh::countLeadingOnes(U.VAL << (APINT_BITS_PER_WORD - BitWidth));
  ...
  unsigned countTrailingZeros() const {
    if (isSingleWord())
      return std::min(unsigned(llvh::countTrailingZeros(U.VAL)), BitWidth);
  ...
  unsigned countTrailingOnes() const {
    if (isSingleWord()) return llvh::countTrailingOnes(U.VAL);
  ...
  unsigned countPopulation() const {
    if (isSingleWord()) return llvh::countPopulation(U.VAL);
  ...
  unsigned nearestLogBase2() const {
    ...
    // UINT32_MAX.
    if (BitWidth == 1) return U.VAL - 1;

    // Handle the zero case.
    if (isNullValue()) return UINT32_MAX;
...
#endif  // CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_ADT_APINT_H_
