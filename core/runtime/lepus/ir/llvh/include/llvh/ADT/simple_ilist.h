...
#ifndef CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_ADT_SIMPLE_ILIST_H_
#define CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_ADT_SIMPLE_ILIST_H_
...
#include "core/runtime/lepus/ir/llvh/include/llvh/ADT/ilist_base.h"
#include "core/runtime/lepus/ir/llvh/include/llvh/ADT/ilist_iterator.h"
#include "core/runtime/lepus/ir/llvh/include/llvh/ADT/ilist_node.h"
#include "core/runtime/lepus/ir/llvh/include/llvh/ADT/ilist_node_options.h"
...
/// LLVH_ENABLE_ABI_BREAKING_CHECKS are enabled).
...
class simple_ilist
 ...
 public:
  ...
  // Copy constructor.
  simple_ilist(const simple_ilist& Other) {
    auto It = Other.begin();
    while (It != Other.end()) {
      push_back(T(*It));
      ++It;
  ...
  void insert(iterator I, Iterator First, Iterator Last) {
    for (; First != Last; ++First) insert(I, *First);
  }
  ...
  void cloneFrom(const simple_ilist& L2, Cloner clone, Disposer dispose) {
    ...
    for (const_reference V : L2) push_back(*clone(V));
  }
  ...
  iterator eraseAndDispose(iterator First, iterator Last, Disposer dispose) {
    while (First != Last) First = eraseAndDispose(First, dispose);
    return Last;
  }
...
template <typename T>
hash_code hash_value(simple_ilist<T>& S) {
  // Don't dereferencing the elements directly
  if (S.empty()) return hash_combine(0);

  return hash_combine(S.size(), reinterpret_cast<uintptr_t>(&S.front()));
}

}  // end namespace llvh

#endif  // CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_ADT_SIMPLE_ILIST_H_
