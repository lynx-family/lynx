...
#ifndef CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_ADT_BITVECTOR_H_
#define CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_ADT_BITVECTOR_H_
...
class BitVector {
 ...
 public:
  ...
  int find_first_in(unsigned Begin, unsigned End) const {
    ...
    for (unsigned i = FirstWord; i <= LastWord; ++i) {
      ...
      if (Copy != 0) return i * BITWORD_SIZE + countTrailingZeros(Copy);
    }
    return -1;
  }
  ...
  void resize(unsigned N, bool t = false) {
    ...
    Size = N;
    if (t || N < OldSize) clear_unused_bits();
  }

  void reserve(unsigned N) {
    if (N > getBitCapacity()) grow(N);
  }
  ...
  BitVector& set(unsigned I, unsigned E) {
    ...
    if (I < E) Bits[I / BITWORD_SIZE] |= PostfixMask;
  ...
  BitVector& flip() {
    for (unsigned i = 0; i < NumBitWords(size()); ++i) Bits[i] = ~Bits[i];
    clear_unused_bits();
    return *this;
  }
  ...
  BitVector& operator&=(const BitVector& RHS) {
    ...
    unsigned RHSWords = NumBitWords(RHS.size());
    unsigned i;
    for (i = 0; i != std::min(ThisWords, RHSWords); ++i) Bits[i] &= RHS.Bits[i];
  ...
  bool test(const BitVector& RHS) const {
    ...
    unsigned i;
    for (i = 0; i != std::min(ThisWords, RHSWords); ++i)
      if ((Bits[i] & ~RHS.Bits[i]) != 0) return true;
  ...
  BitVector& operator>>=(unsigned N) {
    assert(N <= Size);
    if (LLVM_UNLIKELY(empty() || N == 0)) return *this;
 ...
 private:
  ...
  void set_unused_bits(bool t = true) {
    ...
    if (Bits.size() > UsedWords) init_words(Bits.drop_front(UsedWords), t);
  ...
  void init_words(MutableArrayRef<BitWord> B, bool t) {
    if (B.size() > 0) memset(B.data(), 0 - (int)t, B.size() * sizeof(BitWord));
  }
...
inline void swap(llvh::BitVector& LHS, llvh::BitVector& RHS) { LHS.swap(RHS); }
}  // end namespace std

#endif  // CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_ADT_BITVECTOR_H_
