...
#ifndef CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_SUPPORT_GENERICDOMTREECONSTRUCTION_H_
#define CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_SUPPORT_GENERICDOMTREECONSTRUCTION_H_
...
#include "core/runtime/lepus/ir/llvh/include/llvh/ADT/DepthFirstIterator.h"
#include "core/runtime/lepus/ir/llvh/include/llvh/ADT/PointerIntPair.h"
...
struct SemiNCAInfo {
    ...
    static ResultTy Get(NodePtr N, BatchUpdatePtr BUI) {
      ...
      for (auto ChildAndKind : FCIt->second) {
        ...
        if (UK == UpdateKind::Insert) {
          ...
          LLVH_DEBUG(dbgs() << "\tShowing virtual edge " << BlockNamePrinter(N)
                            << " -> " << BlockNamePrinter(Child) << "\n");
          Res.push_back(Child);
        }
      }
  ...
  TreeNodePtr getNodeForBlock(NodePtr BB, DomTreeT& DT) {
    ...
    return (DT.DomTreeNodes[BB] = IDomNode->addChild(
                llvh::make_unique<DomTreeNodeBase<NodeT>>(BB, IDomNode)))
        .get();
  }
  ...
  NodePtr eval(NodePtr VIn, unsigned LastLinked) {
    ...
    if (VInInfo.DFSNum < LastLinked) return VIn;

    SmallVector<NodePtr, 32> Work;
    SmallPtrSet<NodePtr, 32> Visited;

    if (VInInfo.Parent >= LastLinked) Work.push_back(VIn);
    ...
    while (!Work.empty()) {
      ...
      if (VInfo.Parent < LastLinked) continue;
  ...
  void runSemiNCA(DomTreeT& DT, const unsigned MinLevel = 0) {
    ...
    for (unsigned i = NextDFSNum - 1; i >= 2; --i) {
      ...
      for (const auto& N : WInfo.ReverseChildren) {
        ...
        if (TN && TN->getLevel() < MinLevel) continue;
  ...
  static RootsT FindRoots(const DomTreeT& DT, BatchUpdatePtr BUI) {
    ...
    LLVH_DEBUG(dbgs() << "\t\tLooking for trivial roots\n");
    ...
    for (const NodePtr N : nodes(DT.Parent)) {
      ...
      if (!HasForwardSuccessors(N, BUI)) {
        ...
        LLVH_DEBUG(dbgs() << "Found a new trivial root: " << BlockNamePrinter(N)
                          << "\n");
        LLVH_DEBUG(dbgs() << "Last visited node: "
                          << BlockNamePrinter(SNCA.NumToNode[Num]) << "\n");
      }
    }

    LLVH_DEBUG(dbgs() << "\t\tLooking for non-trivial roots\n");
    ...
    LLVH_DEBUG(dbgs() << "Found roots: ");
    LLVH_DEBUG(for (auto* Root
                    : Roots) dbgs()
  ...
  static void RemoveRedundantRoots(const DomTreeT& DT, BatchUpdatePtr BUI,
    ...
    LLVH_DEBUG(dbgs() << "Removing redundant roots\n");
    ...
    for (unsigned i = 0; i < Roots.size(); ++i) {
      ...
      LLVH_DEBUG(dbgs() << "\tChecking if " << BlockNamePrinter(Root)
                        << " remains a root\n");
      SNCA.clear();
      ...
      for (unsigned x = 2; x <= Num; ++x) {
        ...
        // one.
        if (llvh::find(Roots, N) != Roots.end()) {
          LLVH_DEBUG(dbgs() << "\tForward DFS walk found another root "
                            << BlockNamePrinter(N) << "\n\tRemoving root "
                            << BlockNamePrinter(Root) << "\n");
  ...
  static void CalculateFromScratch(DomTreeT& DT, BatchUpdatePtr BUI) {
    ...
    DT.RootNode = (DT.DomTreeNodes[Root] =
                       llvh::make_unique<DomTreeNodeBase<NodeT>>(Root, nullptr))
                      .get();
  ...
  void attachNewSubtree(DomTreeT& DT, const TreeNodePtr AttachTo) {
    ...
    for (size_t i = 1, e = NumToNode.size(); i != e; ++i) {
      NodePtr W = NumToNode[i];
      LLVH_DEBUG(dbgs() << "\tdiscovered a new reachable node "
                        << BlockNamePrinter(W) << "\n");
      ...
      DT.DomTreeNodes[W] = IDomNode->addChild(
          llvh::make_unique<DomTreeNodeBase<NodeT>>(W, IDomNode));
    }
  }
  ...
  static void InsertEdge(DomTreeT& DT, const BatchUpdatePtr BUI,
    ...
    LLVH_DEBUG(dbgs() << "Inserting edge " << BlockNamePrinter(From) << " -> "
                      << BlockNamePrinter(To) << "\n");
    ...
    if (!FromTN) {
      ...
      FromTN =
          (DT.DomTreeNodes[From] = VirtualRoot->addChild(
               llvh::make_unique<DomTreeNodeBase<NodeT>>(From, VirtualRoot)))
              .get();
  ...
  static bool UpdateRootsBeforeInsertion(DomTreeT& DT, const BatchUpdatePtr BUI,
    ...
    auto RIt = llvh::find(DT.Roots, To->getBlock());
    if (RIt == DT.Roots.end())
      return false;  // To is not a root, nothing to update.

    LLVH_DEBUG(dbgs() << "\t\tAfter the insertion, " << BlockNamePrinter(To)
                      << " is no longer a root\n\t\tRebuilding the tree!!!\n");
  ...
  static void UpdateRootsAfterUpdate(DomTreeT& DT, const BatchUpdatePtr BUI) {
    ...
    if (DT.Roots.size() != Roots.size() ||
      ...
      LLVH_DEBUG(dbgs() << "Roots are different in updated trees\n"
  ...
  static void InsertReachable(DomTreeT& DT, const BatchUpdatePtr BUI,
    ...
    LLVH_DEBUG(dbgs() << "\tReachable " << BlockNamePrinter(From->getBlock())
    ...
    LLVH_DEBUG(dbgs() << "\t\tNCA == " << BlockNamePrinter(NCD) << "\n");
    ...
    InsertionInfo II;
    LLVH_DEBUG(dbgs() << "Marking " << BlockNamePrinter(To)
                      << " as affected\n");
    II.Affected.insert(To);
    const unsigned ToLevel = To->getLevel();
    LLVH_DEBUG(dbgs() << "Putting " << BlockNamePrinter(To)
                      << " into a Bucket\n");
    ...
    while (!II.Bucket.empty()) {
      ...
      II.Bucket.pop();
      LLVH_DEBUG(dbgs() << "\tAdding to Visited and AffectedQueue: "
  ...
  static void VisitInsertion(DomTreeT& DT, const BatchUpdatePtr BUI,
    ...
    LLVH_DEBUG(dbgs() << "Visiting " << BlockNamePrinter(TN) << ",  RootLevel "
                      << RootLevel << "\n");
    ...
    do {
      ...
      LLVH_DEBUG(dbgs() << " Next: " << BlockNamePrinter(Next) << "\n");
      ...
      for (const NodePtr Succ :
        ...
        LLVH_DEBUG(dbgs() << "\tSuccessor " << BlockNamePrinter(Succ)
                          << ", level = " << SuccLevel << "\n");

        // Do not process the same node multiple times.
        if (Processed.count(Next) > 0) continue;

        // Succ dominated by subtree From -- not affected.
        // (Based on the lemma 2.5 from the second paper.)
        if (SuccLevel > RootLevel) {
          LLVH_DEBUG(dbgs() << "\t\tDominated by subtree From\n");
          if (II.Visited.count(SuccTN) != 0) {
            LLVH_DEBUG(dbgs() << "\t\t\talready visited at level "
                              << II.Visited[SuccTN] << "\n\t\t\tcurrent level "
                              << RootLevel << ")\n");

            // A node can be necessary to visit again if we see it again at
            // a lower level than before.
            if (II.Visited[SuccTN] >= RootLevel) continue;
          }

          LLVH_DEBUG(dbgs() << "\t\tMarking visited not affected "
          ...
          LLVH_DEBUG(dbgs() << "\t\tMarking affected and adding "
  ...
  static void UpdateInsertion(DomTreeT& DT, const BatchUpdatePtr BUI,
                              const TreeNodePtr NCD, InsertionInfo& II) {
    LLVH_DEBUG(dbgs() << "Updating NCD = " << BlockNamePrinter(NCD) << "\n");

    for (const TreeNodePtr TN : II.AffectedQueue) {
      LLVH_DEBUG(dbgs() << "\tIDom(" << BlockNamePrinter(TN)
  ...
  static void UpdateLevelsAfterInsertion(InsertionInfo& II) {
    ...
    for (const TreeNodePtr TN : II.VisitedNotAffectedQueue) {
      LLVH_DEBUG(dbgs() << "\tlevel(" << BlockNamePrinter(TN) << ") = ("
  ...
  static void InsertUnreachable(DomTreeT& DT, const BatchUpdatePtr BUI,
    ...
    LLVH_DEBUG(dbgs() << "Inserting " << BlockNamePrinter(From)
    ...
    LLVH_DEBUG(dbgs() << "Inserted " << BlockNamePrinter(From)
    ...
    for (const auto& Edge : DiscoveredEdgesToReachable) {
      LLVH_DEBUG(dbgs() << "\tInserting discovered connecting edge "
  ...
  static void ComputeUnreachableDominators(
    ...
    LLVH_DEBUG(dbgs() << "After adding unreachable nodes\n");
  }

  static void DeleteEdge(DomTreeT& DT, const BatchUpdatePtr BUI,
                         const NodePtr From, const NodePtr To) {
    assert(From && To && "Cannot disconnect nullptrs");
    LLVH_DEBUG(dbgs() << "Deleting edge " << BlockNamePrinter(From) << " -> "
                      << BlockNamePrinter(To) << "\n");
    ...
    auto IsSuccessor = [BUI](const NodePtr SuccCandidate, const NodePtr Of) {
      ...
      return llvh::find(Successors, SuccCandidate) != Successors.end();
    };
    (void)IsSuccessor;
    ...
    if (ToTN != NCD) {
      ...
      LLVH_DEBUG(dbgs() << "\tNCD " << BlockNamePrinter(NCD) << ", ToIDom "
  ...
  static void DeleteReachable(DomTreeT& DT, const BatchUpdatePtr BUI,
    ...
    LLVH_DEBUG(dbgs() << "Deleting reachable " << BlockNamePrinter(FromTN)
                      << " -> " << BlockNamePrinter(ToTN) << "\n");
    LLVH_DEBUG(dbgs() << "\tRebuilding subtree\n");
    ...
    // scratch.
    if (!PrevIDomSubTree) {
      LLVH_DEBUG(dbgs() << "The entire tree needs to be rebuilt\n");
      CalculateFromScratch(DT, BUI);
      return;
    }
    ...
    LLVH_DEBUG(dbgs() << "\tTop of subtree: " << BlockNamePrinter(ToIDomTN)
                      << "\n");

    SemiNCAInfo SNCA(BUI);
    SNCA.runDFS(ToIDom, 0, DescendBelow, 0);
    LLVH_DEBUG(dbgs() << "\tRunning Semi-NCA\n");
  ...
  static bool HasProperSupport(DomTreeT& DT, const BatchUpdatePtr BUI,
    ...
    LLVH_DEBUG(dbgs() << "IsReachableFromIDom " << BlockNamePrinter(TN)
                      << "\n");
    for (const NodePtr Pred :
         ChildrenGetter<!IsPostDom>::Get(TN->getBlock(), BUI)) {
      LLVH_DEBUG(dbgs() << "\tPred " << BlockNamePrinter(Pred) << "\n");
      if (!DT.getNode(Pred)) continue;

      const NodePtr Support =
          DT.findNearestCommonDominator(TN->getBlock(), Pred);
      LLVH_DEBUG(dbgs() << "\tSupport " << BlockNamePrinter(Support) << "\n");
      if (Support != TN->getBlock()) {
        LLVH_DEBUG(dbgs() << "\t" << BlockNamePrinter(TN)
  ...
  static void DeleteUnreachable(DomTreeT& DT, const BatchUpdatePtr BUI,
                                const TreeNodePtr ToTN) {
    LLVH_DEBUG(dbgs() << "Deleting unreachable subtree "
                      << BlockNamePrinter(ToTN) << "\n");
    assert(ToTN);
    ...
    if (IsPostDom) {
      ...
      LLVH_DEBUG(dbgs() << "\tDeletion made a region reverse-unreachable\n");
      LLVH_DEBUG(dbgs() << "\tAdding new root " << BlockNamePrinter(ToTN)
                        << "\n");
    ...
    auto DescendAndCollect = [Level, &AffectedQueue, &DT](NodePtr, NodePtr To) {
      ...
      if (llvh::find(AffectedQueue, To) == AffectedQueue.end())
    ...
    for (const NodePtr N : AffectedQueue) {
      ...
      LLVH_DEBUG(dbgs() << "Processing affected node " << BlockNamePrinter(TN)
    ...
    for (unsigned i = LastDFSNum; i > 0; --i) {
      const NodePtr N = SNCA.NumToNode[i];
      const TreeNodePtr TN = DT.getNode(N);
      LLVH_DEBUG(dbgs() << "Erasing node " << BlockNamePrinter(TN) << "\n");
    ...
    LLVH_DEBUG(dbgs() << "DeleteUnreachable: running DFS with MinNode = "
    ...
    LLVH_DEBUG(dbgs() << "Previous IDom(MinNode) = "
  ...
  static void EraseNode(DomTreeT& DT, const TreeNodePtr TN) {
    ...
    auto ChIt = llvh::find(IDom->Children, TN);
  ...
  static void ApplyUpdates(DomTreeT& DT, ArrayRef<UpdateT> Updates) {
    ...
    BatchUpdateInfo BUI;
    LLVH_DEBUG(dbgs() << "Legalizing " << BUI.Updates.size() << " updates\n");
    ...
    LLVH_DEBUG(dbgs() << "About to apply " << NumLegalized << " updates\n");
    LLVH_DEBUG(if (NumLegalized < 32) for (const auto& U
                                           : reverse(BUI.Updates)) {
      dbgs() << "\t";
      U.dump();
      dbgs() << "\n";
  ...
  static void ApplyNextUpdate(DomTreeT& DT, BatchUpdateInfo& BUI) {
    ...
    LLVH_DEBUG(dbgs() << "Applying update: ");
    LLVH_DEBUG(CurrentUpdate.dump(); dbgs() << "\n");
  ...
  static bool VerifyDFSNumbers(const DomTreeT& DT) {
    if (!DT.DFSInfoValid || !DT.Parent) return true;
  ...
  bool verifyParentProperty(const DomTreeT& DT) {
    for (auto& NodeToTN : DT.DomTreeNodes) {
      ...
      LLVH_DEBUG(dbgs() << "Verifying parent property of node "
                        << BlockNamePrinter(TN) << "\n");
      clear();
...
bool Verify(const DomTreeT& DT, typename DomTreeT::VerificationLevel VL) {
  ...
  if (!SNCA.IsSameAsFreshTree(DT)) return false;
  ...
  if (VL == DomTreeT::VerificationLevel::Basic ||
    ...
    if (!SNCA.verifyParentProperty(DT)) return false;
  if (VL == DomTreeT::VerificationLevel::Full)
    if (!SNCA.verifySiblingProperty(DT)) return false;
...
#endif  // CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_SUPPORT_GENERICDOMTREECONSTRUCTION_H_
