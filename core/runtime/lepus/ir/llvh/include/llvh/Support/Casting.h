...
#ifndef CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_SUPPORT_CASTING_H_
#define CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_SUPPORT_CASTING_H_
...
template <class X, class Y>
LLVM_NODISCARD inline typename cast_retty<X, Y*>::ret_type cast_or_null(
    Y* Val) {
  if (!Val) return nullptr;
  assert(isa<X>(Val) && "cast_or_null<Ty>() argument of incompatible type!");
  return cast<X>(Val);
...
template <class X, class Y>
LLVM_NODISCARD inline typename cast_retty<X, Y*>::ret_type dyn_cast_or_null(
    Y* Val) {
  return (Val && isa<X>(Val)) ? cast<X>(Val) : nullptr;
}
...
#endif  // CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_SUPPORT_CASTING_H_
