...
#ifndef CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_SUPPORT_CFGUPDATE_H_
#define CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_SUPPORT_CFGUPDATE_H_

#include <algorithm>
#include <utility>

#include "core/runtime/lepus/ir/llvh/include/llvh/ADT/APInt.h"
...
void LegalizeUpdates(ArrayRef<Update<NodePtr>> AllUpdates,
  ...
  for (const auto& U : AllUpdates) {
    ...
    NodePtr To = U.getTo();
    if (InverseGraph) std::swap(From, To);  // Reverse edge for postdominators.
...
#endif  // CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_SUPPORT_CFGUPDATE_H_
