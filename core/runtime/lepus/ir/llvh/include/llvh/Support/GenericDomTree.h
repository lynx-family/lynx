...
#ifndef CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_SUPPORT_GENERICDOMTREE_H_
#define CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_SUPPORT_GENERICDOMTREE_H_
...
#include "core/runtime/lepus/ir/llvh/include/llvh/ADT/GraphTraits.h"
#include "core/runtime/lepus/ir/llvh/include/llvh/ADT/PointerIntPair.h"
#include "core/runtime/lepus/ir/llvh/include/llvh/ADT/STLExtras.h"
...
#include "core/runtime/lepus/ir/llvh/include/llvh/Support/CFGUpdate.h"
...
class DomTreeNodeBase {
 ...
 public:
  ...
  bool compare(const DomTreeNodeBase* Other) const {
    if (getNumChildren() != Other->getNumChildren()) return true;
    ...
    for (const DomTreeNodeBase* I : *this) {
      ...
      if (OtherChildren.count(N) == 0) return true;
    }
    return false;
  }
...
struct dom_trait {
  using parent_type = decltype(std::declval<NodeT*>()->getParent());
  static parent_type* cast(parent_type* in) { return in; }
};
...
class DominatorTreeBase {
 public:
  ...
  using NodePtr = NodeT*;
  using ParentPtr = typename dom_trait<NodeT>::parent_type;
 ...
 public:
  ...
  DomTreeNodeBase<NodeT>* getNode(const NodeT* BB) const {
    ...
    if (I != DomTreeNodes.end()) return I->second.get();
    return nullptr;
  }
  ...
  bool dominates(const DomTreeNodeBase<NodeT>* A,
    ...
    if (DFSInfoValid) return B->DominatedBy(A);
  ...
  NodeT* findNearestCommonDominator(NodeT* A, NodeT* B) const {
    ...
    if (!isPostDominator()) {
      NodeT& Entry = A->getParentOp()->front();
      if (A == &Entry || B == &Entry) return &Entry;
    }
  ...
  DomTreeNodeBase<NodeT>* setNewRoot(NodeT* BB) {
    ...
    DFSInfoValid = false;
    DomTreeNodeBase<NodeT>* NewNode =
        (DomTreeNodes[BB] =
             llvh::make_unique<DomTreeNodeBase<NodeT>>(BB, nullptr))
            .get();
    if (Roots.empty()) {
      addRoot(BB);
    } else {
...
#endif  // CORE_RUNTIME_LEPUS_IR_LLVH_INCLUDE_LLVH_SUPPORT_GENERICDOMTREE_H_
