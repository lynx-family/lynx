...
#include "core/runtime/lepus/ir/llvh/include/llvh/Support/Allocator.h"
...
void FoldingSetNodeID::AddInteger(unsigned long I) {
  if (sizeof(long) == sizeof(int))
    ...
    llvh_unreachable("unexpected sizeof(long)");
  }
}
...
/// interned data.
FoldingSetNodeIDRef FoldingSetNodeID::Intern(
...
static FoldingSetBase::Node* GetNextPtr(void* NextInBucketPtr) {
  ...
  if (reinterpret_cast<intptr_t>(NextInBucketPtr) & 1) return nullptr;
...
///
void FoldingSetBase::GrowHashTable() { GrowBucketCount(NumBuckets * 2); }
...
FoldingSetBase::Node* FoldingSetBase::FindNodeOrInsertPos(
    const FoldingSetNodeID& ID, void*& InsertPos) {
  ...
  FoldingSetNodeID TempID;
  while (Node* NodeInBucket = GetNextPtr(Probe)) {
    if (NodeEquals(NodeInBucket, ID, IDHash, TempID)) return NodeInBucket;
    TempID.clear();
...
FoldingSetBase::Node* FoldingSetBase::GetOrInsertNode(FoldingSetBase::Node* N) {
  ...
  GetNodeProfile(N, ID);
  void* IP;
  if (Node* E = FindNodeOrInsertPos(ID, IP)) return E;
  InsertNode(N, IP);
  return N;
}
