...
#ifndef CORE_RUNTIME_LEPUS_IR_LLVH_LIB_SUPPORT_UNIX_UNIX_H_
#define CORE_RUNTIME_LEPUS_IR_LLVH_LIB_SUPPORT_UNIX_UNIX_H_
...
#include "core/runtime/lepus/ir/llvh/include/llvh/Support/Errno.h"
#include "llvh/Config/llvh-config.h"  // Get autoconf configuration settings
...
inline struct timespec toTimeSpec(TimePoint<> TP) {
  ...
  RetVal.tv_nsec = (TP.time_since_epoch() % std::chrono::seconds(1)).count();
  return RetVal;
}
...
inline struct timeval toTimeVal(TimePoint<std::chrono::microseconds> TP) {
  ...
  RetVal.tv_sec = toTimeT(TP);
  RetVal.tv_usec = (TP.time_since_epoch() % std::chrono::seconds(1)).count();
  return RetVal;
}
...
#endif  // CORE_RUNTIME_LEPUS_IR_LLVH_LIB_SUPPORT_UNIX_UNIX_H_
