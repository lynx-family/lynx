...
function case9DataProcessor(raw) {
  let data = raw || {};
  ...
  let className = getClassName("case9-summary");
...
renderPage = function ($renderData) {
  ...
  $cardInstance = $cardConstructor($currentComponentId, "src/cards/case9/index");
  if ($renderData) {
    Object.assign($cardInstance.data, $renderData);
  }
  let processed = processData($cardInstance.data, "case9_data_processor");
...
updatePage = function ($newData, options) {
  ...
  let processed = processData(merged, "case9_data_processor");
...
registerDataProcessor(case9DataProcessor, "case9_data_processor");
renderPage({ seed: 1, limit: 2, label: "alpha" });
let diff = updatePage({ seed: 2, limit: 5, label: "gamma" }, { mode: "case9" });
let root = $page.children[0];
var result = root.children[0].text + "|" + root.children[1].text;

Assert(diff.changed === true, "case9: diff.changed");
Assert($update === true, "case9: update flag");
Assert($cardInstance.data.total === 1856, "case9: total");
Assert($cardInstance.data.checksum === 2251, "case9: checksum");
Assert($cardInstance.data.bias === 390, "case9: bias");
Assert($cardInstance.data.accent === 322, "case9: accent");
Assert(root.children[0].attrs["class"] === "case9-summary mall-font-large", "case9: summary class");
Assert(root.children[1].attrs["font-scale"] === 1.15, "case9: font scale");
Assert(root.children[1].attrs["version"] === 1, "case9: version");
Assert(root.children[1].attrs["mask-style"] === "width: 180.5px;height: 286.507936507937px", "case9: mask style");
Assert(root.children[0].text === "update|gamma|1856|2251|251", "case9: summary text");
Assert(root.children[1].text === "bias=390|stable=573|pick=2,66,251", "case9: detail text");
Assert(result === "update|gamma|1856|2251|251|bias=390|stable=573|pick=2,66,251", "case9: result");
