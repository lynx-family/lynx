...
function case8DataProcessor(raw) {
  let data = raw || {};
  ...
  let className = getClassName("case8-summary");
...
renderPage = function ($renderData) {
  ...
  $cardInstance = $cardConstructor($currentComponentId, "src/cards/case8/index");
  if ($renderData) {
    Object.assign($cardInstance.data, $renderData);
  }
  let processed = processData($cardInstance.data, "case8_data_processor");
...
updatePage = function ($newData, options) {
  ...
  let processed = processData(merged, "case8_data_processor");
...
registerDataProcessor(case8DataProcessor, "case8_data_processor");
renderPage({ seed: 1, limit: 2, label: "alpha" });
let diff = updatePage({ seed: 3, limit: 4, label: "beta" }, { mode: "case8" });
let root = $page.children[0];
var result = root.children[0].text + "|" + root.children[1].text;

Assert(diff.changed === true, "case8: diff.changed");
Assert($update === true, "case8: update flag");
Assert($cardInstance.data.total === 1784, "case8: total");
Assert($cardInstance.data.checksum === 2179, "case8: checksum");
Assert($cardInstance.data.bias === 391, "case8: bias");
Assert(root.children[0].text === "update|beta|1784|2179|251", "case8: summary text");
Assert(root.children[1].text === "bias=391|stable=573|pick=3,66,251", "case8: detail text");
Assert(result === "update|beta|1784|2179|251|bias=391|stable=573|pick=3,66,251", "case8: result");
