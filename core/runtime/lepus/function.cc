...
long Function::GetClosureVarToplevelReg(long index) {
  if (upvalue_index_to_toplevel_reg_.find(index) ==
      upvalue_index_to_toplevel_reg_.end()) {
    return -1;
  }
  return upvalue_index_to_toplevel_reg_[index];
}

void Function::RecordUpvalueIndex2ToplevelReg(long index, long reg_index) {
