...
struct Instruction {
  ...
  static Instruction ABCDCode(long d, long a, long b, long c) {
  ...
  inline static bool IsJmpInstruction(Instruction& i) {
    auto opcode = GetOpCode(i);
    return opcode == TypeOp_Jmp || opcode == TypeOp_JmpTrue ||
           opcode == TypeOp_JmpFalse || opcode == TypeOp_BoolJmpFalse ||
           opcode == TypeOp_BoolJmpTrue;
  }

  inline static bool IsCmpJmpInstruction(Instruction& i) {
    auto opcode = GetOpCode(i);
