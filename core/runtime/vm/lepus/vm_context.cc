...
#define GET_REGISTER_A(i) \
  RestrictedValue* a = (regs + Instruction::GetParamA(i));
#define GET_REGISTER_B(i) \
  RestrictedValue* b = (regs + Instruction::GetParamB(i));
#define GET_REGISTER_C(i) \
  RestrictedValue* c = (regs + Instruction::GetParamC(i));
...
#define GET_REGISTER_ABC_FROM_CTX(ctx)               \
  RestrictedValue* a = GET_REGISTER_A_FROM_CTX(ctx); \
  RestrictedValue* b = GET_REGISTER_B_FROM_CTX(ctx); \
  RestrictedValue* c = GET_REGISTER_C_FROM_CTX(ctx);
...
RestrictedValue* VMContext::CallPrologue(const base::String& name) {
  ...
  RestrictedValue* function = heap_.top_;
...
bool VMContext::UpdateTopLevelVariableByPath(base::Vector<std::string>& path,
  ...
  RestrictedValue* ptr = heap_.base() + reg + 1;
...
bool VMContext::CheckTableShadowUpdatedWithTopLevelVariable(
  ...
  for (auto& update_data_iterator : *update_table_value) {
    ...
    const RestrictedValue* ptr = heap_.base() + reg + 1;
...
int32_t VMContext::CallFunction(RestrictedValue* function, size_t argc,
  ...
  if (unlikely(function->IsClosure())) {
    ...
    if (function->GetCFunctionType() == CFunctionType_Default) {
      ...
      RestrictedValue::Assign(*ret, cfunction(this));
    } else {
...
#define DECLARE(x) &&case_##x,
  ...
  RestrictedValue* regs = current_frame_->register_;
...
#define BREAK SWITCH(Instruction::GetOpCode(i))
    ...
    done_LoadNil:
      ...
      CASE(TypeOp_GetContextSlot) : CASE(TypeOp_SetContextSlot) : {
        ...
        if (op_code == TypeOp_GetContextSlot) {
          *a = array->get(index);
          ...
          CArray::Unsafe::SetIndex(*array, index, *a);
        }
      }
      BREAK;
      CASE(TypeOp_GetUpvalue) : {
        GET_REGISTER_A(i);
        RestrictedValue* b = GET_UPVALUE_B(i);
        RestrictedValue::Assign(*a, *b);
      }
      BREAK;
      ...
      CASE(TypeOp_SetContextSlotMove) : {
        ...
        GET_REGISTER_C(i);
        CArray::Unsafe::SetIndex(*a->Array(), array_index, *c);
      }
      BREAK;
      ...
      CASE(TypeOp_NewArray) : {
        ...
        for (int i = 0; i < argc; ++i) {
          CArray::Unsafe::PushBack(*arr, *(a + i + 1));
        }
      ...
      CASE(TypeOp_CreateContext) : {
        ...
        // context + data
        {
          ...
          CArray::Unsafe::SetIndex(*arr, 0, current_closure->GetContext());
          a->SetArray(std::move(arr));
        }
      ...
      CASE(TypeOp_SetTable) : {
        ...
        if (a->IsTable() && b->IsString()) {
          ...
          if (likely(b->IsInt64())) {
            CArray::Unsafe::SetIndex(*UnsafeOp::TypeSure::GetArray(*a),
                                     UnsafeOp::TypeSure::GetInt64(*b), *c);
          } else if (b->IsNumber()) {
            CArray::Unsafe::SetIndex(
                *UnsafeOp::TypeSure::GetArray(*a),
                static_cast<size_t>(
...
void VMContext::GenerateClosure(RestrictedValue* value, long index) {
  ...
  for (int i = 0; static_cast<size_t>(i) < upvalues_count; ++i) {
    ...
    if (info->in_parent_vars_) {
      RestrictedValue* v = frame->register_ + info->register_;
