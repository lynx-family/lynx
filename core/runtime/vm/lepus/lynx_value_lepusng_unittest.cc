...
class LynxValueLepusNGTest : public ::testing::Test {
 protected:
  LynxValueLepusNGTest()
    ...
    lynx_value_api_attach_lepusng(env_, ctx_);
  }
...
TEST_F(LynxValueLepusNGTest, LynxValueString) {
  ...
  std::string str;
  status = env_->lynx_value_to_string_utf8(env_, l_val_str, &str);
  ASSERT_TRUE(status == lynx_api_ok);
...
static void ArrayIteratorCallback(lynx_api_env env, lynx_value key,
                                  lynx_value value, void* pfunc,
                                  void* raw_data) {
  ASSERT_TRUE(env != nullptr);
  int32_t i = *(reinterpret_cast<int32_t*>(raw_data));
  int32_t val_key;
  env->lynx_value_get_int32(env, key, &val_key);
  ASSERT_TRUE(i == val_key);
  (*(reinterpret_cast<int32_t*>(raw_data)))++;
}
...
TEST_F(LynxValueLepusNGTest, LynxValueArray) {
  ...
  int32_t* raw_data = new int32_t;
  *raw_data = 0;
  status = env_->lynx_value_iterate_value(
      env_, l_val_arr, ArrayIteratorCallback, nullptr, raw_data);
  ASSERT_TRUE(status == lynx_api_ok);
...
static void MapIteratorCallback(lynx_api_env env, lynx_value key,
                                lynx_value value, void* pfunc, void* raw_data) {
  ASSERT_TRUE(env != nullptr);
  void* str_ref;
  env->lynx_value_get_string_ref(env, key, &str_ref);
  auto* base_str = reinterpret_cast<base::RefCountedStringImpl*>(str_ref);
  if (base_str->str() == "key1") {
    int32_t val;
    env->lynx_value_get_int32(env, value, &val);
    ASSERT_TRUE(val == 10);
  }
}
...
TEST_F(LynxValueLepusNGTest, LynxValueMap) {
  ...
  status = env_->lynx_value_iterate_value(env_, l_val_map, MapIteratorCallback,
                                          nullptr, nullptr);
  ASSERT_TRUE(status == lynx_api_ok);
...
TEST_F(LynxValueLepusNGTest, LynxValueRefCounted) {
  ...
  std::string str;
  status = env_->lynx_value_to_string_utf8(env_, l_val_arr, &str);
  ASSERT_TRUE(status == lynx_api_ok);
  LOGI("str: " << str);

  bool is_arr;
  status = env_->lynx_value_is_array(env_, l_val_arr, &is_arr);
  ASSERT_TRUE(status == lynx_api_ok);
  ASSERT_TRUE(is_arr);
