...
piper::Value JSVMHelper::createValue(JSVM_Value value, JSVM_Env env) {
  ...
  JSVM_CALL_RETURN(OH_JSVM_Typeof, piper::Value(), env, value, &type);
  if (type == JSVM_ValueType::JSVM_NUMBER) {
    double result;
    JSVM_CALL_RETURN(OH_JSVM_GetValueDouble, piper::Value(), env, value,
                     &result);
  ...
  if (type == JSVM_ValueType::JSVM_BIGINT) {
    int64_t result;
    bool lossless;
    JSVM_CALL_RETURN(OH_JSVM_GetValueBigintInt64, piper::Value(), env, value,
                     &result, &lossless);
    return piper::Value(static_cast<int>(result));
  }
  if (type == JSVM_ValueType::JSVM_BOOLEAN) {
    bool result;
    JSVM_CALL_RETURN(OH_JSVM_GetValueBool, piper::Value(), env, value, &result);
    return piper::Value(result);
  }
...
void JSVMHelper::symbolRef(const piper::Symbol& sym, JSVM_Value* value) {
  ...
  JSVM_CALL(OH_JSVM_GetReferenceValue, jsvm_sym->env_, jsvm_sym->sym_ref_,
            value);
}
...
void JSVMHelper::stringRef(const piper::String& str, JSVM_Value* value) {
  ...
  JSVM_CALL(OH_JSVM_GetReferenceValue, jsvm_str->env_, jsvm_str->str_ref_,
            value);
}
...
void JSVMHelper::objectRef(const piper::Object& obj, JSVM_Value* value) {
  ...
  JSVM_CALL(OH_JSVM_GetReferenceValue, jsvm_obj->env_, jsvm_obj->obj_ref_,
            value);
}

std::string JSVMHelper::JSStringToSTLString(JSVM_Value s, JSVM_Env env) {
  size_t len = 0;
  JSVM_CALL_RETURN(OH_JSVM_GetValueStringUtf8, std::string(), env, s, nullptr,
                   JSVM_AUTO_LENGTH, &len);
  std::string output_str;
  output_str.resize(len + 1);
  JSVM_CALL_RETURN(OH_JSVM_GetValueStringUtf8, std::string(), env, s,
...
std::optional<piper::Value> JSVMHelper::call(piper::JSVMRuntime* rt,
  ...
  if (this_value == nullptr) {
    JSVM_CALL_RETURN(OH_JSVM_GetGlobal, std::optional<piper::Value>(),
  ...
  JSVM_CALL_RETURN(OH_JSVM_CallFunction, std::optional<piper::Value>(),
...
void JSVMHelper::ThrowJsException(JSVM_Env env,
  ...
  JSVM_CALL(OH_JSVM_ThrowError, env, error_stack.c_str(),
            error_message.c_str());
}
