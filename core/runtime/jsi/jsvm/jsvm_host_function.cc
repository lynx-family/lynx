...
JSVM_Value JSVMHostFunctionProxy::createFunctionFromHostFunction(
  ...
  if (function_template_ref == nullptr) {
    ...
    static JSVM_CallbackStruct ctor_callback = {
        .callback = [](JSVM_Env env, JSVM_CallbackInfo info) -> JSVM_Value {
          ...
          JSVM_CALL(OH_JSVM_GetCbInfo, env, info, &argc, &argv, &this_value,
                    nullptr);
          uint64_t ptr = 0;
          bool lossless = false;
          ...
          JSVM_CALL(OH_JSVM_Wrap, env, this_value,
                    reinterpret_cast<void*>(proxy),
                    JSVMHostFunctionProxy::onFinalize, nullptr, nullptr);
          JSVM_CALL(OH_JSVM_TypeTagObject, env, this_value,
                    GetHostFunctionTag());
          return this_value;
        },
        .data = nullptr};
    ...
    JSVM_CALL(OH_JSVM_DefineClassWithPropertyHandler, env, "HostFunctionClazz",
              JSVM_AUTO_LENGTH, &ctor_callback, 0, nullptr, &property_cfg,
              &call_as_func_callback, &function_template);
  ...
  JSVM_CALL(OH_JSVM_CreateBigintUint64, env, reinterpret_cast<uint64_t>(proxy),
            &ptr_value);
  JSVM_CALL(OH_JSVM_NewInstance, env, function_template, 1, &ptr_value,
            &instance);
...
JSVM_Value JSVMHostFunctionProxy::FunctionCallback(JSVM_Env env,
  ...
  JSVM_CALL(OH_JSVM_Unwrap, env, this_value,
