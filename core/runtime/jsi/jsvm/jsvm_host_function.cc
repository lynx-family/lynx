...
JSVM_Value JSVMHostFunctionProxy::createFunctionFromHostFunction(
  ...
  if (function_template_ref == nullptr) {
    ...
    static JSVM_CallbackStruct ctor_callback = {
        .callback = [](JSVM_Env env, JSVM_CallbackInfo info) -> JSVM_Value {
          ...
          JSVM_CALL(OH_JSVM_TypeTagObject,
                    (env, this_value, GetHostFunctionTag()));
          return this_value;
        },
        .data = nullptr};
    ...
    JSVM_CALL(OH_JSVM_CreateReference,
              (env, function_template, 1, &function_template_ref));
    rt->SetHostFunctionTemplate(function_template_ref);
  } else {
    JSVM_CALL(OH_JSVM_GetReferenceValue,
              (env, function_template_ref, &function_template));
  }
  ...
  JSVM_CALL(OH_JSVM_NewInstance,
            (env, function_template, 1, &ptr_value, &instance));
...
JSVM_Value JSVMHostFunctionProxy::FunctionCallback(JSVM_Env env,
  ...
  size_t argc = 0;
  JSVM_CALL(OH_JSVM_GetCbInfo,
            (env, info, &argc, nullptr, &this_value, nullptr));
  JSVMHostFunctionProxy* proxy_ptr = nullptr;
  JSVM_CALL(OH_JSVM_Unwrap,
            (env, this_value, reinterpret_cast<void**>(&proxy_ptr)));
