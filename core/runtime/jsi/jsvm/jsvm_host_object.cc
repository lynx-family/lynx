...
JSVM_Value JSVMHostObjectProxy::getProperty(JSVM_Env env, JSVM_Value name,
  ...
  JSVM_CALL(OH_JSVM_Unwrap, env, this_arg,
...
piper::Object JSVMHostObjectProxy::createObject(
  ...
  if (object_template_ref == nullptr) {
    ...
    JSVM_CALL(OH_JSVM_DefineClassWithPropertyHandler, env,
    ...
    JSVM_CALL(OH_JSVM_CreateReference, env, object_template, 1,
              &object_template_ref);
    rt->SetHostObjectTemplate(object_template_ref);
  } else {
    JSVM_CALL(OH_JSVM_GetReferenceValue, env, object_template_ref,
              &object_template);
  }
  ...
  JSVM_CALL(OH_JSVM_CreateBigintUint64, env, reinterpret_cast<uint64_t>(proxy),
            &ptr_value);
  JSVM_CALL(OH_JSVM_NewInstance, env, object_template, 1, &ptr_value,
            &instance);
