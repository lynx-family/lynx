...
std::vector<PropNameID> JSBI::getPropertyNames(Runtime& rt) {
  ...
  vec.push_back(PropNameID::forUtf8(rt, CONSTRUCTOR_BIG_INT));
  vec.push_back(PropNameID::forUtf8(rt, OPERATOR_ADD));
  vec.push_back(PropNameID::forUtf8(rt, OPERATOR_SUBTRACT));
  vec.push_back(PropNameID::forUtf8(rt, OPERATOR_MULTIPLY));
  vec.push_back(PropNameID::forUtf8(rt, OPERATOR_DIVIDE));
  vec.push_back(PropNameID::forUtf8(rt, OPERATOR_REMAINDER));
  vec.push_back(PropNameID::forUtf8(rt, OPERATOR_EQUAL));
  vec.push_back(PropNameID::forUtf8(rt, OPERATOR_NOT_EQUAL));
  vec.push_back(PropNameID::forUtf8(rt, OPERATOR_LESS_THAN));
  vec.push_back(PropNameID::forUtf8(rt, OPERATOR_LESS_THAN_OR_EQUAL));
  vec.push_back(PropNameID::forUtf8(rt, OPERATOR_GREATER_THAN));
  vec.push_back(PropNameID::forUtf8(rt, OPERATOR_GREATER_THAN_OR_EQUAL));
...
base::expected<Value, JSINativeException> JSBI::BigInt(
  ...
  if (count > 0) {
    ...
    auto bigint = runtime::js::BigInt::createWithString(*rt, number);
    if (!bigint) {
...
base::expected<Value, JSINativeException> JSBI::operate(
  ...
  if (count >= 2) {
    ...
    if (num1->isObject() && num2->isObject()) {
      ...
      std::optional<runtime::js::BigInt> bigint_res;
      if (func_name.compare(OPERATOR_ADD) == 0) {
        BigInteger res = big_int_num1 + big_int_num2;

        bigint_res = runtime::js::BigInt::createWithString(*rt, res.toString());
        ...
        return Value(isGreaterThanOrEqual);
      }
      if (bigint_res) {
        return *bigint_res;
      }
