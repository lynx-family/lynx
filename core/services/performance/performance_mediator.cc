...
void PerformanceMediator::OnPerformanceEvent(
    std::unique_ptr<pub::Value> performance_entry, PerformanceObserverEnv env) {
  ...
  if ((env & kPerformanceObserverOfBTSEngine) && runtime_actor_) {
  ...
  if ((env & kPerformanceObserverOfMTSEngine) && engine_actor_) {
