...
class JSBlockingMonitor {
 public:
  ...
  // @param event entry
  void OnSetupEvent(const std::unique_ptr<pub::Value>& entry_map);
  ...
  // Forces JS blocking monitoring to be enabled or disabled, overriding other
  // settings.
 ...
 private:
  ...
  void ReportBlockingInfo(const std::string& stage, int64_t total_duration_ms,
                          int64_t time_after_fcp_ms);

  void ReportWithTimer(int8_t index);
  lynx::tasm::performance::PerformanceEventSender* sender_;
