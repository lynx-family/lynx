...
bool WriteFixtureZip(const std::string& zip_path, const FixtureData& data) {
  ...
  // NativeModule call matcher.
  ScriptBuilder ignored_keys;
  ignored_keys << "[";
  bool first_key = true;
  for (const char* key : kDefaultIgnoredKeys) {
    ignored_keys << (first_key ? "\"" : ",\"") << key << "\"";
    first_key = false;
  }
  ...
  if (!config_doc.HasParseError() && config_doc.IsObject() &&
    ...
    for (auto& item : config_doc["jsbIgnoredInfo"].GetArray()) {
      if (item.IsString()) {
        ignored_keys << "," << JsonEscapeString(item.GetString());
      }
    }
  }
  ignored_keys << "]";
  fixture << BuildNativeModuleMatcherScript(ignored_keys.str());
  fixture << "\n";

  // NativeModule handlers.
