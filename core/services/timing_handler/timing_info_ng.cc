...
std::unique_ptr<lynx::pub::Value> TimingInfoNg::GetMetricFcpEntry(
  ...
  uint64_t load_bundle_paint_end = 0;
  uint64_t load_bundle_start = 0;
  if (pipeline_id.empty()) {
    // If there's no pipeline ID, check if metrics_[kLynxFCP] exists.
    ...
    load_bundle_paint_end = timing_map.GetTimestamp(kPaintEnd).value_or(0);
    load_bundle_start = timing_map.GetTimestamp(kLoadBundleStart).value_or(0);
  }
...
std::unique_ptr<lynx::pub::Value> TimingInfoNg::GetMetricTtiEntry(
  ...
  uint64_t load_bundle_paint_end = 0;
  uint64_t load_bundle_start = 0;
  uint64_t load_background_end = 0;
  if (pipeline_id.empty()) {
    // If there's no pipeline ID, check if metrics_[kLynxTTi] exists.
    //      If not, exit; if it exists, extract kPaintEnd for calculation.
  ...
  // always exists.
  if (load_bundle_paint_end == 0 ||
      (!enable_js_runtime_ && load_background_end == 0)) {
    return nullptr;
  }
   ...
   * - LoadBundleEntry.loadBundleStart tti = max(LoadBundleEntry.paintEnd,
   * LoadBundleEntry.loadBackgroundEnd) -
   * InitContainerEntry.prepareTemplateStart totalTti =
   * max(LoadBundleEntry.paintEnd, LoadBundleEntry.loadBackgroundEnd) -
...
std::unique_ptr<lynx::pub::Value> TimingInfoNg::GetMetricFmpEntry(
  ...
  if (pipeline_id.empty()) {
    ...
    if (!load_bundle_timing_map_.Empty()) {
      load_bundle_start =
          load_bundle_timing_map_.GetTimestamp(kLoadBundleStart).value_or(0);
    }
  }
  if (load_bundle_paint_end == 0) {
    return nullptr;
   ...
   * InitContainerEntry.prepareTemplateStart totalActualFmp =
   * PipelineEntry.paintEnd - InitContainerEntry.openTime
   */
  if (metrics_.find(kLynxActualFMP) == metrics_.end() ||
      load_bundle_start == 0) {
    has_update_metrics |=
        UpdateMetrics(kLynxActualFMP, kLoadBundleStart, kPaintEnd,
                      load_bundle_start, load_bundle_paint_end);
  }
