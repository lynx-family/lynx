...
std::unique_ptr<lynx::pub::Value> TimingInfoNg::GetMetricFcpEntry(
  ...
  auto pushMetric = [&](const std::string& name, const std::string& start_name,
  ...
  if (pipeline_id.empty()) {
    ...
    if (current_key == kPrepareTemplateStart &&
      ...
      pushMetric(kFCP, kPrepareTemplateStart, kPaintEnd,
    ...
    if (current_key == kOpenTime &&
      ...
      pushMetric(kTotalFCP, kOpenTime, kPaintEnd,
    ...
    pushMetric(kLynxFCP, kLoadBundleStart, kPaintEnd, *load_bundle_start,
               *paint_end);
    ...
    if (open_time.has_value()) {
      pushMetric(kTotalFCP, kOpenTime, kPaintEnd, *open_time, *paint_end);
    }
  }
...
std::unique_ptr<lynx::pub::Value> TimingInfoNg::GetMetricTtiEntry(
  ...
  auto pushMetric = [&](const std::string& name, const std::string& start_name,
                        const std::string& end_name_first,
                        const std::string& end_name_second, uint64_t start_time,
                        uint64_t end_time_first, uint64_t end_time_second) {
    auto duration = std::max(end_time_first, end_time_second) - start_time;
    auto metric_map = value_factory_->CreateMap();
    metric_map->PushStringToMap(kName, name);
    metric_map->PushStringToMap(kStartTimestampName, start_name);
    metric_map->PushDoubleToMap(kStartTimestamp, ConvertUsToDouble(start_time));
    if (end_time_first > end_time_second) {
      metric_map->PushStringToMap(kEndTimestampName, end_name_first);
      metric_map->PushDoubleToMap(kEndTimestamp,
                                  ConvertUsToDouble(end_time_first));
    } else {
      metric_map->PushStringToMap(kEndTimestampName, end_name_second);
      metric_map->PushDoubleToMap(kEndTimestamp,
                                  ConvertUsToDouble(end_time_second));
    }
    metric_map->PushDoubleToMap(kDuration, ConvertUsToDouble(duration));
    auto result = metrics_.emplace(name, std::move(metric_map));
    if (result.second) {
  ...
  if (pipeline_id.empty()) {
    ...
    if (current_key == kPrepareTemplateStart &&
      ...
      pushMetric(kTTI, kPrepareTemplateStart, kPaintEnd, kLoadBackgroundEnd,
                 *init_timing_info_.GetTimestamp(kPrepareTemplateStart),
                 load_bundle_paint_end, load_background_end_val);
    }
    if (current_key == kOpenTime &&
        metrics_.find(kTotalTTI) == metrics_.end()) {
      pushMetric(kTotalTTI, kOpenTime, kPaintEnd, kLoadBackgroundEnd,
                 *init_timing_info_.GetTimestamp(kOpenTime),
                 load_bundle_paint_end, load_background_end_val);
    }
  } else {
    ...
    pushMetric(kLynxTTI, kLoadBundleStart, kPaintEnd, kLoadBackgroundEnd,
               *load_bundle_start, *paint_end, load_background_end_val);
    ...
    if (prepare_template_start.has_value()) {
      pushMetric(kTTI, kPrepareTemplateStart, kPaintEnd, kLoadBackgroundEnd,
                 *prepare_template_start, *paint_end, load_background_end_val);
    }
    auto open_time = init_timing_info_.GetTimestamp(kOpenTime);
    if (open_time.has_value()) {
      pushMetric(kTotalTTI, kOpenTime, kPaintEnd, kLoadBackgroundEnd,
                 *open_time, *paint_end, load_background_end_val);
    }
  }
...
std::unique_ptr<lynx::pub::Value> TimingInfoNg::GetMetricFmpEntry(
  ...
  if (pipeline_id.empty()) {
    ...
    if (current_key == kPrepareTemplateStart &&
      ...
      pushMetric(kActualFMP, kPrepareTemplateStart, kPaintEnd,
                 *init_timing_info_.GetTimestamp(kPrepareTemplateStart),
                 paint_end);
    }
    if (current_key == kOpenTime &&
        metrics_.find(kTotalActualFMP) == metrics_.end()) {
      pushMetric(kTotalActualFMP, kOpenTime, kPaintEnd,
    ...
    pushMetric(kLynxActualFMP, kLoadBundleStart, kPaintEnd, *load_bundle_start,
               *paint_end);
    ...
    auto open_time = init_timing_info_.GetTimestamp(kOpenTime);
    if (open_time.has_value()) {
      pushMetric(kTotalActualFMP, kOpenTime, kPaintEnd, *open_time, *paint_end);
    }
  }
