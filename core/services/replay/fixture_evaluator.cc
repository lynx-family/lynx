...
std::string ValueToJson(Runtime& runtime, const Value& value,
  ...
  if (!json.has_value() || !json.value().isString()) {
    return fallback;
  }
  return json.value().getString(runtime).utf8(runtime);
}
              ...
              [&result, &state](
                ...
                if (args[1].isObject()) {
                  ...
                  if (function.has_value()) {
                    ...
                    if (!function.value()
                             .call(current_runtime, nullptr, 0)
                             .has_value()) {
                      state.callback_failed = true;
                      return base::unexpected(BUILD_JSI_NATIVE_EXCEPTION(
                          "Fixture after callback evaluation failed"));
                    }
                    for (size_t i = action_count; i < result.actions.size();
                         ++i) {
