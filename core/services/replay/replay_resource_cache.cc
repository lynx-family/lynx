...
void ReplayResourceCache::AddExternalScript(const std::string& url,
  ...
  if (!path.empty() && path != url) {
    external_script_aliases_[path] = external_scripts_[url];
  }
}
...
void ReplayResourceCache::Clear() {
  ...
  external_script_aliases_.clear();
}

bool ReplayResourceCache::IsEmpty() const {
  std::lock_guard<std::mutex> lock(mutex_);
  return external_scripts_.empty() && external_script_aliases_.empty();
}
...
bool ReplayResourceCache::ResolveResource(
  ...
  std::string path = NormalizePath(request.url);
  if (!path.empty() && LookupByUrl(external_script_aliases_, path, &out)) {
    response->data = std::move(out);
    return true;
  }
  return false;
