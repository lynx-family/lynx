...
TEST_F(ReplayResourceCacheTest, LoadRecordScriptsPreservesEmptyScript) {
  rapidjson::Document dom;
  dom.Parse(R"({"Scripts":{"http://example.com/empty.js":"encoded"}})");

  LoadRecordScriptsIntoReplayResourceCache(
      &cache_, dom, [](const char* value) { return std::string(value); },
      [](const std::vector<uint8_t>&) -> std::optional<std::vector<uint8_t>> {
        return std::vector<uint8_t>{};
      });

  pub::LynxResourceRequest request;
  request.url = "http://example.com/empty.js";
  request.type = pub::LynxResourceType::kExternalJs;
