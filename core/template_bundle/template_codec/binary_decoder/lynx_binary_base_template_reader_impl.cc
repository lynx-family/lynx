...
template <typename T>
void ReinterpretHeaderInfoValue(T& tgt, const HeaderExtInfoByteArray& src) {
  if (src.size() == sizeof(T)) {
    tgt = *reinterpret_cast<const T*>(src.data());
  }
}

void ReinterpretHeaderInfoValue(std::string& tgt,
                                const HeaderExtInfoByteArray& src) {
...
bool LynxBinaryBaseTemplateReader::DecodeHeader() {
  ...
  ERROR_UNLESS(DecodeMagicWord(magic_word));
  ...
  if (lepus_version > MIN_SUPPORTED_VERSION) {
    ...
    // currently android_version == ios_version, is the engine version in
    // project config
    if (ios_version != "unknown") {
      if (!CheckLynxVersion(ios_version)) {
        error_message_ =
            "version check miss, should "
            "(lynx sdk version >= App Bundle's engine version >= min "
            "supported version), but engine version: ";
        error_message_ += ios_version;
        error_message_ +=
...
#define CONVERT_FIXED_LENGTH_FIELD(type, field, id) \
  ReinterpretHeaderInfoValue(compile_options_.field, header_info_map_[id])
...
bool LynxBinaryBaseTemplateReader::DecodeHeaderInfoField() {
  ...
  ERROR_UNLESS(stream_->ReadData(reinterpret_cast<uint8_t*>(&header_info_field),
                                 (int)size_to_read));
...
bool LynxBinaryBaseTemplateReader::DecodeFlexibleTemplateBody() {
  ...
  const auto& order = compile_options_.enable_fiber_arch_
                          ? GetFlexibleTemplateSectionOrder()
                          : kSectionOrder;
...
bool LynxBinaryBaseTemplateReader::DecodeSpecificSection(
  ...
  switch (section) {
    ...
    case BinarySection::CONFIG: {
      ERROR_UNLESS(DecodeConfigSection());
      break;
    }
