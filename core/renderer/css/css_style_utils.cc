...
tasm::StyleMap ResolveKeyframeStyleWithCustomProperties(
  ...
  auto resolved_custom_properties = *custom_properties;
  ...
  for (const auto& [id, value] : source) {
    ...
    resolved_styles.insert_or_assign(
        id, it != parsed_styles.end() ? it->second : value);
  }
...
void CSSStyleUtils::UpdateCSSKeyframes(
  ...
  if (!keyframes.IsTable()) {
    ...
    for (size_t i = 0; i < ary->size(); ++i) {
      ...
      keyframes_map[name]->GetKeyframesCustomPropertyContent().insert(
          std::make_pair(i * interval, ProcessCSSCustomPropertyAttrsMap(
                                           ary->get(i), configs)));
    }
    return;
  }
  ...
  for (auto& iter : *table) {
    ...
    keyframes_map[name]->GetKeyframesCustomPropertyContent().insert(
        std::make_pair(
            interval,
            starlight::CSSStyleUtils::ProcessCSSCustomPropertyAttrsMap(
                iter.second, configs)));
  }
}
