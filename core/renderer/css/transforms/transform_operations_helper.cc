...
constexpr uint32_t PatternBit(tasm::CSSValuePattern pattern) {
  return 1u << static_cast<uint32_t>(pattern);
}

constexpr uint32_t kCalcUnitDependencies =
    PatternBit(tasm::CSSValuePattern::CALC) |
    PatternBit(tasm::CSSValuePattern::PERCENT) |
    PatternBit(tasm::CSSValuePattern::RPX) |
    PatternBit(tasm::CSSValuePattern::EM) |
    PatternBit(tasm::CSSValuePattern::REM) |
    PatternBit(tasm::CSSValuePattern::VW) |
    PatternBit(tasm::CSSValuePattern::VH);

void AddUnitDependency(const starlight::NLength& length,
                       tasm::CSSValuePattern pattern,
                       ResolvedTransformOperations& result) {
  if (length.IsCalc()) {
    // NLength keeps the resolved fixed and percentage components, but not the
    // individual units inside calc(). Conservatively re-resolve calc values
    // for any dynamic unit update instead of parsing the CSS expression again.
    result.unit_dependencies |= kCalcUnitDependencies;
  } else if (pattern == tasm::CSSValuePattern::SP) {
    // Font-scale updates are delivered to animations as EM and REM updates.
    result.unit_dependencies |= PatternBit(tasm::CSSValuePattern::EM) |
                                PatternBit(tasm::CSSValuePattern::REM);
  } else if (pattern < tasm::CSSValuePattern::COUNT) {
    result.unit_dependencies |= PatternBit(pattern);
  }
}

bool DependsOnElementSize(const starlight::NLength& length) {
  return length.IsCalc() && length.ContainsPercentage();
}

void CollectTransformDependencies(
    const base::Vector<starlight::TransformRawData>& transform_raw_data,
    ResolvedTransformOperations& result) {
  for (const auto& item : transform_raw_data) {
    AddUnitDependency(item.p0, item.unit_type0, result);
    AddUnitDependency(item.p1, item.unit_type1, result);
    AddUnitDependency(item.p2, item.unit_type2, result);

    switch (item.type) {
      case starlight::TransformType::kTranslate:
        result.depends_on_element_size |=
            DependsOnElementSize(item.p0) || DependsOnElementSize(item.p1);
        break;
      case starlight::TransformType::kTranslateX:
      case starlight::TransformType::kTranslateY:
        result.depends_on_element_size |= DependsOnElementSize(item.p0);
        break;
...
std::optional<ResolvedTransformOperations> ResolveTransformOperations(
  ...
  starlight::CSSStyleUtils::ComputeTransform(raw_data, false, transform_data,
                                             measure_context, parser_configs);
  ResolvedTransformOperations result;
  CollectTransformDependencies(*transform_data, result);
  result.operations = ConvertToGfxTransformOperations(
      *transform_data, reference_width, reference_height);
  return result;
}
