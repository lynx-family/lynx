...
- `pipeline_context.*`: owns one pipeline run's options, version, lifecycle state, and observer notification data.
- `pipeline_context_manager.*`: owned by `TemplateAssembler`; creates, stores, looks up, and removes contexts, and manages TemplateAssembler-level observer registration.
...
- When observer callbacks can re-enter, preserve snapshot semantics for both the observer list and the callback data for the current notification.
...
- Missing or duplicate lifecycle callbacks usually point to manager/context observer attach or deduplication drift.
