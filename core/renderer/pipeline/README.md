...
- `PipelineContext`
  - Holds one pipeline run's options, version, lifecycle, hash, observers.
  - Owns lifecycle transition notification.
- `PipelineContextManager`
  - Owned by `TemplateAssembler`.
  - Creates, stores, removes contexts by version.
  - Owns instance-level observer registry.
- `PipelineScope`
...
- New contexts auto-attach all registered observers.
...
- Re-entrant callback safety:
  - `PipelineContext::NotifyLifecycleChanged` iterates over a copied observer snapshot.
  - Removing observers inside callbacks does not invalidate current iteration.
- Expired weak observer cleanup:
  - Invalid weak pointers are erased during add/remove/notify paths.
