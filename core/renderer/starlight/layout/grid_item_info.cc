...
void GridItemInfo::InitSpanInfo(Dimension dimension, int32_t explicit_end,
  ...
  // If the placement for a grid item contains two lines, and the start line is
  // further end-ward than the end line, swap the two lines.
  if (start > end && is_start_definite && is_end_definite) {
    std::swap(start, end);
  }

  // A definite value for any two of Start, End, and Span in a given dimension
  // implies a definite value for the third.
