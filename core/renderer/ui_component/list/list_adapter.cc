        ...
        data_source, [this, list_container_info](const lepus::Value& key,
          ...
          if (key_str == list::kDataSourceEstimatedHeightPx) {
            ...
            list_container_info->SetValue(
                BASE_STATIC_STRING(list::kDataSourceStickyStart), value);
          } else if (key_str == list::kDataSourceStickyBottom) {
            adapter_helper_->UpdateStickyBottoms(value);
            list_container_info->SetValue(
                BASE_STATIC_STRING(list::kDataSourceStickyEnd), value);
...
void ListAdapter::UpdateAnchorRefItem(
  ...
  bool optimized_search_anchor = list_container_->optimized_search_anchor();
  std::unordered_map<ItemHolder*, ItemHolder*> tmp_anchor_ref_map;
  if (has_valid_diff && optimized_search_anchor &&
      !on_screen_children.empty() && !children.empty()) {
    list_children_helper->ForEachChild(
        on_screen_children, [this, list_children_helper, &children,
                             &tmp_anchor_ref_map](ItemHolder* on_screen_child) {
