...
void TextLayoutTextra::BuildParagraphRecursively(Element* element,
  ...
  if (element->is_text()) {
    ...
    if (!element_content.empty()) {
      ...
      text_position_ += static_cast<int32_t>(
          base::SizeOfUtf8(decoded_content.c_str(), decoded_content.length()));
    }
...
void TextLayoutTextra::ProcessChildStyleAndProps(Element* element,
  ...
  if (!building_inline_truncation_ && IsInlineTruncationElement(child)) {
    // TODO: Implement separate truncation indexing and hit testing later.
...
void TextLayoutTextra::RecordTextEventTargetRange(Element* element,
  ...
  if (!element->EnableFragmentLayerRender() || start >= end ||
      !element->HasEventListener("tap")) {
    return;
  }
  building_event_target_ranges_.push_back({element->impl_id(), start, end});
}
...
void TextLayoutTextra::BuildInlineTruncation(Element* element,
  ...
  auto* previous_truncation = truncation_;
  truncation_ = element;
  ...
  truncation_ = previous_truncation;
}
