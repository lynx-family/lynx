...
void PlatformRendererContext::EnsureAndroidViewCreated(int32_t id) {
  // Skip registry lookup and reference counting when nothing is deferred.
  if (!HasPendingPreparations() || destroyed_) {
    return;
  }
...
void PlatformRendererContext::RegisterPlatformRenderer(
  ...
  if (!destroyed_) {
    ...
    if (renderer->HasPendingPreparation()) {
