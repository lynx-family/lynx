...
PlatformRendererContext::GetTextEventTargetRegions(
  ...
  const auto range_data_size = static_cast<jsize>(range_data.size());
  base::android::ScopedLocalJavaRef<jintArray> ranges(
      env, env->NewIntArray(range_data_size));
  if (ranges.IsNull()) {
    return regions;
  }
  if (!range_data.empty()) {
    env->SetIntArrayRegion(ranges.Get(), 0, range_data_size, range_data.data());
  }
