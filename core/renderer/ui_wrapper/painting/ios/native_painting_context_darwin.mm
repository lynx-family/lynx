...
void NativePaintingCtxDarwin::CreatePlatformRenderer(int id, PlatformRendererType type,
  ...
  Enqueue([ref = platform_ref_, id, type, init_data]() {
    std::static_pointer_cast<NativePaintingCtxDarwinRef>(ref)->CreatePlatformRenderer(id, type,
                                                                                      init_data);
  });
}
...
void NativePaintingCtxDarwin::CreatePlatformExtendedRenderer(
  ...
  Enqueue([ref = platform_ref_, id, tag_name, init_data]() {
    std::static_pointer_cast<NativePaintingCtxDarwinRef>(ref)->CreatePlatformExtendedRenderer(
        id, tag_name, init_data);
  });
}
...
void NativePaintingCtxDarwin::UpdateDisplayList(int id, DisplayList display_list) {
  Enqueue([ref = platform_ref_, id, dl = std::move(display_list)]() mutable {
    std::static_pointer_cast<NativePaintingCtxDarwinRef>(ref)->UpdateDisplayList(id, std::move(dl));
  });
}
