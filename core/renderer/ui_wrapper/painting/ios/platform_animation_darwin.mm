    ...
    if (playback) {
      ...
      platformPlayback.claimEvent = ^BOOL(NSString* event) {
        ...
        if ([event isEqualToString:@"animationstart"]) {
          ...
          return playback->ClaimEvent(executor, Phase::kActive, active);
        }
        return playback->ClaimEvent(
            executor, [event isEqualToString:@"animationend"] ? Phase::kAfter : Phase::kCanceled);
      };
    }
