...
- (void)testPausedHandoffUsesLogicalTimeAndDetachDoesNotCancelOrRestoreStyle {
  ...
  using Phase = gfx::AnimationPlaybackState::Phase;
  auto playback = std::make_shared<gfx::AnimationPlaybackState>(fml::TimeDelta::FromSeconds(3),
                                                                true, Phase::kActive);
  ...
  XCTAssertTrue(playback->phase() == Phase::kActive);
  [events stopMocking];
}
...
- (void)testHandoffAfterLogicalCompletionUpdatesFillWithoutEvents {
  self.ui = [[LynxUI alloc]
      initWithView:[[ManuallyDrivenAnimationView alloc] initWithFrame:CGRectMake(0, 0, 100, 100)]];
  using Phase = gfx::AnimationPlaybackState::Phase;
  auto playback = std::make_shared<gfx::AnimationPlaybackState>(fml::TimeDelta::FromSeconds(12),
                                                                true, Phase::kAfter);
  auto command = OpacityCommand();
  command.playback = playback;
...
- (void)testRoutedRemovalStillSendsCancelOnce {
  ...
  OCMVerifyAll(events);
  XCTAssertTrue(playback->phase() == Phase::kCanceled);
  [events stopMocking];
}
