...
fml::RefPtr<PropBundle> PreparePropBundle(
  ...
  if (!props || !queue->IsEngineAsync()) {
    return props;
  }
  static_cast<PropBundleHarmony*>(props.get())->SnapshotValues();
  return props;
}
...
void PaintingContextHarmony::SetFrameAppBundle(
  ...
  Enqueue([platform_ref = platform_ref_, id, bundle]() mutable {
    auto harmony_ref =
        std::static_pointer_cast<PaintingContextHarmonyRef>(platform_ref);
    harmony_ref->SetFrameAppBundle(id, std::move(bundle));
  });
}
