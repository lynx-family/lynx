...
PlatformRendererHarmony::PlatformRendererHarmony(
  ...
  is_platform_extended_renderer_ =
      ShouldCreatePlatformExtendedRenderer(init_config);
...
bool PlatformRendererHarmony::InitializeUIOwnerRenderer(
  ...
  auto* props = init_data == nullptr
                    ? &empty_props
                    : static_cast<PropBundleHarmony*>(init_data.get());
  ui_owner->CreateUI(GetId(), tag_name.str(), props, GetId(), context_);
...
bool PlatformRendererHarmony::InitializeFragmentLayerRenderer() {
  ...
  harmony::UIBase* host = type_ == PlatformRendererType::kPage
                              ? ui_owner->CreateFragmentLayerRootHost(GetId())
                              : ui_owner->CreateFragmentLayerHost(GetId());
  return AttachRendererToUI(host);
}
...
void PlatformRendererHarmony::OnUpdateAttributes(
  ...
  if (ui_owner != nullptr) {
    ui_owner->UpdateUI(GetId(),
                       static_cast<PropBundleHarmony*>(attributes.get()));
  }
}
...
void PlatformRendererHarmony::OnAddChild(PlatformRenderer* child, int index,
  ...
  if (child_host == nullptr ||
      (!should_update_ui_owner && harmony_child->IsOverlay())) {
    return;
  }
  auto* ui_owner = context_ != nullptr ? context_->GetUIOwner() : nullptr;
...
void PlatformRendererHarmony::OnRemoveFromParent(bool should_update_ui_owner) {
  ...
  if (host == nullptr || parent_host == nullptr ||
      (!should_update_ui_owner && IsOverlay())) {
    return;
  }
  auto* ui_owner = context_ != nullptr ? context_->GetUIOwner() : nullptr;
