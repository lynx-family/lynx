...
base::flex_optional<base::geometry::FloatPoint> GetPageCoordinateOffset(
    Element* element, PageCoordinateSpace space) {
  if (element == nullptr || element->element_manager() == nullptr) {
    return {};
  }
  const auto& snapshot =
      element->element_manager()->GetPageCoordinateSnapshot();
  const bool has_offset = space == PageCoordinateSpace::kWindow
                              ? snapshot.has_window_offset
                              : snapshot.has_screen_offset;
  const float x = space == PageCoordinateSpace::kWindow ? snapshot.window_x
                                                        : snapshot.screen_x;
  const float y = space == PageCoordinateSpace::kWindow ? snapshot.window_y
                                                        : snapshot.screen_y;
  if (!has_offset || !std::isfinite(x) || !std::isfinite(y)) {
    return {};
  }
  return base::geometry::FloatPoint(x, y);
}
