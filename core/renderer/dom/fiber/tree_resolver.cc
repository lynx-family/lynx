...
void TreeResolver::NotifyNodeInserted(Element* insertion_point, Element* node) {
  ...
  for (const auto& child : node->children()) {
    NotifyNodeInserted(insertion_point, static_cast<Element*>(child.get()));
  }
}
...
void TreeResolver::NotifyNodeRemoved(Element* insertion_point, Element* node) {
  ...
  for (const auto& child : node->children()) {
    if (static_cast<Element*>(child.get())->is_raw_text()) {
      continue;
    }
    NotifyNodeRemoved(insertion_point, static_cast<Element*>(child.get()));
  }
}

Element* TreeResolver::FindFirstChildOrSiblingAsRefNode(Element* ref) {
  return ref ? static_cast<Element*>(ref->FindFirstNonWrapperChildOrSibling())
             : nullptr;
}
...
void TreeResolver::AttachChildToTargetParentForWrapper(Element* parent,
  ...
  while (!ref_node && temp_parent && temp_parent->is_wrapper()) {
    ref_node = static_cast<Element*>(temp_parent->next_render_sibling());
    ...
    if (ref_node && ref_node->EnableLayoutInElementMode()) {
      ...
      ref_node =
          static_cast<Element*>(ref_node->FindFirstNonVirtualRenderSibling());
    }
    ...
    temp_parent = static_cast<Element*>(temp_parent->render_parent());
  }

  Element* real_parent =
      static_cast<Element*>(parent->FindFirstNonWrapperRenderAncestor());
...
std::pair<Element*, int> TreeResolver::FindParentForChildForWrapper(
  ...
  while (node->is_wrapper()) {
    auto* p = static_cast<Element*>(node->render_parent());
    if (!p) {
      return {nullptr, -1};
    }
...
int TreeResolver::GetLayoutIndexForChildForWrapper(Element* parent,
  ...
  for (const auto& it : parent->children()) {
    auto* current = static_cast<Element*>(it.get());
    if (child == current) {
      found = true;
      break;
    }
...
size_t TreeResolver::GetLayoutChildrenCountForWrapper(Element* node) {
  ...
  for (const auto& current : node->children()) {
    auto* current_fiber = static_cast<Element*>(current.get());
...
void TreeResolver::AttachChildToTargetContainerRecursive(Element* parent,
  ...
  auto* grand = static_cast<Element*>(child->first_render_child());
  while (grand) {
    AttachChildToTargetContainerRecursive(parent, grand, ref);
    grand = static_cast<Element*>(grand->next_render_sibling());
  }
}

Element* TreeResolver::FindTheRealParent(Element* node) {
  return node ? static_cast<Element*>(node->FindFirstNonWrapperRenderAncestor())
              : nullptr;
}
...
void TreeResolver::RemoveChildRecursively(Element* parent, Element* child) {
  if (!child->is_wrapper()) {
    ...
    while (grand) {
      RemoveChildRecursively(parent, static_cast<Element*>(grand));
...
fml::RefPtr<lepus::Dictionary> TreeResolver::GetTemplateParts(
  ...
  for (const auto& c : template_element->children()) {
    GetPartsRecursively(fml::static_ref_ptr_cast<Element>(c), parts_map);
  }
  return parts_map;
}
...
fml::RefPtr<Element> TreeResolver::CloneElements(
  ...
  fml::RefPtr<Element> res = fml::static_ref_ptr_cast<Element>(
  ...
  for (const auto& c : root->children()) {
    auto* child = static_cast<Element*>(c.get());
    if (cloning_depth == CloningDepth::kTemplateScope &&
        child->IsTemplateElement()) {
      continue;
    }
    res->InsertNode(CloneElements(fml::static_ref_ptr_cast<Element>(c),
...
fml::RefPtr<Element> TreeResolver::CloneElementRecursively(
  ...
  for (const auto& c : element->children()) {
    res->InsertNode(CloneElementRecursively(static_cast<Element*>(c.get()),
...
void TreeResolver::AttachRootToElementManager(
    fml::RefPtr<Element>& root, ElementManager* element_manager,
    const std::shared_ptr<CSSStyleSheetManager>& style_manager,
...
void TreeResolver::AttachToElementManagerRecursively(
  ...
  for (auto& child : element.children()) {
    AttachToElementManagerRecursively(*static_cast<Element*>(child.get()),
...
std::list<ParallelFlushReturn> TreeResolver::StyleTrees(
  ...
  while (!discovered.empty()) {
    ...
    if (target->ShouldResolveStyle()) {
      ...
      DCHECK((target->dirty() & ~Element::kDirtyTree) == 0);
    }
    ...
    for (auto& child : target->children()) {
      ...
      if (target->NeedPropagateInheritedDirtyFlag(true)) {
        fiber_child->MarkDirtyLite(Element::kDirtyPropagateInherited);
      }
