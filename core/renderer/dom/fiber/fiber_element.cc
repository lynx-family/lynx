...
style::DynamicStyleObjectRef CreateDynamicStyleObjectRef(StyleMap style_map) {
  if (style_map.empty()) {
    return nullptr;
  }
...
void FiberElement::AddDynamicSimpleStyles(tasm::StyleMap &&new_styles) {
  ...
  if (!dynamic_simple_object_ && parsed_dynamic_styles_map_.has_value() &&
    ...
    dynamic_simple_object_ =
        CreateDynamicStyleObjectRef(*parsed_dynamic_styles_map_);
  }

  if (!dynamic_simple_object_) {
    dynamic_simple_object_ = CreateDynamicStyleObjectRef(std::move(new_styles));
    MarkDirty(kDirtyForceUpdate | kDirtyDynamicStyleObjects);
    return;
  }
...
void FiberElement::AddDynamicSimpleStyleKV(tasm::CSSPropertyID id,
  ...
  if (!dynamic_simple_object_) {
    ...
    dynamic_simple_object_ =
        CreateDynamicStyleObjectRef(std::move(dynamic_styles));
    MarkDirty(kDirtyForceUpdate | kDirtyDynamicStyleObjects);
    return;
  }
