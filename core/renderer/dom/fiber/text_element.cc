...
void TextElement::OnLayoutNodeCreated() {
  if (!is_inline_element()) {
    SetMeasureFunc(
...
// static
void TextElement::BuildAttributedStringProps(int pos_start, int pos_end,
                                             PropArray* props) {
  if (text_props_) {
    ...
    if (text_props_->text_align) {
      ...
      props->AddProp((int)*text_props_->text_align);
    }
    ...
    if (text_props_->vertical_align_type &&
      ...
      props->AddProp((int)*text_props_->vertical_align_type);
