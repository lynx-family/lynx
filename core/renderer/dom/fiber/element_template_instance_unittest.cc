...
TEST_P(ElementTemplateInstanceTest,
       AttributeSlotUpdateSkipsUnrelatedTemplateAttributes) {
  auto entry = std::make_shared<TemplateEntry>();
  entry->SetName(DEFAULT_ENTRY_NAME);
  tasm->template_entries_[DEFAULT_ENTRY_NAME] = entry;
  ...
  auto template_info = std::make_shared<ElementTemplateInfo>();
  template_info->exist_ = true;
  template_info->key_ = "targeted_attribute_template";
  auto root_info = ElementInfo();
  root_info.tag_enum_ = ElementBuiltInTagEnum::ELEMENT_VIEW;
  root_info.attributes_ =
      std::make_shared<const TemplateAttributes>(TemplateAttributes{
          Attribute{ATTRIBUTE_BINDING_TYPE_STATIC, base::String("class"),
                    lepus::Value("stable"), 0},
          Attribute{ATTRIBUTE_BINDING_TYPE_STATIC, base::String("bindtap"),
                    lepus::Value("onTap"), 0},
          Attribute{ATTRIBUTE_BINDING_TYPE_DYNAMIC, base::String("first"),
                    lepus::Value(), 0},
          Attribute{ATTRIBUTE_BINDING_TYPE_DYNAMIC, base::String("second"),
                    lepus::Value(), 1},
      });
  template_info->elements_.emplace_back(std::move(root_info));
  entry->template_bundle_
      .element_template_infos_["targeted_attribute_template"] = template_info;

  auto slots = lepus::CArray::Create();
  slots->emplace_back(lepus::Value("before"));
  slots->emplace_back(lepus::Value("untouched"));
  auto instance = fml::AdoptRef<ElementTemplateInstance>(
      new ElementTemplateInstance(manager));
  instance->SetTASM(tasm.get());
  instance->SetBundleUrl(base::String(DEFAULT_ENTRY_NAME));
  instance->SetTemplateKey(base::String("targeted_attribute_template"));
  instance->SetAttributeSlots(lepus::Value(slots));

  auto resolved = instance->GetRoot();
  ASSERT_NE(resolved, nullptr);
  ASSERT_EQ(resolved->event_map().count("tap"), 1u);
  resolved->updated_attr_map_.clear();
  resolved->ResetAllDirtyBits();

  instance->SetAttributeSlot(0, lepus::Value("after"));

  EXPECT_FALSE(resolved->StyleDirty());
  EXPECT_EQ(resolved->dirty() & Element::kDirtyEvent, 0u);
  EXPECT_TRUE(resolved->AttrDirty());
  ASSERT_EQ(resolved->updated_attr_map_.size(), 1u);
  EXPECT_EQ(resolved->updated_attr_map_.at("first").StdString(), "after");
  EXPECT_EQ(resolved->updated_attr_map_.count("second"), 0u);
  EXPECT_EQ(resolved->data_model_->attributes().at("second").StdString(),
            "untouched");
  EXPECT_EQ(resolved->event_map().count("tap"), 1u);
}

TEST_P(ElementTemplateInstanceTest,
       AttributeSlotUpdateRetainsFullSpreadPrecedence) {
  auto entry = std::make_shared<TemplateEntry>();
  entry->SetName(DEFAULT_ENTRY_NAME);
  tasm->template_entries_[DEFAULT_ENTRY_NAME] = entry;
  ...
  auto template_info = std::make_shared<ElementTemplateInfo>();
  template_info->exist_ = true;
  template_info->key_ = "spread_precedence_template";
  auto root_info = ElementInfo();
  root_info.tag_enum_ = ElementBuiltInTagEnum::ELEMENT_VIEW;
  root_info.attributes_ =
      std::make_shared<const TemplateAttributes>(TemplateAttributes{
          Attribute{ATTRIBUTE_BINDING_TYPE_DYNAMIC, base::String("order"),
                    lepus::Value(), 0},
          Attribute{ATTRIBUTE_BINDING_TYPE_SPREAD, base::String("spread"),
                    lepus::Value(), 1},
          Attribute{ATTRIBUTE_BINDING_TYPE_STATIC, base::String("order"),
                    lepus::Value("static"), 0},
      });
  template_info->elements_.emplace_back(std::move(root_info));
  entry->template_bundle_
      .element_template_infos_["spread_precedence_template"] = template_info;

  auto spread = lepus::Dictionary::Create();
  spread->SetValue(base::String("order"), lepus::Value("spread"));
  auto slots = lepus::CArray::Create();
  slots->emplace_back(lepus::Value("before"));
  slots->emplace_back(lepus::Value(spread));
  auto instance = fml::AdoptRef<ElementTemplateInstance>(
      new ElementTemplateInstance(manager));
  instance->SetTASM(tasm.get());
  instance->SetBundleUrl(base::String(DEFAULT_ENTRY_NAME));
  instance->SetTemplateKey(base::String("spread_precedence_template"));
  instance->SetAttributeSlots(lepus::Value(slots));
  ...
  instance->SetAttributeSlot(0, lepus::Value("changed"));

  ASSERT_EQ(resolved->updated_attr_map_.count("order"), 1u);
  EXPECT_EQ(resolved->updated_attr_map_.at("order").StdString(), "static");
}
