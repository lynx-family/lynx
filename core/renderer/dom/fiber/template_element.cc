...
void InsertElementBefore(const fml::RefPtr<Element>& parent,
  ...
  auto index = parent->IndexOf(ref_node.get());
  if (index < 0) {
    return;
  }
...
void TemplateElement::InsertInitialElementSlotChild(
  ...
  InsertElementBefore(mount_point.parent_, child, mount_point.ref_node_);
}
...
void TemplateElement::MountElementSlotChild(
  ...
  InsertElementBefore(mount_point.parent_, mounted_child, target_ref_node);
}
