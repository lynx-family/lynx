...
napi_finalize GetTemplateBundleFinalizeCallback() {
...
napi_value LynxTemplateBundleHarmony::CreateFromNative(
  ...
  auto* bundle_harmony = new LynxTemplateBundleHarmony();
  bundle_harmony->SetBundle(bundle);
  status = napi_wrap(env, js_bundle, static_cast<void*>(bundle_harmony),
                     GetTemplateBundleFinalizeCallback(), nullptr, nullptr);
  if (status != napi_ok) {
    LOGE("wrap native TemplateBundle object failed " << status);
    delete bundle_harmony;
    return nullptr;
  }
...
napi_value LynxTemplateBundleHarmony::New(napi_env env,
  ...
  status = napi_wrap(env, js_this, static_cast<void*>(bundle),
                     GetTemplateBundleFinalizeCallback(), nullptr, nullptr);
  if (status != napi_ok) {
    delete bundle;
