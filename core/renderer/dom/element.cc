...
std::vector<float> Element::GetRectToWindow() {
  return catalyzer_->GetRectToWindow(this);
}
...
void Element::SetCallableEventHandler(const base::String& name,
                                      const base::String& type,
                                      const lepus::Value& callback,
                                      runtime::MTSRuntime* context) {
  TRACE_EVENT(LYNX_TRACE_CATEGORY, FIBER_ELEMENT_SET_LEPUS_EVENT_HANDLER);

  data_model_->SetCallableEvent(type, name, callback, context);
  MarkDirty(kDirtyEvent);
}
