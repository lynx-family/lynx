...
void CSSTransitionManager::SyncTransitionData(
  ...
  PlatformTransitionMap retained_platform_animations;
  ...
  // Retained transitions have been moved out. Cancel only the remaining
  // transitions whose properties are absent from the new configuration.
  ...
  // The member now owns retained transitions; canceled states are destroyed
  // with the local map after the swap.
  platform_transition_animations_.swap(retained_platform_animations);
}
...
void CSSTransitionManager::SetTransitionDataInternal(
  ...
  if (platform_animation != platform_transition_animations_.end()) {
    retained_platform_animations.insert_or_assign(
...
void CSSTransitionManager::TryToStopTransitionAnimator(
  ...
  CancelPlatformTransition(data->second.name);
}
...
void CSSTransitionManager::CancelPlatformTransition(const base::String& name) {
  auto platform_animation = platform_transition_animations_.find(name);
  if (platform_animation != platform_transition_animations_.end()) {
...
bool CSSTransitionManager::UpdateTransitionAnimator(
  ...
  auto existing_new_animator_animation =
      animations_map_.find(data->second.name);
  if (existing_new_animator_animation != animations_map_.end()) {
    ...
    if (play_handles_initial_frame) {
      existing_new_animator_animation->second->Destroy(false);
    } else {
      PrepareTransitionRemovalCleanup(existing_new_animator_animation->second);
    }
    animations_map_.erase(existing_new_animator_animation);
  }
  ...
  if (build_result.platform_animation.has_value()) {
    ...
    if (platform_animation != platform_transition_animations_.end()) {
      new_platform_animation.RetainIdentityFrom(platform_animation->second);
  ...
  CancelPlatformTransition(data->second.name);
  if (build_result.new_animator_animation == nullptr) {
    return false;
  }
  std::shared_ptr<Animation> animation =
      std::move(build_result.new_animator_animation);
