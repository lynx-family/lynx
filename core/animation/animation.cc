...
void Animation::DoFrame(fml::TimePoint& frame_time) {
  ...
  if (frame_time != fml::TimePoint::Min()) {
    if (Tick(frame_time)) {
      // Seeking a paused animation to its end must not resume it on the next
      // seek. Keep fill cleanup independent of the playback state.
      if (state_ != State::kPause) {
        Stop();
      }
      if (keyframe_effect_) {
...
KeyframeEffect::KeyframeSampleResult Animation::SampleAt(
  ...
  if (state_ == State::kPause && frame_time == GetAnimationDummyStartTime() &&
    ...
    // timing or advance timeline state.
    return result;
  }
  ...
  if (state_ == State::kPause) {
    ...
    if (frame_time == GetAnimationDummyStartTime() && has_last_sample_) {
      // SeekTo/SetPaused may have installed a newer hold point. Only use
      // sample history when pause timing has not been established yet.
      if (!was_paused_ || pause_time_ == fml::TimePoint::Min()) {
...
void Animation::ReplaceEffectFrom(Animation& replacement) {
  ...
  const auto event_phase = playback_->phase();
  if (old_phase == gfx::TimingPhase::BEFORE ||
      event_phase == gfx::AnimationPlaybackState::Phase::kBefore) {
    previous_run_state_ = gfx::KeyframeModel::STARTING;
  } else if (event_phase == gfx::AnimationPlaybackState::Phase::kAfter) {
...
void Animation::SendStartEvent() {
  event_phase_ = gfx::AnimationPlaybackState::Phase::kActive;
  if (playback_ && !playback_->ClaimEvent(executor_, event_phase_)) {
    return;
  }
...
void Animation::SendEndEvent() {
  event_phase_ = gfx::AnimationPlaybackState::Phase::kAfter;
  if (playback_ && !playback_->ClaimEvent(executor_, event_phase_)) {
    return;
  }
  CreateEventAndSend(is_transition_
...
void Animation::SendCancelEvent() {
  event_phase_ = gfx::AnimationPlaybackState::Phase::kCanceled;
  if (playback_ && !playback_->ClaimEvent(executor_, event_phase_)) {
    return;
  }
  CreateEventAndSend(is_transition_
