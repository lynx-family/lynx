...
TEST_F(AnimationTest, PausedSeekToEndAndBackThroughDoFrame) {
  for (auto fill : {starlight::AnimationFillModeType::kBoth,
                    starlight::AnimationFillModeType::kNone}) {
    auto a = InitTestAnimation();
    auto data = InitAnimationData(base::String("test_animation"), 1000, 0,
                                  starlight::TimingFunctionData(), 1, fill,
                                  starlight::AnimationDirectionType::kNormal,
                                  starlight::AnimationPlayStateType::kRunning);
    a->UpdateAnimationData(data);
    class RecordingDelegate : public animation::MockCSSKeyframeManager {
     public:
      explicit RecordingDelegate(Element* element)
          : MockCSSKeyframeManager(element) {}
      void UpdateFinalStyleMap(const StyleMap& value) override {
        styles = value;
      }
      StyleMap styles;
    } delegate(element_.get());
    ...
    time = time + fml::TimeDelta::FromMilliseconds(4000);
    a->SeekTo(fml::TimeDelta::FromMilliseconds(1000), time);
    time = time + fml::TimeDelta::FromMilliseconds(16);
    a->DoFrame(time);
    EXPECT_EQ(animation::Animation::State::kPause, a->GetState());
    EXPECT_EQ(fml::TimeDelta::FromMilliseconds(1000), a->GetCurrentTime());
    if (fill == starlight::AnimationFillModeType::kBoth) {
      EXPECT_NEAR(1.0, delegate.styles.at(kPropertyIDOpacity).AsNumber(),
                  0.001);
    } else {
      EXPECT_EQ("test_animation", delegate.GetClearEffectAnimationName());
    }

    time = time + fml::TimeDelta::FromMilliseconds(1000);
    a->SeekTo(fml::TimeDelta::FromMilliseconds(500), time);
    for (int i = 0; i < 2; ++i) {
      time = time + fml::TimeDelta::FromMilliseconds(16);
      a->DoFrame(time);
      EXPECT_EQ(animation::Animation::State::kPause, a->GetState());
      EXPECT_EQ(fml::TimeDelta::FromMilliseconds(500), a->GetCurrentTime());
      EXPECT_NEAR(0.5, delegate.styles.at(kPropertyIDOpacity).AsNumber(),
                  0.001);
    }
  }
}
