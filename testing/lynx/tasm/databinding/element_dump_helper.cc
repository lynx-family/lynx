...
lepus::Value ElementDumpHelper::DumpToSnapshot(Element* element,
  ...
  if (data_model) {
    ...
    static_cast<Element*>(element)->DumpStyle(computed_styles);
...
std::string ElementDumpHelper::DumpTree(PageProxy* proxy) {
  ...
  if (proxy->radon_page_) {
    ...
    dumped_virtual_tree =
        DumpFiberElementToJSON(dumped_document, static_cast<Element*>(page));
  } else {
    return std::string();
  }
  ...
  if (children_size > 0) {
    ...
    for (auto&& child : element->children()) {
      children_json.GetArray().PushBack(
          DumpFiberElementToJSON(doc, static_cast<Element*>(child.get())),
          allocator);
    }
