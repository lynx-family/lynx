...
private final class ExplorerLoadingView: UIView {
  ...
  private static func sparklingImage() -> UIImage? {
    guard let resourceRoot = Bundle.main.path(forResource: "Resource", ofType: nil) else {
      return nil
    }
    let path = (resourceRoot as NSString).appendingPathComponent(
      "extensions/sparkling-go/static/image/sparkling_icon.781d0b63.png")
