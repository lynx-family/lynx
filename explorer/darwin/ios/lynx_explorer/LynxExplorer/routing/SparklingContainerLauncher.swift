...
struct ExplorerSparklingAssetResolver {
  ...
  /// Keeps Explorer resource fetchers alive for exactly one Sparkling page.
  @MainActor
...
final class SparklingContainerLauncher: ContainerLaunching {
  private let schemeBuilder: SparklingCanonicalSchemeBuilding
    ...
    private let lifetimeFactory: @MainActor () -> SparklingPageLifetimeConfiguring
    ...
    init(
      ...
      lifetimeFactory: (@MainActor () -> SparklingPageLifetimeConfiguring)? = nil
    ) {
