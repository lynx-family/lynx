...
struct LaunchDescriptorParser {
  ...
  private func makeDescriptor(
    ...
    let enableNAPIAddon =
      Self.truthyValue(named: "enable_napi_addon", in: semanticItems) ?? false
    ...
    return LaunchDescriptor(
      ...
      debugOptions: DebugOptions(enableNAPIAddon: enableNAPIAddon),
  ...
  /// `enable_napi_addon` controls background-runtime creation. Preserve the
  /// shell's deliberately narrower legacy allow-list instead of accepting all
  /// values that `NSString.boolValue` considers true.
