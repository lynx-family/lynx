...
STALE_FML_THREAD_CONFIG_CTOR = re.compile(
    r"^_+ZN4lynx3fml6Thread12ThreadConfigC[12].*10shared_ptr.*b$"
)
...
def _animax_fml_header_errors(derived_data_path, lynx_root):
    """Prove AnimaX compiled against the checkout's LynxBase ABI."""
    expected_header = (Path(lynx_root) / "base/include/fml/thread.h").resolve()
    intermediates = Path(derived_data_path) / "Build/Intermediates.noindex"
    dependency_files = sorted(intermediates.glob("**/AnimaX.build/**/*.d"))
    actual_headers = set()
    path_pattern = re.compile(
        r"(?P<path>/(?:\\ |[^\s])*/base/include/fml/thread\.h)(?=\s|$)"
    )
    for dependency_file in dependency_files:
        try:
            ...
            return [f"cannot read AnimaX dependency file {dependency_file}: {error}"]
        for match in path_pattern.finditer(content):
            actual_headers.add(
                Path(match.group("path").replace("\\ ", " ")).resolve()
            )
    if not actual_headers:
        return ["AnimaX dependency files do not reference FML thread.h"]
    if actual_headers != {expected_header}:
        actual = ", ".join(str(path) for path in sorted(actual_headers))
        return [
            "AnimaX compiled against non-canonical FML ThreadConfig header; "
            f"expected {expected_header}, found: {actual}"
        ]
    return []
...
def verify_artifact(
    ...
    errors.extend(_animax_fml_header_errors(derived_data_path, lynx_root))
    ...
    for identifier, symbol in link_symbols:
        if STALE_FML_THREAD_CONFIG_CTOR.fullmatch(symbol):
            owner = link_owners.get(identifier, f"link-map object {identifier}")
            errors.append(
                "legacy bool ThreadConfig constructor linked from " f"{owner}"
            )
    runtime_symbols = []
    for identifier, symbol in link_symbols:
