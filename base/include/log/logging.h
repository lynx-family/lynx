...
// Lowest LOG macro level retained by the native logging build. INFO includes
// MONITOR/OBSERVE, so its lowest retained level is reported as MONITOR.
BASE_EXPORT const char* GetCompileMinLogLevelName();
