...
void MessageLoopHarmony::SetupNapiCallback(napi_env env) {
  BASE_TRACE_EVENT(LYNX_BASE_TRACE_CATEGORY,
                   MESSAGE_LOOP_HARMONY_SETUP_NAPI_CALLBACK);
  env_ = env;
  napi_value fn{};
...
bool MessageLoopHarmony::RunExpiredTasksByPromiseMicrotask() {
  ...
  if (status != napi_ok) {
    // Ark consumes the deferred even when resolution leaves an exception.
    // Clear it before continuing with any remaining tasks or direct fallback.
    napi_value exception{};
    napi_get_and_clear_last_exception(env_, &exception);
  }
  // A replaced Promise.then may succeed without registering the callback.
  return did_run_expired_tasks_in_microtask_;
}
