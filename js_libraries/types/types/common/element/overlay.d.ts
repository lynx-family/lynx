...
export interface OverlayProps extends Omit<StandardProps, 'binderror'> {
  ...
  /**
   * Sets the Android navigation bar background and contrasting system-bar appearance. `auto` inherits the host Activity navigation bar. Use `transparent` together with `android-adapt-edge-to-edge="true"` to extend the overlay behind the navigation bar on Android 15 (API level 35) and above.
   * @Android
   * @since 4.1
   * @defaultValue 'auto'
   */
