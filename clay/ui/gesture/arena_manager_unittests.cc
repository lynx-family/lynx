...
class DeferredMember : public ArenaMember {
 public:
  explicit DeferredMember(bool platform = false) : platform_(platform) {}
  bool IsPlatformGestureMember() const override { return platform_; }
...
TEST_F(DeferredArenaTest, NativeAcceptanceOverridesEarlierParentRequest) {
  ...
  EXPECT_TRUE(parent_entry->IsDeferred());
...
TEST_F(DeferredArenaTest, ParentCanWithdrawBeforeNativeDecision) {
  parent_entry->Resolve(GestureDisposition::kAccept);
...
TEST(PlatformGesturePointerTest, EarlierClayAcceptanceWaitsAtClose) {
  ArenaManager arena;
  DeferredMember local;
  auto entry = arena.Add(1, local.weak());
  entry->Resolve(GestureDisposition::kAccept);
