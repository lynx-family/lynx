...
TEST(ArenaManagerTest, CloseWithoutArenaDoesNotRecordPointerEvent) {
  ArenaManager arena_manager;
  auto down_event =
      CreateSinglePointer(ID(), PointerEvent::EventType::kDownEvent);

  arena_manager.Close(down_event);

  EXPECT_EQ(arena_manager.RecordingEventCountForTest(), 0u);
}

TEST(ArenaManagerTest, SweepClearsRecordedPointerEvent) {
  ArenaManager arena_manager;
  int pointer_id = ID();
  auto down_event =
  ...
  arena_manager.Close(down_event);
  EXPECT_EQ(arena_manager.RecordingEventCountForTest(), 1u);

  arena_manager.Sweep(pointer_id);

  EXPECT_EQ(arena_manager.RecordingEventCountForTest(), 0u);
}

TEST(ArenaManagerTest, CancelClearsRecordedPointerEvent) {
  ArenaManager arena_manager;
  int pointer_id = ID();
  auto down_event =
      CreateSinglePointer(pointer_id, PointerEvent::EventType::kDownEvent);
  ...
  arena_manager.Cancel(pointer_id);

  EXPECT_EQ(member.rejected_count, 1);
  EXPECT_EQ(arena_manager.RecordingEventCountForTest(), 0u);
}
...
TEST_F(GestureManagerTest, PassiveArenaAcceptClearsRecognizerArenaEntry) {
  TapGestureRecognizer recognizer(gesture_manager());
  auto down_event =
      CreateSinglePointer(ID(), PointerEvent::EventType::kDownEvent);

  recognizer.AddPointer(down_event);
  EXPECT_EQ(recognizer.ArenaEntryCountForTest(), 1u);

  gesture_manager()->arena_manager()->Close(down_event);

  EXPECT_EQ(recognizer.ArenaEntryCountForTest(), 0u);
}
