...
static Length GetLength(const clay::Value& value, const clay::Value& type,
  ...
  if (length_type == LengthUnit::kNum) {
    length_value *= pixel_ratio;
  }
...
static std::vector<Length> GetBorderRadius(const clay::Value::Array& array,
  ...
  for (size_t i = 0; i < array.size() / 2; i++) {
    values.emplace_back(GetLength(array[i * 2], array[i * 2 + 1], pixel_ratio));
  }
  return values;
}
  ...
  switch (property_id) {
    ...
    case KeywordID::kBackgroundPosition: {
      ...
      if (array.size() % 2 == 0) {
        ...
        for (size_t i = 0; i < array.size(); i += 2) {
          ...
          if (type == ClayPlatformLengthUnit::kNumber) {
            value *= pixel_ratio;
          }
    ...
    case KeywordID::kBackgroundSize: {
      ...
      if (array.size() % 2 == 0) {
        ...
        for (size_t i = 0; i < array.size(); i += 2) {
          double value = utils::GetDouble(array[i]);
          ClayBackgroundSizeType background_size_type =
              static_cast<ClayBackgroundSizeType>(-value);
          ClayPlatformLengthUnit type =
              static_cast<ClayPlatformLengthUnit>(utils::GetInt(array[i + 1]));
          if (background_size_type != ClayBackgroundSizeType::kAuto &&
              background_size_type != ClayBackgroundSizeType::kContain &&
              background_size_type != ClayBackgroundSizeType::kCover &&
              type == ClayPlatformLengthUnit::kNumber) {
            value *= pixel_ratio;
          }
    ...
    case KeywordID::kMaskSize: {
      ...
      if (array.size() % 2 == 0) {
        ...
        for (size_t i = 0; i < array.size(); i += 2) {
          double value = utils::GetDouble(array[i]);
          ClayMaskSizeType mask_size_type =
              static_cast<ClayMaskSizeType>(-value);
          ClayPlatformLengthUnit type =
              static_cast<ClayPlatformLengthUnit>(utils::GetInt(array[i + 1]));
          if (mask_size_type != ClayMaskSizeType::kAuto &&
              mask_size_type != ClayMaskSizeType::kContain &&
              mask_size_type != ClayMaskSizeType::kCover &&
              type == ClayPlatformLengthUnit::kNumber) {
            value *= pixel_ratio;
          }
          sizes.emplace_back(value, type);
    ...
    case KeywordID::kBorderWidth: {
      float width = utils::GetDouble(value) * pixel_ratio;
    ...
    case KeywordID::kBorderRadius: {
      view->SetBorderRadius(
          4, GetBorderRadius(utils::GetArray(value), pixel_ratio));
    } break;
    case KeywordID::kBorderTopLeftRadius: {
      view->SetBorderRadius(
          0, GetBorderRadius(utils::GetArray(value), pixel_ratio));
    } break;
    case KeywordID::kBorderTopRightRadius: {
      view->SetBorderRadius(
          1, GetBorderRadius(utils::GetArray(value), pixel_ratio));
    } break;
    case KeywordID::kBorderBottomRightRadius: {
      view->SetBorderRadius(
          2, GetBorderRadius(utils::GetArray(value), pixel_ratio));
    } break;
    case KeywordID::kBorderBottomLeftRadius: {
      view->SetBorderRadius(
          3, GetBorderRadius(utils::GetArray(value), pixel_ratio));
    } break;
    ...
    case KeywordID::kTransform: {
      ...
      for (size_t i = 0; i < array.size(); i++) {
        ...
        if (transform_raw.type ==
          ...
          // x
          transform_raw.values[0] =
              GetLength(values_arr[1], values_arr[2], pixel_ratio);
          // y
          transform_raw.values[1] =
              GetLength(values_arr[3], values_arr[4], pixel_ratio);
          // z
          transform_raw.values[2] =
              GetLength(values_arr[5], values_arr[6], pixel_ratio);
          // append op
          transform_raws.emplace_back(std::move(transform_raw));
        }
      }
      ...
      if (array.size() == 0 || array.size() == 2 || array.size() == 4) {
        ...
        if (array.size() >= 2) {
          transform_origin_values.emplace_back(
              GetLength(array[0], array[1], pixel_ratio));
          if (array.size() == 4) {
            transform_origin_values.emplace_back(
                GetLength(array[2], array[3], pixel_ratio));
          }
        }
    ...
    case KeywordID::kBoxShadow: {
      ...
      for (size_t i = 0; i < array.size(); i++) {
        ...
        shadows[i].offset_x = utils::GetDouble(arr[0]) * pixel_ratio;
        shadows[i].offset_y = utils::GetDouble(arr[1]) * pixel_ratio;
        shadows[i].blur_radius = utils::GetDouble(arr[2]) * pixel_ratio;
        shadows[i].spread_radius = utils::GetDouble(arr[3]) * pixel_ratio;
    ...
    case KeywordID::kOutlineWidth: {
      view->SetOutlineWidth(utils::GetInt(value) * pixel_ratio);
    } break;
