  ...
  if (!is_composing && new_value_length > max_length_) {
    if (old_value_length == max_length_) {
      *value = GetTextEditingValue();
      ...
      value->SetText(value->GetU16Text().substr(0, max_length_),
                     TextRange(max_length_));
      UpdateRemoteStateIfNeeded(*value);
...
void EditableView::OnSelectionChanged(const TextEditingValue& value,
  ...
  page_view()->SendEvent(callback_id_, event_attr::kEventEditSelectionChange,
                         ...
                         value.selection().reversed() ? "backward" : "forward");
}
...
void EditableView::OnUserInput(const TextEditingValue& value,
  ...
  page_view()->SendEvent(
      ...
      {"value", "cursor", "isComposing"}, text_editing_str.c_str(),
      static_cast<int>(text_editing_value.selection().extent()),
...
bool EditableView::OnKeyEventInternal(const KeyEvent* key_event) {
  ...
  switch (keycode) {
    ...
    case KeyCode::kSpace: {
#if defined(OS_ANDROID) || defined(OS_HARMONY) && !defined(ENABLE_HEADLESS)
...
void EditableView::SetContent(const std::string& content,
  ...
  FilterInputIfNeeded(&text_editing_value);
  SetTextEditingValue(text_editing_value, trigger_input_event);
  MarkNeedsLayout();
}
...
void EditableView::setValue(const LynxModuleValues& args,
  ...
  auto text_editing_value = GetTextEditingValue();
  text_editing_value.SetText(content, TextRange(index));
  SetTextEditingValue(text_editing_value);
  MarkNeedsLayout();
...
void EditableView::UpdateEditingState(std::string text, TextSelection selection,
  ...
  // When text is changed by filter, we need update text state to remote
  bool need_update_remote = old_value.GetU16Text() != new_value.GetU16Text();
  bool trigger_input_event =
      !is_composing && pre_text_value_ != new_value.GetU16Text();
  SetTextEditingValue(new_value, trigger_input_event, need_update_remote);
  if (trigger_input_event) {
    pre_text_value_ = new_value.GetU16Text();
  }
  MarkNeedsLayout();
}
...
void TextEditingHistoryState::Update(
  ...
  editable_view_->SetTextEditingValue(*new_value, true);
}
