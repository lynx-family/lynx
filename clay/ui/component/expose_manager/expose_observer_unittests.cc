...
TEST_F(ExposeObserverTest, ExposureUIClipTracksAncestorAttributeUpdates) {
  auto* parent = new View(1, page_.get());
  page_->AddChild(parent);
  parent->SetBound(0, 0, 100, 50);
  parent->SetOverflow(CSSProperty::OVERFLOW_HIDDEN);
  parent->SetAttribute("enable-exposure-ui-clip", Value(false));
  AddObservedView(parent, 2, 0, 60, 100, 30);

  manager()->NotifyObservers();
  EXPECT_EQ(CustomEventCount("uiappear"), 1u);

  parent->SetAttribute("enable-exposure-ui-clip", Value(true));
  NotifyObserversOnNextFrame();
  EXPECT_EQ(CustomEventCount("uidisappear"), 1u);
