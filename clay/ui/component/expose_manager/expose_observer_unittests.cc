...
TEST_F(ExposeObserverTest, SmallTargetUsesNodeReadyFallback) {
  View* target = AddVisibleObservedView(1);
...
TEST_F(ExposeObserverTest, ReappearFanoutPreservesOneEventPerObserver) {
  AddVisibleObservedView(1);
  AddVisibleObservedView(2);
  manager()->NotifyObservers();
  manager()->StopExposure(true);
  manager()->ResumeExposure();
  manager()->NotifyObservers();

  EXPECT_EQ(custom_events(), Events({"uiappear", "uiappear", "uidisappear",
                                     "uidisappear", "uiappear", "uiappear"}));
}
