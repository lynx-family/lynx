...
#ifdef ENABLE_SKITY
  if (!node_ready_ || !page_view_ || !page_view_->DeferredImageDecode() ||
      (source_.empty() && placeholder_.empty())) {
    return;
  }
  // Check transitions to intrinsic size once before taking this fast path.
  if (last_checked_decode_size_ == Size{} &&
      (IsSVG() || !GetRenderImage()->ShouldDecodeToViewSize())) {
    return;
  }
...
void BaseImageView::FetchSource(bool reload) {
  ...
  source_fetch_id_ = page_view_->GetImageResourceFetcher()->FetchImage(
      ...
      [self = weak_factory_.GetWeakPtr(), reload](
        ...
        // not currently exposed, so we temporarily enable default.
