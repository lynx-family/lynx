...
#include "clay/ui/component/nested_scroll/nested_draggable.h"
...
NestedScrollable::~NestedScrollable() {
  ...
  if (handle_signal_event_) {
    page_view()->gesture_manager()->FireEndSignalEvent();
  }
        ...
        [weak = this->GetWeakPtr()](const PointerEvent& event) {
          ...
          switch (event.signal_kind) {
            ...
            case PointerEvent::SignalKind::kScroll: {
              ...
              if (scrollable->signal_scroll_started_) {
                if (event.is_precise_scroll) {
                  scrollable->DoScroll(delta);
                } else {
                  scrollable->DoWheelScroll(delta);
                }
              } else {
                if (auto parent =
                        scrollable->FindAncestorNestedDraggableView()) {
                  if (event.is_precise_scroll) {
                    parent->DoScroll(delta);
                  } else {
                    parent->DoWheelScroll(delta);
                  }
                }
              }
              break;
