...
void PageView::ReportTopViewEvent(const PointerEvent& event,
  ...
  if (event.signal_kind == PointerEvent::SignalKind::kScroll) {
    if (UNLIKELY(top_view->Is<NativeView>())) {
      ...
      if (native_view->IsScrollEnabled()) {
    ...
    switch (event.device) {
      case PointerEvent::DeviceType::kTouch: {
        ...
        bool is_raw_events = type == kClayEventTypeTouchStart ||
                             type == kClayEventTypeTouchEnd ||
      ...
      case PointerEvent::DeviceType::kMouse: {
        ...
        if (event_delegate_) {
          event_delegate_->OnMouseEvent(
              ...
              buttons_state_, transformed_position.x(),
              transformed_position.y(), position.x(), position.y());
          return;
        }
      } break;
...
void PageView::ReportKeyEvent(const KeyEvent& event) {
  if (event_delegate_ && (event.GetType() == KeyEventType::kUp ||
    ...
    auto* focused_node = focus_manager_.GetLeafFocusedNode();
    auto* focused_view = static_cast<BaseView*>(focused_node);
    if (!focused_view) {
      return;
    }

    int focused_view_id = focused_view->id();
    ...
    event_delegate_->OnKeyEvent(EventTypeToString(temp_type), focused_view_id,
                                web_key.c_str());
  }
}
...
void PageView::DispatchKeyEvent(
  ...
  FML_DCHECK(event);
  // Tell parent view to not handle this event anymore. Just like the return
  // value of android View.OnXXEvent.
