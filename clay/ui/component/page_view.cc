...
void PageView::SetEnablePointerEvents(bool enabled) {
  if (enable_pointer_events_ == enabled) {
    return;
  }
  enable_pointer_events_ = enabled;
#if defined(OS_WIN) || defined(OS_MAC)
  pointer_view_map_.clear();
  primary_pointer_ids_.clear();
  active_pointer_ids_.clear();
  mouse_region_manager_->ResetPointerEvents();
#endif
}
...
bool PageView::DispatchPointerEvent(std::vector<PointerEvent> events) {
  ...
  if (events.size() > 1 &&
    ...
    for (const auto& event : events) {
      consumed |= DispatchPointerEvent({event});
    }
    return consumed;
  }
#endif
  ...
  if (enable_pointer_events_) {
    for (const auto& event : events) {
      ...
      if (consumed || event.type == PointerEvent::EventType::kHoverEvent) {
  ...
  if (legacy_events.empty() && !events.empty() &&
      events.front().dispatch_mode ==
          PointerEvent::DispatchMode::kPenWithTouchCompatibility) {
    return consumed;
  }
#endif
#if defined(ENABLE_MOUSE_TRACKING)
  // Keep all legacy boundary events ahead of the batch's main events.
  mouse_region_manager_->HandleEvents(this, legacy_events);
#endif
  ...
  if (consumed) {
    ...
    // well.
    ReportTopViewRawEvents(legacy_events);
...
#if defined(ENABLE_MOUSE_TRACKING)
    for (const PointerEvent& event : legacy_events) {
