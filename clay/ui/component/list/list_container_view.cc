...
ListContainerView::ListContainerView(int32_t id, PageView* page_view)
    // TODO(dongjiajian): lynx list container currently not support horizontal.
...
void ListContainerView::SetScrollState(ScrollState state) {
  ...
  scroll_state_ = state;
  if (status_ == Scrollable::ScrollStatus::kIdle &&
      page_view_->GetUIComponentDelegate()) {
    page_view_->GetUIComponentDelegate()->OnScrollStopped(id());
  }
  // TODO(dongjiajian): add `attachedCells`.
  page_view_->SendEvent(id(), event_attr::kEventScrollStateChange, {"state"},
