...
void ScrollbarView::HandleTrackTapDown(const PointerEvent& event) {
  ...
  if (delegate_ && old_position != position_) {
    delegate_->OnScrollbarScrolled(old_position, position_, true, true);
  }
}
...
void ScrollbarView::SetPosition(float position) {
  if (position_ != position) {
    ...
    UpdateThumb();
    if (delegate_) {
      delegate_->OnScrollbarScrolled(old_position, position_, false, false);
    }
  }
}
...
void ScrollbarView::OnThumbDrag(const FloatSize& delta) {
  ...
  if (delegate_ && old_position != position_) {
    delegate_->OnScrollbarScrolled(old_position, position_, true, false);
  }
