...
void FrameView::SetAttribute(const char* attr, const clay::Value& value) {
  ...
  if (name == kSrc) {
    ...
    pending_data_ = ToFrameChildData(value);
    is_props_updated_ = true;
  } else if (name == kGlobalProps) {
    pending_global_props_ = ToFrameChildData(value);
    is_props_updated_ = true;
...
void FrameView::SetEventForwarder(
    ...
    std::function<int(const FloatPoint&, int, bool*)> native_view_hit_test) {
  pointer_event_forwarder_ = std::move(pointer);
  key_event_forwarder_ = std::move(key);
  native_view_hit_test_forwarder_ = std::move(native_view_hit_test);
}

void FrameView::ClearEventForwarder() {
  pointer_event_forwarder_ = nullptr;
  key_event_forwarder_ = nullptr;
  native_view_hit_test_forwarder_ = nullptr;
}

int FrameView::ForwardNativeViewHitTest(const FloatPoint& position,
                                        int platform_try_hit_id,
                                        bool* has_hit_target) const {
  if (!native_view_hit_test_forwarder_) {
    if (has_hit_target) {
      *has_hit_target = false;
    }
    return -1;
  }
  return native_view_hit_test_forwarder_(position, platform_try_hit_id,
                                         has_hit_target);
}
...
std::shared_ptr<lynx::tasm::FrameChildData> FrameView::ToFrameChildData(
...
#if defined(OS_ANDROID)
  return std::make_shared<FrameChildDataAndroid>(value);
#else
  if (value.IsLong()) {
