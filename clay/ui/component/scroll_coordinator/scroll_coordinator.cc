...
constexpr std::array<KeywordID, 3> kProxyAttributes = {{
    KeywordID::kHeaderOverSlot,
    KeywordID::kScrollEnable,
    KeywordID::kGranularity,
}};

constexpr std::array<std::string_view, 1> kProxyEvents = {{
    "offset",
}};
...
void ScrollCoordinator::AddEventCallback(const char* event) {
  if (std::find(kProxyEvents.begin(), kProxyEvents.end(), event) !=
      kProxyEvents.end()) {
