...
class FrameView : public WithTypeInfo<FrameView, BaseView> {
 public:
  ...
  void SetEventForwarder(std::function<bool(const PointerEvent&)> pointer,
                         ...
                         std::function<int(const FloatPoint&, int, bool*)>
                             native_view_hit_test);
  void ClearEventForwarder();
  int ForwardNativeViewHitTest(const FloatPoint& position,
  ...
  const std::shared_ptr<lynx::tasm::FrameChildData>& pending_data() const {
    return pending_data_;
  }
  const std::shared_ptr<lynx::tasm::FrameChildData>& pending_global_props()
      const {
 ...
 private:
  ...
  static std::shared_ptr<lynx::tasm::FrameChildData> ToFrameChildData(
  ...
  std::shared_ptr<lynx::tasm::FrameChildData> pending_data_;
  std::shared_ptr<lynx::tasm::FrameChildData> pending_global_props_;
  ...
  std::function<int(const FloatPoint&, int, bool*)>
      native_view_hit_test_forwarder_;
};
