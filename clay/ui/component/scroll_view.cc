...
void ScrollView::CorrectScrollOffset() {
  ...
  if (scroll_direction_ == ScrollDirection::kVertical) {
    ...
    if (!RoughlyEqualToZero(scroll_offset_.y() - target_offset)) {
      OnScrollUpdate(target_offset);
      return;
    }
    if (!RoughlyEqualToZero(scroll->ScrollTop() - target_offset)) {
    ...
    if (!RoughlyEqualToZero(scroll_offset_.x() - target_offset)) {
      OnScrollUpdate(target_offset);
      return;
    }
    if (!RoughlyEqualToZero(scroll->ScrollLeft() - target_offset)) {
