...
IntersectionObserverManager::IntersectionObserverManager(BaseView* page_view)
    : direct_page_child_first_add_exposure_enabled_(
          setting::CLAY_ENABLE_DIRECT_PAGE_CHILD_FIRST_ADD_EXPOSURE.value()),
      large_exposure_target_after_layout_enabled_(
          setting::CLAY_ENABLE_LARGE_EXPOSURE_TARGET_AFTER_LAYOUT.value()),
...
bool IntersectionObserverManager::TryNotifyDirectPageChildOnFirstAdd(
  ...
  if (!direct_page_child_first_add_exposure_enabled_) {
    return false;
  }
...
bool IntersectionObserverManager::TryReconcileLargeExposureTargetAfterLayout(
  ...
  if (!large_exposure_target_after_layout_enabled_ || !layout_view ||
      !page_view_ || expose_observers_map_.empty() ||
