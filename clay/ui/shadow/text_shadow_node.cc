...
void TextShadowNode::OnLayout(float width, TextMeasureMode width_mode,
  ...
  BaseTextShadowNode::OnLayout(width, width_mode, height, height_mode, paddings,
                               borders);
  auto content_width =
      width - paddings[0] - paddings[2] - borders[0] - borders[2];
  auto content_height =
      height - paddings[1] - paddings[3] - borders[1] - borders[3];
  auto constraint = MeasureConstraint(
      {content_width, width_mode, content_height, height_mode});
