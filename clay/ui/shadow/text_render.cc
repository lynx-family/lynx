...
void TextRender::ReprocessAttributeIfNeeded(double layout_width) {
  ...
  for (auto* child : measure_node_->GetChildren()) {
    if (!child->IsRawTextShadowNode() &&
      ...
      auto parent_run_metrics = parent_paragraph->GetLineMetrics()
                                    .front()
                                    .run_metrics.find(0)
                                    ->second;
      ...
      FontMetrics parent_metrics{
          ...
          parent_run_metrics.font_metrics.fXHeight,
      ...
      if (child->IsInlineTextShadowNode()) {
        ...
        child->SetBaselineOffset(parent_metrics,
                                 child_paragraph->GetLineMetrics()
                                     .front()
                                     .run_metrics.begin()
      ...
      if (cache_paragraph_ &&
        ...
        if (truncation_size.width() > constraint.width) {
          for (auto truncation_child : child->GetChildren()) {
            if (truncation_child->IsInlineImageShadowNode() ||
                truncation_child->IsInlineViewShadowNode()) {
              truncation_child->SetEndIndex(0);
            }
          }
