...
clay::Value TextRender::GetTextInfo(const char* text, const clay::Value& params,
  ...
  if (endptr == font_size_str.c_str()) {
    ...
    text_style.font_size =
        page_view ? page_view->ConvertFrom<kPixelTypeLogical>(num) : num;
  }
...
std::unique_ptr<txt::Paragraph> TextRender::LayoutParagraph(
  ...
  paragraph->Layout(layout_width);
#if defined(CLAY_ENABLE_TTTEXT)
  auto* tttext_paragraph = static_cast<txt::ParagraphTTText*>(paragraph.get());
  truncation_direction_ = tttext_paragraph->GetResolvedWriteDirection() ==
