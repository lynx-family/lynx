...
std::optional<PointerEvent> PointerEvent::ToLegacyEvent() const {
  if (type == EventType::kAddEvent) {
    return std::nullopt;
  }
  if (dispatch_mode != DispatchMode::kPenWithTouchCompatibility) {
    auto event = *this;
    if (type == EventType::kRemoveEvent) {
  ...
  // Pen hover and presence changes must not enter the legacy touch path.
  ...
  // Legacy touch recognizers expect primary-button contact state.
