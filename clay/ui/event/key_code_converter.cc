...
// Keep this table sorted by logical key for binary search.
constexpr std::pair<uint64_t, const char*> logical_key2web_key[] = {
...
std::string KeyCodeConverter::ConvertToWebKey(LogicalKeyboardKey key_code,
  ...
  const auto* end = logical_key2web_key + sizeof(logical_key2web_key) /
                                              sizeof(logical_key2web_key[0]);
  const auto* entry =
      std::lower_bound(logical_key2web_key, end, logical,
                       [](const std::pair<uint64_t, const char*>& entry,
                          uint64_t key) { return entry.first < key; });
  if (entry != end && entry->first == logical) {
    return entry->second;
  }
  if (!character.empty()) {  // visual characters
    return character;
