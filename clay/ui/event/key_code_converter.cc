...
const std::unordered_map<uint64_t, const char*>& LogicalKeyToWebKey() {
  static const std::unordered_map<uint64_t, const char*> key_map = {
...
std::string KeyCodeConverter::ConvertToWebKey(LogicalKeyboardKey key_code,
  ...
  const auto& key_map = LogicalKeyToWebKey();
  auto iter = key_map.find(logical);
  if (iter != key_map.end()) {
    return iter->second;
  }
