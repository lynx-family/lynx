...
#ifndef ENABLE_SKITY
void ImagePainter::PaintMaskImage(SkCanvas* canvas) {
  ...
  for (size_t index = 0; index < mask.images.size(); index++) {
    ...
    canvas->clipRect(ConvertSkityRectToSkRect(clip_rect));
...
void ImagePainter::PaintSingleMaskImage(SkCanvas* canvas,
  ...
  if (image_resource && image_resource->GetGraphicsImage() &&
    ...
    canvas->drawImageRect(image, ConvertSkityRectToSkRect(src_rect),
    ...
    mask_paint.setShader(shader);
    canvas->drawRect(ConvertSkityRectToSkRect(dst_rect), mask_paint);
  }
}
#endif  // ENABLE_SKITY
