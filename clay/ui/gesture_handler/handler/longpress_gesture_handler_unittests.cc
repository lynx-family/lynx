// Copyright 2026 The Lynx Authors. All rights reserved.
// Licensed under the Apache License Version 2.0 that can be found in the
// LICENSE file in the root directory of this source tree.
#include <memory>

#include "clay/public/value.h"
#include "clay/ui/gesture_handler/handler/gesture_handler_test_utils.h"
#include "clay/ui/gesture_handler/handler/longpress_gesture_handler.h"
#include "third_party/googletest/googletest/include/gtest/gtest.h"

namespace clay {
namespace testing {

using ::testing::_;
using ::testing::Eq;
using ::testing::StrEq;

class LongPressGestureHandlerTest : public ::testing::Test {};

TEST_F(LongPressGestureHandlerTest,
       HandleConfigMap_MinDurationAndMaxDistanceApplied) {
  auto runner = TestTaskRunner::Create();
  auto page_view = MakeTestPageView(0, runner);
  TestGestureArenaMember member(1);

  Value::Map config_map;
  config_map.emplace(GestureConstants::MIN_DURATION, Value(30));
  config_map.emplace(GestureConstants::MAX_DISTANCE, Value(1.0f));
  Value config(std::move(config_map));
  auto detector =
      MakeDetector(1, GestureHandlerType::LongPress, {}, {}, std::move(config));
  LongPressGestureHandler handler(1, page_view.get(), detector,
                                  member.GetWeakPtr());

  PointerEvent down =
      MakePointerEvent(PointerEvent::EventType::kDownEvent, {0, 0}, 1);
  PointerEvent move =
      MakePointerEvent(PointerEvent::EventType::kMoveEvent, {10, 0}, 2);

  handler.HandleMotionEvent(&down, 0, 0, false, nullptr);
  handler.HandleMotionEvent(&move, 0, 0, false, nullptr);
  EXPECT_EQ(handler.GetGestureStatus(), GestureConstants::LYNX_STATE_FAIL);
}

TEST_F(LongPressGestureHandlerTest,
       OnHandle_Down_SetsBegin_EmitsOnBegin_StartsTimer) {
  auto runner = TestTaskRunner::Create();
  auto page_view = MakeTestPageView(0, runner);
  MockEventDelegate delegate;
  page_view->SetEventDelegate(&delegate);

  TestGestureArenaMember member(1);
  Value::Map config_map;
  config_map.emplace(GestureConstants::MIN_DURATION,
                     Value(static_cast<int64_t>(30)));
  Value config(std::move(config_map));
  auto detector =
      MakeDetector(1, GestureHandlerType::LongPress,
                   {GestureConstants::ON_BEGIN}, {}, std::move(config));
  LongPressGestureHandler handler(1, page_view.get(), detector,
                                  member.GetWeakPtr());

  PointerEvent down =
      MakePointerEvent(PointerEvent::EventType::kDownEvent, {0, 0}, 1);

  EXPECT_CALL(delegate, OnGestureHandlerEvent(StrEq(GestureConstants::ON_BEGIN),
                                              Eq(1), Eq(1), _, _, _, _, _, _))
      .Times(1);
  handler.HandleMotionEvent(&down, 0, 0, false, nullptr);
  EXPECT_EQ(handler.GetGestureStatus(), GestureConstants::LYNX_STATE_BEGIN);
}

TEST_F(LongPressGestureHandlerTest, TimerFires_ActivatesAndEmitsOnStart) {
  auto runner = TestTaskRunner::Create();
  auto page_view = MakeTestPageView(0, runner);
  MockEventDelegate delegate;
  page_view->SetEventDelegate(&delegate);

  TestGestureArenaMember member(1);
  Value::Map config_map;
  config_map.emplace(GestureConstants::MIN_DURATION,
                     Value(static_cast<int64_t>(30)));
  Value config(std::move(config_map));
  auto detector =
      MakeDetector(1, GestureHandlerType::LongPress,
                   {GestureConstants::ON_START}, {}, std::move(config));
  LongPressGestureHandler handler(1, page_view.get(), detector,
                                  member.GetWeakPtr());

  PointerEvent down =
      MakePointerEvent(PointerEvent::EventType::kDownEvent, {3, 4}, 1);

  EXPECT_CALL(delegate,
              OnGestureHandlerEvent(StrEq(GestureConstants::ON_START), Eq(1),
                                    Eq(1), _, _, Eq(3.0f), Eq(4.0f), Eq(1), _))
      .Times(1);
  handler.HandleMotionEvent(&down, 0, 0, false, nullptr);
  down.position = {30, 40};
  down.timestamp = 10;
  runner->AdvanceBy(fml::TimeDelta::FromMilliseconds(30));
  EXPECT_TRUE(handler.IsActive());
}

TEST_F(LongPressGestureHandlerTest, ResetStopsPendingTimer) {
  auto runner = TestTaskRunner::Create();
  auto page_view = MakeTestPageView(0, runner);
  TestGestureArenaMember member(1);

  Value::Map config_map;
  config_map.emplace(GestureConstants::MIN_DURATION, Value(30));
  Value config(std::move(config_map));
  auto detector =
      MakeDetector(1, GestureHandlerType::LongPress, {}, {}, std::move(config));
  LongPressGestureHandler handler(1, page_view.get(), detector,
                                  member.GetWeakPtr());

  PointerEvent down =
      MakePointerEvent(PointerEvent::EventType::kDownEvent, {0, 0}, 1);
  handler.HandleMotionEvent(&down, 0, 0, false, nullptr);
  handler.Reset();
  runner->AdvanceBy(fml::TimeDelta::FromMilliseconds(30));

  EXPECT_EQ(handler.GetGestureStatus(), GestureConstants::LYNX_STATE_INIT);
}

TEST_F(LongPressGestureHandlerTest, EndStopsPendingTimer) {
  auto runner = TestTaskRunner::Create();
  auto page_view = MakeTestPageView(0, runner);
  TestGestureArenaMember member(1);

  Value::Map config_map;
  config_map.emplace(GestureConstants::MIN_DURATION, Value(30));
  Value config(std::move(config_map));
  auto detector =
      MakeDetector(1, GestureHandlerType::LongPress, {}, {}, std::move(config));
  LongPressGestureHandler handler(1, page_view.get(), detector,
                                  member.GetWeakPtr());

  PointerEvent down =
      MakePointerEvent(PointerEvent::EventType::kDownEvent, {0, 0}, 1);
  handler.HandleMotionEvent(&down, 0, 0, false, nullptr);
  handler.End();
  runner->AdvanceBy(fml::TimeDelta::FromMilliseconds(30));

  EXPECT_EQ(handler.GetGestureStatus(), GestureConstants::LYNX_STATE_END);
}

TEST_F(LongPressGestureHandlerTest,
       OnHandle_Move_ExceedDistance_FailsAndStopsTimer) {
  auto runner = TestTaskRunner::Create();
  auto page_view = MakeTestPageView(0, runner);
  TestGestureArenaMember member(1);

  Value::Map config_map;
  config_map.emplace(GestureConstants::MIN_DURATION,
                     Value(static_cast<int64_t>(30)));
  config_map.emplace(GestureConstants::MAX_DISTANCE, Value(1.0f));
  Value config(std::move(config_map));
  auto detector =
      MakeDetector(1, GestureHandlerType::LongPress, {}, {}, std::move(config));
  LongPressGestureHandler handler(1, page_view.get(), detector,
                                  member.GetWeakPtr());

  PointerEvent down =
      MakePointerEvent(PointerEvent::EventType::kDownEvent, {0, 0}, 1);
  PointerEvent move =
      MakePointerEvent(PointerEvent::EventType::kMoveEvent, {10, 0}, 2);

  handler.HandleMotionEvent(&down, 0, 0, false, nullptr);
  handler.HandleMotionEvent(&move, 0, 0, false, nullptr);
  EXPECT_EQ(handler.GetGestureStatus(), GestureConstants::LYNX_STATE_FAIL);

  runner->AdvanceBy(fml::TimeDelta::FromMilliseconds(30));
  EXPECT_EQ(handler.GetGestureStatus(), GestureConstants::LYNX_STATE_FAIL);
}

TEST_F(LongPressGestureHandlerTest, OnHandle_Up_StopsTimer_ThenFails) {
  auto runner = TestTaskRunner::Create();
  auto page_view = MakeTestPageView(0, runner);
  TestGestureArenaMember member(1);

  Value::Map config_map;
  config_map.emplace(GestureConstants::MIN_DURATION,
                     Value(static_cast<int64_t>(30)));
  Value config(std::move(config_map));
  auto detector =
      MakeDetector(1, GestureHandlerType::LongPress, {}, {}, std::move(config));
  LongPressGestureHandler handler(1, page_view.get(), detector,
                                  member.GetWeakPtr());

  PointerEvent down =
      MakePointerEvent(PointerEvent::EventType::kDownEvent, {0, 0}, 1);
  PointerEvent up =
      MakePointerEvent(PointerEvent::EventType::kUpEvent, {0, 0}, 2);

  handler.HandleMotionEvent(&down, 0, 0, false, nullptr);
  handler.HandleMotionEvent(&up, 0, 0, false, nullptr);
  EXPECT_EQ(handler.GetGestureStatus(), GestureConstants::LYNX_STATE_FAIL);
}

TEST_F(LongPressGestureHandlerTest, OnHandle_NullPointer_Ignores) {
  auto runner = TestTaskRunner::Create();
  auto page_view = MakeTestPageView(0, runner);
  TestGestureArenaMember member(1);
  auto detector = MakeDetector(1, GestureHandlerType::LongPress, {});
  LongPressGestureHandler handler(1, page_view.get(), detector,
                                  member.GetWeakPtr());

  handler.HandleMotionEvent(nullptr, 0, 0, false, nullptr);
  EXPECT_EQ(handler.GetGestureStatus(),
            GestureConstants::LYNX_STATE_UNDETERMINED);
}
}  // namespace testing

}  // namespace clay
