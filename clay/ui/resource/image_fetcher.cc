...
uint64_t ImageFetcher::FetchImage(const std::string& original_url, bool is_svg,
  ...
  // Mobile image loaders need the target size when starting a decode. Pending
  // requests for the same URL but different explicit decode sizes must not
  // share one ImageRequest: a URL-only key would merge them and use the first
  // request's size for all callbacks, potentially returning an undersized image
  // or decoding an unnecessarily large one. Include nonzero decode dimensions
  // in the key to keep these decode operations and their callbacks independent.
  // The cache identifier remains URL-based; FindImageFromCache separately
  // checks whether a cached image can satisfy the requested size.
