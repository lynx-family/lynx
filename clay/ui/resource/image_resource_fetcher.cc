...
#include "clay/ui/resource/loader/resource_loader.h"
#include "clay/ui/resource/loader/resource_loader_factory.h"
#include "clay/ui/resource/loader/resource_loader_intercept.h"
...
ImageFetchID ImageResourceFetcher::FetchImageAsync(
    ...
    bool use_texture_backend, bool is_deferred, bool need_redirect,
  ...
  if (it == url_loader_map_.end()) {
    ...
    auto task = [self = weak_factory_.GetWeakPtr(), final_url, begin, is_svg,
      ...
      self->OnDownloadEnd(success, final_url, data, is_svg, use_texture_backend,
                          enable_low_quality_image, is_deferred, is_promise);
    };
...
std::unique_ptr<ImageResource> ImageResourceFetcher::FetchPromiseImage(
  ...
  image_resource = image_manager_->CreateImageResourceFromCachedData(
      url, false, use_texture_backend, use_promise, false, false);
  if (image_resource) {
  ...
  FetchImageAsyncInternal(url, callback, use_texture_backend, false,
...
void ImageResourceFetcher::GetImageResource(
    ...
    bool is_deferred, bool enable_low_quality_image) {
  FML_DCHECK(task_runners_.GetUITaskRunner()->RunsTasksOnCurrentThread());

  std::string trim_url = TrimUrl(url);
  std::string final_url = USE_TRIM_URL_IF_COULD(trim_url, url);

  image_manager_->GetImageResource(final_url, callback, source, len,
...
ImageResourceFetcher::GetImageResourceFromSVGContent(const std::string& source,
                                                     bool use_texture_backend,
                                                     bool is_deferred) {
  FML_DCHECK(task_runners_.GetUITaskRunner()->RunsTasksOnCurrentThread());

  return image_manager_->GetImageResourceFromSVGContent(
      source, use_texture_backend, is_deferred);
}
#endif
...
void ImageResourceFetcher::OnDownloadEnd(bool success, const std::string& url,
                                         GrDataPtr data, bool is_svg,
                                         bool use_texture_backend,
                                         bool enable_low_quality_image,
                                         bool is_deferred, bool is_promise) {
  FML_DCHECK(task_runners_.GetUITaskRunner()->RunsTasksOnCurrentThread());
  // remove loader first.
