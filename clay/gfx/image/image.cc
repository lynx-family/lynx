...
std::shared_ptr<Image> Image::CreateImage(
  ...
  auto image = new Image(url, data, weak_notifier, ui_runner, raster_runner,
                         is_svg, is_promise, enable_low_quality_image,
  ...
  if (!notifier) {
    ...
    fml::closure decode_callback =
        fml::closure([weak = image->weak_from_this()]() {
...
std::shared_ptr<Image> Image::CloneImage(std::shared_ptr<Image> image) {
  ...
  return Image::CreateImage(
      image->url_, image->image_producer_->GetData(),
#ifndef ENABLE_SKITY
      nullptr,
#else
      image->image_producer_->GetDescriptor(),
#endif  // ENABLE_SKITY
      image->weak_notifier_, image->ui_runner_, image->raster_runner_,
      image->is_svg_, image->use_texture_backend_, image->is_promise_,
      image->image_producer_->EnableLowQualityImage(), image->is_deferred_);
}
...
fml::RefPtr<GraphicsImage> Image::GetGraphicsImage() {
  std::unique_ptr<std::lock_guard<std::recursive_mutex>> lock;
  if (is_deferred_) {
    lock = std::make_unique<std::lock_guard<std::recursive_mutex>>(*mutex_);
  }
...
void Image::DecodeFinish() {
  bool result = is_svg_ ? image_producer_->SVGFrameReady()
                        : image_producer_->NextFrameReady();
  if (IsDeferred()) {
    ...
    // here.
    if (result && image_producer_->WillNextFrameSizeChange()) {
      ui_runner_->PostTask([weak_self = weak_from_this()] {
        auto self = weak_self.lock();
        if (self) {
