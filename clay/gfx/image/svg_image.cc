...
void SVGImage::Upload(GraphicsContext* context, Size size) {
  if (!gpu_image_.object() || gpu_image_.object()->width() < size.width() ||
    ...
    gpu_image_ =
        GPUObject(GraphicsImage::Make(image), context->GetUnrefQueue());
    context->GetUnrefQueue()->GetTaskRunner()->PostTask(
        [context = context->GetUnrefQueue()->GetContext(), image, pixmap]() {
