...
TEST(ValueAnimatorTest, LifecyclePauseResumeAtExplicitTime) {
  AnimationHandler handler;
  ValueAnimator animator;
  animator.SetAnimationHandler(&handler);
  animator.SetDuration(100);
  ...
  animator.PauseForTest(1050);
  const auto paused_fraction = animator.GetPresentationFraction(5000);
  ASSERT_TRUE(paused_fraction.has_value());
  EXPECT_FLOAT_EQ(*paused_fraction, 0.5f);

  animator.ResumeForTest(6050);
  const auto resumed_fraction = animator.GetPresentationFraction(6050);
  ASSERT_TRUE(resumed_fraction.has_value());
  EXPECT_FLOAT_EQ(*resumed_fraction, 0.5f);

  handler.DoAnimationFrame(6100, true);
  EXPECT_FALSE(animator.IsStarted());
}
