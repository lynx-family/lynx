...
TEST(AnimationHandlerTest, RemovingEarlierCallbackReschedulesRemainingOne) {
  ...
  // The deadline may already be due if the test process was suspended between
  // scheduling and removal. In that case, an immediate wake-up is valid.
  EXPECT_GE(scheduled_delays.back(), 0);
