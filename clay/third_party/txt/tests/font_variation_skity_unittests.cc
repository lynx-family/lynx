...
std::shared_ptr<skity::Typeface> LoadVariableFont() {
  const std::string path =
      std::string(clay::testing::GetFixturesPath()) + "/Variable.ttf";
...
TEST(VariableFontSkityTest, MaterializesFiltersAndCachesRealAxes) {
  auto base = LoadVariableFont();
  ASSERT_NE(base, nullptr);

  auto manager = std::make_shared<DynamicFontManager>();
  const std::map<std::string, float> requested_axes = {
      {"opsz", 42.f}, {"wdth", 150.f}, {"wght", 715.f}};
  auto first = manager->GetOrCreateVariation(base, requested_axes);
  ...
  EXPECT_NE(varied->TypefaceId(), base->TypefaceId());
  ...
  auto cached = manager->GetOrCreateVariation(base, requested_axes);
...
TEST(VariableFontSkityTest, ResolvesSansSerifThroughDefaultTypeface) {
  auto default_typeface = LoadVariableFont();
