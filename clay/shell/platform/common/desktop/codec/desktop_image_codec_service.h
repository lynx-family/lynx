...
using DesktopImageCodecFactory =
    fml::RefPtr<Codec> (*)(std::shared_ptr<skity::Data> encoded_data);
...
class DesktopImageCodecService
 ...
 public:
  ...
  fml::RefPtr<Codec> CreateCodec(
      const std::shared_ptr<skity::Data>& encoded_data) const {
    for (auto factory : factories_) {
      if (auto codec = factory(encoded_data)) {
        return codec;
      }
    }
    return nullptr;
