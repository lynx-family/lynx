...
TEST_F(TransformLayerTest, RasterTransformIgnoresStackingZ) {
  ...
  auto layer = std::make_shared<TransformLayer>(
      operations, skity::Vec2{0.0f, 0.0f}, skity::Vec2{0.0f, 0.0f});
  auto mutator = AnimationMutator::Create(
      clay::ElementId(1), AnimationMutatorType::kTransform, layer.get());
  lynx::gfx::TransformOperations animated_operations;
  animated_operations.AppendTranslate({10.0f, lynx::gfx::LengthUnit::kNumber},
                                      {20.0f, lynx::gfx::LengthUnit::kNumber},
                                      {50.0f, lynx::gfx::LengthUnit::kNumber});
  mutator->SetProperty(ClayAnimationPropertyType::kTransform,
                       animated_operations, true);

  auto animation_host = std::make_shared<AnimationHost>();
  animation_host->AddAnimationMutator(layer.get(), mutator);

  EXPECT_FLOAT_EQ(GetTranslateZ(mutator->asTransform()->transform()), 50.0f);
