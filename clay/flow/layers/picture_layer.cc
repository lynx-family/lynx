...
PictureLayer::PictureLayer(const skity::Vec2& offset,
  ...
  if (picture_.object() != nullptr) {
    ...
    picture_raster_cache_item_ = PictureRasterCacheItem::Make(
        picture_.object()->raw(), offset_, is_complex, will_change);
  }
}
#else
PictureLayer::PictureLayer(const skity::Vec2& offset,
  ...
  if (picture_.object() != nullptr) {
    ...
    bounds_ = skity::Rect::MakeLTRB(rect.Left(), rect.Top(), rect.Right(),
                                    rect.Bottom());
    picture_raster_cache_item_ = PictureRasterCacheItem::Make(
        picture_.object()->raw(), picture_.object()->unique_id(), offset_,
...
#ifndef ENABLE_SKITY
  context->has_dl_running_animation = HasAnimationRunning();
  picture_raster_cache_item_->SetHasRunningTransformAnimation(
      context->has_dl_running_animation);
#endif  // ENABLE_SKITY
...
// TODO(feiyue.1998): Support lazy image logic in skia.
  ...
  if (has_lazy_image_) {
    has_lazy_image_ =
        clay::SkityLazyImageTraveller::Traversal(picture(), decode_callback);
  }
#endif  // ENABLE_SKITY
