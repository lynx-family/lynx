...
void PictureRasterCacheItem::PrerollSetup(PrerollContext* context,
  ...
  if (context->has_deferred_image ||
      !IsPictureWorthRasterizing(
          picture_, will_change_ || context->has_dl_running_animation,
          is_complex_ || has_running_transform_animation_,
...
void PictureRasterCacheItem::PrerollFinalize(PrerollContext* context,
  ...
  if (cache_state_ == CacheState::kNone || !context->raster_cache ||
      !context->raster_cached_entries || context->has_dl_running_animation ||
      context->has_deferred_image) {
    return;
  }
