...
void FrameSurfaceLayer::Diff(DiffContext* context, const Layer* old_layer) {
  ...
  const bool has_drawable_image =
      surface_->drawable_image_damage_rect().has_value();
  if (has_drawable_image) {
  ...
  if (!context->IsSubtreeDirty()) {
    ...
    if (!can_use_child_damage) {
      ...
      context->MarkSubtreeDirty(context->GetOldLayerPaintRegion(old_layer));
    } else if (current_surface_id == prev->surface_id() && has_drawable_image &&
               !AddMappedChildDamage(context,
                                     *surface_->drawable_image_damage_rect())) {
      context->MarkSubtreeDirty(context->GetOldLayerPaintRegion(old_layer));
    }
  }
...
bool FrameSurfaceLayer::AddMappedSurfaceDamage(DiffContext* context) const {
  ...
  return AddMappedChildDamage(context, child_damage);
}

bool FrameSurfaceLayer::AddMappedChildDamage(DiffContext* context,
                                             skity::Rect child_damage) const {
  if (!surface_->layer_tree()) {
    return false;
  }
