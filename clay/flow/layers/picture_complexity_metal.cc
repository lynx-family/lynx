...
void PictureMetalComplexityCalculator::MetalHelper::OnDrawAtlasImage(
    const std::vector<uint32_t> key, const std::shared_ptr<skity::Image> image,
    const skity::Rect& src, const skity::Rect& dst,
    const skity::SamplingOptions& sampling, const skity::Paint* paint) {
  OnDrawImageRect(image, src, dst, sampling, paint);
}
