...
void TransformLayer::Preroll(PrerollContext* context) {
  bool prev = context->has_running_transform_animation;
  context->has_running_transform_animation |= HasAnimationRunning();
  ...
  context->has_running_transform_animation = prev;
}
