...
class FrameChildPageHost final : public clay::RenderDelegate,
 ...
 public:
  FrameChildPageHost(clay::ViewContext* parent_view_context,
  ...
  bool UpdateMetaData(const std::shared_ptr<FrameChildData>& data,
                      const std::shared_ptr<FrameChildData>& global_props);
  ...
  int ForwardNativeViewHitTest(const clay::FloatPoint& position,
 ...
 private:
  ...
  FrameChildRuntimeOptions BuildChildRuntimeOptions(
  ...
  std::shared_ptr<FrameChildRuntimeFactory> runtime_factory_;
  ...
  std::unique_ptr<FrameChildRuntime> runtime_;
