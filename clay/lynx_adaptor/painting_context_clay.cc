...
PaintingContextClay::PaintingContextClay(
    ...
    std::shared_ptr<FrameChildRuntimeFactory> frame_child_runtime_factory)
    : view_context_(view_context),
      frame_child_runtime_factory_(std::move(frame_child_runtime_factory)) {
...
void PaintingContextClay::CreatePaintingNode(
  ...
  auto task = [view_context = view_context_, id, tag = std::move(resolved_tag),
               painting_data, flatten, create_node_async, node_index,
               this]() mutable {
...
void PaintingContextClay::LoadFrameChildBundle(int tag) {
  ...
  if (!host) {
    host = std::make_unique<FrameChildPageHost>(view_context_, resource_loader_,
                                                frame_child_runtime_factory_);
  }
  ...
  frame_view->SetEventForwarder(
      ...
      [host_ptr](const clay::FloatPoint& position, int platform_try_hit_id,
                 bool* has_hit_target) {
        return host_ptr->ForwardNativeViewHitTest(
            position, platform_try_hit_id, has_hit_target);
      });
