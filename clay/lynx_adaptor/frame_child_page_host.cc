...
FrameChildPageHost::FrameChildPageHost(
    ...
    std::shared_ptr<FrameChildRuntimeFactory> runtime_factory)
...
FrameChildRuntimeOptions FrameChildPageHost::BuildChildRuntimeOptions(
  ...
  FrameChildRuntimeOptions options;
  options.resource_loader = resource_loader_;
  options.device_pixel_ratio = dpr;
  options.viewport_width = std::max(
      parent_page_view_->ConvertTo<clay::kPixelTypeLogical>(
          viewport_size.width()),
      0.f);
  options.viewport_height = std::max(
      parent_page_view_->ConvertTo<clay::kPixelTypeLogical>(
          viewport_size.height()),
      0.f);
  options.embedded_mode = frame_view->embedded_mode();
  options.enable_multi_async_thread = frame_view->enable_multi_async_thread();
  return options;
}
...
void FrameChildPageHost::EnsureChildPage(clay::FrameView* frame_view) {
  ...
  auto options = BuildChildRuntimeOptions(frame_view);
  ui_delegate_ = std::make_unique<UIDelegateClay>(child_view_context_.get());
  ui_delegate_->SetFrameChildRuntimeFactory(runtime_factory_);

  if (runtime_factory_) {
    runtime_ = runtime_factory_->CreateRuntime(ui_delegate_.get(), options);
  }
}
...
bool FrameChildPageHost::LoadBundle(
    ...
    const std::shared_ptr<FrameChildData>& global_props) {
  ...
  if (!runtime_) {
    FML_LOG(ERROR) << "No frame child runtime is available for this platform";
    return false;
  }

  UpdateViewport(frame_view);
  if (!runtime_->LoadBundle(frame_view->url(), *bundle, data, global_props)) {
    return false;
  }
...
bool FrameChildPageHost::UpdateMetaData(
  ...
  return runtime_->UpdateMetaData(data, global_props);
}
...
void FrameChildPageHost::UpdateViewport(clay::FrameView* frame_view) {
  ...
  const auto& parent_metrics = parent_page_view_->GetViewportMetrics();
  const auto viewport_size = frame_view->child_viewport_size();
  // FrameView uses Clay pixels: physical on Android and logical elsewhere.
  const float viewport_width = std::max(viewport_size.width(), 0.f);
  const float viewport_height = std::max(viewport_size.height(), 0.f);
  const float logical_width =
      parent_page_view_->ConvertTo<clay::kPixelTypeLogical>(viewport_width);
  const float logical_height =
      parent_page_view_->ConvertTo<clay::kPixelTypeLogical>(viewport_height);
  const float physical_width =
      parent_page_view_->ConvertTo<clay::kPixelTypePhysical>(viewport_width);
  const float physical_height =
      parent_page_view_->ConvertTo<clay::kPixelTypePhysical>(viewport_height);
  ...
  metrics.physical_width = std::lround(physical_width);
  metrics.physical_height = std::lround(physical_height);
  child_page_view_->SetViewportMetrics(metrics);
  if (!viewport_changed) {
    return;
  }
  if (runtime_) {
    runtime_->UpdateViewport(logical_width, width_mode, logical_height,
                             height_mode, true);
  }
  child_viewport_size_ = viewport_size;
  child_viewport_device_pixel_ratio_ = dpr;
...
int FrameChildPageHost::ForwardNativeViewHitTest(
    const clay::FloatPoint& position, int platform_try_hit_id,
    bool* has_hit_target) {
  if (has_hit_target) {
  ...
  clay::FloatPoint child_position = frame_view->GetPointBySelf(position);
  child_position.Move(-frame_view->ContentInsetLeft(),
                      -frame_view->ContentInsetTop());
  return child_page_view_->GetHitTestingTargetNativeViewId(
      child_position, platform_try_hit_id, has_hit_target);
}
