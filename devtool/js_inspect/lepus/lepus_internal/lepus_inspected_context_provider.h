...
class LepusInspectedContextProvider {
 public:
  ...
  // Registers an override factory and returns the previously registered one
  // (nullptr if none). Passing nullptr restores the default open-source
  // implementation.
  static Factory RegisterFactory(Factory factory);
};
