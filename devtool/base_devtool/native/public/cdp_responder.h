...
class CDPResponder {
 public:
  ...
  // Read id_ with operator* only after has_value(). libc++ marks value()
  // unavailable before iOS 12 because its empty path throws
  // bad_optional_access.
  ...
  void SendSuccess(Json::Value result = Json::Value(Json::objectValue)) {
    ...
    if (id_.has_value()) {
      response["id"] = static_cast<Json::Int64>(*id_);
    }
