...
void LynxDevToolMediator::Init(
  ...
  element_executor->SetGlobalPropsEnabled(global_props_enabled);
  {
    ...
    element_executor->SetDOMState(dom_enabled_, Json::Value());
...
void LynxDevToolMediator::DispatchDOMStateChange(
  ...
  dom_enabled_ = enable;
  // Keep cache and queue order aligned without invoking callbacks under the
  // lock.
  tasm_task_runner_->PostTask(
      [element_executor = element_executor_, sender, message, enable]() {
        if (enable) {
