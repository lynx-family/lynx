...
TEST_F(InspectorHSRAgentTest, ForwardsAllLoadSourcesWithoutChangingText) {
  facade_.handler = [](auto callback) { callback("{}", ""); };
...
TEST_F(InspectorHSRAgentTest, ReturnsJsonValuesAndUndefined) {
  for (const auto* result :
    ...
    facade_.handler = [result](auto callback) { callback(result, ""); };
...
TEST_F(InspectorHSRAgentTest,
  ...
  facade_.handler = [](auto callback) {
    callback(R"({"valueType":"undefined"})", "");
  };
...
TEST_F(InspectorHSRAgentTest, DoesNotAcknowledgeBeforeAsynchronousCompletion) {
  ...
  std::thread worker([callback = std::move(callback)]() mutable {
    callback(R"({"valueType":"json","value":4})", "");
    callback("", "late duplicate failure");
  });
  worker.join();
  auto response = dispatcher_.sender->WaitForResponse();
  Drain();
...
TEST_F(InspectorHSRAgentTest, ReportsRuntimeErrorsInsteadOfPartialResults) {
  facade_.handler = [](auto callback) {
    callback("{}", "Script initialization failed");
  };
  auto response =
      Dispatch("HSR.loadScript",
               Parse(R"({"source":{"type":"inline","script":"throw 1"}})"));
  ExpectError(response, CDPErrorCode::ServerError);
  EXPECT_EQ(response["error"]["message"].asString(),
...
TEST_F(InspectorHSRAgentTest,
       ReportsAbandonedCallbackInsteadOfFallbackSuccess) {
  facade_.handler = [](auto callback) {};
  ExpectError(Dispatch("HSR.evaluate", Parse(R"({"expression":"0"})")),
              CDPErrorCode::ServerError);
}

TEST_F(InspectorHSRAgentTest, RejectsMalformedCompletionPayloads) {
  for (const auto* result :
       {"garbage", "null", "[]", "{}",
        R"({"valueType":"json","value":4} trailing)", R"({"valueType":"json"})",
        R"({"valueType":"undefined","value":null})", R"({"valueType":0})",
        R"({"valueType":"promise"})"}) {
    facade_.handler = [result](auto callback) { callback(result, ""); };
    ExpectError(Dispatch("HSR.evaluate", Parse(R"({"expression":"0"})")),
                CDPErrorCode::InternalError);
  }
  facade_.handler = [](auto callback) { callback(R"({"accepted":true})", ""); };
