...
void ApplyRootBoxModelOffset(std::vector<double>& box_model,
  ...
  if (root_box_model.size() != kBoxModelSize) {
    return;
  }
...
void ApplyBoxModelToComputedStyle(ComputedStyleMap& dict,
  ...
  if (dict["border-left"] == dict["border-right"] &&
    ...
    border_str << dict["border-top"] << " " << dict["border-right"] << " "
...
void InspectorTasmExecutor::DOM_GetBoxModel(
  ...
  auto send_response = [sender, response](Json::Value content) mutable {
    if (content.empty()) {
  ...
  if (ptr == nullptr || ptr->IsDetached() ||
      !ElementInspector::HasDataModel(ptr)) {
    send_response(BuildBoxModelError());
    return;
  }
        ...
        if (BuildBoxModelQuery(ptr, query)) {
          ...
          if (devtool_mediator) {
            devtool_mediator->GetBoxModel(
                query,
                [sender, response, dict = std::move(dict),
                 layouts_unit_per_px](std::vector<double> box_info) mutable {
                  Json::Value content(Json::ValueType::objectValue);
...
Json::Value InspectorTasmExecutor::GetDocumentBodyFromNodeWithBoxModel(
  ...
  auto set_node_func = [this, &queries, &box_model_paths, &node_path](
    ...
    tasm::Element* box_model_element = ptr;
    if (ElementInspector::IsNeedEraseId(box_model_element)) {
      box_model_element =
          ElementInspector::GetChildElementForComponentRemoveView(
              box_model_element);
    }
    if (box_model_element != nullptr && !box_model_element->IsDetached() &&
        ElementInspector::HasDataModel(box_model_element)) {
      InspectorBoxModelQuery query;
      if (BuildBoxModelQuery(box_model_element, query)) {
  ...
  if (comp_ptr && ElementInspector::IsNeedEraseId(comp_ptr)) {
    ...
    size_t current_res_paths_begin = box_model_paths.size();
    set_node_func(res, ptr);

    Json::Value current_res = res;
    while (1) {
      size_t comp_paths_begin = box_model_paths.size();
      Json::Value comp = Json::Value(Json::ValueType::objectValue);
      set_node_func(comp, comp_ptr);
      comp["childNodeCount"] = 1;
      Json::ArrayIndex child_index = comp["children"].size();
      InsertChildIndexInPaths(box_model_paths, current_res_paths_begin,
                              comp_paths_begin, node_path.size(), child_index);
      ...
      current_res_paths_begin = comp_paths_begin;
    }
