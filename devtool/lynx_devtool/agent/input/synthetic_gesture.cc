...
constexpr int64_t kFallbackFrameIntervalMs = 16;
...
bool SyntheticTapGesture::Inject(PointerEventType type, int64_t frame_time_us,
  ...
  event.type = type;
  event.changed_pointer_id = pointer_id_;
...
SyntheticGestureResult SyntheticTapGesture::ForwardInputEvents(
  ...
  if (!target || state_ == State::kDone) {
    return state_ == State::kDone ? SyntheticGestureResult::kDone
...
void SyntheticGestureController::RequestNextFrame() {
  ...
  task_runner_->PostDelayedTask(
      ...
      fml::TimeDelta::FromMilliseconds(kFallbackFrameIntervalMs));
}
