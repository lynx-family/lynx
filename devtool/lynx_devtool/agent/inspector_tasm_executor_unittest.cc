...
TEST_F(InspectorTasmExecutorTest,
  ...
  Json::Value snapshot =
      element_executor_->BuildAnimationSnapshot(animation.get());
...
TEST_F(InspectorTasmExecutorTest,
  ...
  auto snapshot = element_executor_->BuildAnimationSnapshot(animation.get());
  EXPECT_EQ(snapshot["source"], original["source"]);

  // Rule cleanup must not alter metadata for an already-created model.
  rules.clear();
  snapshot = element_executor_->BuildAnimationSnapshot(animation.get());
  EXPECT_EQ(snapshot["source"], original["source"]);
...
TEST_F(InspectorTasmExecutorTest,
       AnimationEnableBackfillsCreatedBeforeStartedEvents) {
  ...
  auto second =
      BuildAnimationForSnapshot("corePulse", 1200, {0.0, 1.0},
                                animation::Animation::Origin::kCSSAnimation);
  element_executor_->OnAnimationCreated(first.get());
  element_executor_->OnAnimationCreated(second.get());
  ...
  element_executor_->AnimationEnable(response_sender, message);
  FlushDevtoolTasks();
...
TEST_F(InspectorTasmExecutorTest,
       AnimationSeekAnimationsSeeksBatchWithSharedReferenceTime) {
  auto first = BuildAnimationForSnapshot(
  ...
  element_executor_->AnimationSeekAnimations(response_sender, message);
...
TEST_F(InspectorTasmExecutorTest,
       AnimationSeekAnimationsRejectsInvalidBatchAtomically) {
  auto animation = BuildAnimationForSnapshot(
      "orbit", 3000, {0.0, 1.0}, animation::Animation::Origin::kCSSAnimation);
  element_executor_->OnAnimationCreated(animation.get());
...
TEST_F(InspectorTasmExecutorTest,
       AnimationSeekAnimationsValidatesTimeAndAllowsEmptyBatch) {
  auto response_sender = std::make_shared<RecordingMessageSender>();
  Json::Value message(Json::ValueType::objectValue);
  message["id"] = 204;
  message["params"]["animations"] = Json::Value(Json::arrayValue);
...
TEST_F(InspectorTasmExecutorTest,
       AnimationSetPausedPausesAndResumesBatchAtSharedReferenceTime) {
  auto first = BuildAnimationForSnapshot(
      "orbit", 3000, {0.0, 1.0}, animation::Animation::Origin::kCSSAnimation);
  auto second = BuildAnimationForSnapshot(
      "pulse", 3000, {0.0, 1.0}, animation::Animation::Origin::kWebAnimation);
  ...
  element_executor_->AnimationSetPaused(response_sender, message);
...
TEST_F(InspectorTasmExecutorTest,
       AnimationSetPausedRejectsInvalidBatchAtomically) {
  auto animation = BuildAnimationForSnapshot(
      "orbit", 3000, {0.0, 1.0}, animation::Animation::Origin::kCSSAnimation);
...
TEST_F(InspectorTasmExecutorTest,
       AnimationSetPausedValidatesBooleanAndAllowsEmptyBatch) {
  auto response_sender = std::make_shared<RecordingMessageSender>();
  Json::Value message(Json::ValueType::objectValue);
  message["id"] = 210;
  message["params"]["animations"] = Json::Value(Json::arrayValue);
...
TEST_F(InspectorTasmExecutorTest,
       InlineStyleUpdatesFlushedStreamsOneCoalescedEventPerFlushWhileEnabled) {
  auto first_target = CreateInlineStyleUpdateTarget();
  auto second_target = CreateInlineStyleUpdateTarget();
  auto event_sender = std::make_shared<RecordingMessageSender>();
  devtools_ng_->message_sender_ = event_sender;
  ...
  Json::Value enable(Json::ValueType::objectValue);
  enable["id"] = 301;
  element_executor_->DOM_Enable(response_sender, enable);
  ...
  ASSERT_EQ(event_sender->json_messages_.size(), 2U);
  const Json::Value& first_event = event_sender->json_messages_[0].second;
  EXPECT_EQ(first_event["method"].asString(), "DOM.inlineStyleUpdatesFlushed");
  EXPECT_FALSE(first_event.isMember("id"));
  EXPECT_GT(first_event["params"]["timestamp"].asDouble(), 0);
  const Json::Value& node_updates = first_event["params"]["nodeUpdates"];
  ASSERT_EQ(node_updates.size(), 2U);

  const Json::Value* first_node_update = nullptr;
  const Json::Value* second_node_update = nullptr;
  for (const Json::Value& node_update : node_updates) {
    if (node_update["backendNodeId"].asInt() == first_target->impl_id()) {
      first_node_update = &node_update;
    } else if (node_update["backendNodeId"].asInt() ==
               second_target->impl_id()) {
      second_node_update = &node_update;
    }
  }
  ASSERT_NE(first_node_update, nullptr);
  ASSERT_NE(second_node_update, nullptr);

  bool saw_transform = false;
  bool saw_opacity_removal = false;
  bool saw_numeric_width = false;
  for (const Json::Value& property : (*first_node_update)["properties"]) {
    if (property["name"].asString() == "transform") {
      saw_transform = property["value"].asString() == "translateX(12px)";
    } else if (property["name"].asString() == "opacity") {
      saw_opacity_removal =
  ...
  EXPECT_TRUE(saw_opacity_removal);
  EXPECT_TRUE(saw_numeric_width);
  ASSERT_EQ((*second_node_update)["properties"].size(), 1U);
  EXPECT_EQ((*second_node_update)["properties"][0]["value"].asString(), "0.8");

  const Json::Value& second_event = event_sender->json_messages_[1].second;
  EXPECT_EQ(second_event["method"].asString(), "DOM.inlineStyleUpdatesFlushed");
  ASSERT_EQ(second_event["params"]["nodeUpdates"].size(), 1U);
  EXPECT_EQ(second_event["params"]["nodeUpdates"][0]["backendNodeId"].asInt(),
            second_target->impl_id());
  EXPECT_EQ(second_event["params"]["nodeUpdates"][0]["properties"][0]["value"]
...
TEST_F(InspectorTasmExecutorTest, DOMDisableDropsPendingInlineStyleUpdates) {
  auto target = CreateInlineStyleUpdateTarget();
  auto event_sender = std::make_shared<RecordingMessageSender>();
  devtools_ng_->message_sender_ = event_sender;
  auto response_sender = std::make_shared<RecordingMessageSender>();
